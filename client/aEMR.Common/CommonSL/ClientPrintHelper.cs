    ...
    public class AxonActiveXPrinter : ActiveXPdfPrinterBase
    {
                    ...
                    if (!PrinterServerAvailable)
                    {
                        if (AutomationFactory.IsAvailable)
                        {
                            ...
                            if (PrintServer != null)
                            {
                                ...
                                PrintServer.InitPrintServer(string.Empty, initRetCode);
                            }
                        }
                        else
                        {
                            var retCode = HtmlPage.Window.Invoke("checkAxonPrinterInstallCorrectly");
                            if ((bool)retCode)
                            {
                                PrinterServerAvailable = true;
                                //@"\\AX03\Canon MF4320-4350"
                                HtmlPage.Window.Invoke("initAxonPrinter", string.Empty, initRetCode);
                            }
                        }
                    }
                }
            ...
            try
            {
                if (PrinterServerAvailable)
                {
                    if (Application.Current.IsRunningOutOfBrowser)
                    {
                        if (PrintServer != null)
                        {
                            PrintServer.InitPrintServer(PrinterName, retCode);
                        }
                    }
                    else
                    {
                        HtmlPage.Window.Invoke("initAxonPrinter", PrinterName, retCode);
                    }
                }
                else
                {
        ...
        public override void PrintPdfFile(string fileName)
        {
            if (PrinterServerAvailable)
            {
                ...
                {
                    //TODO:Sau nay can thi doc file roi in.
                }
            }
            else
            {
        ...
        public override void PrintPdfData(byte[] data)
        {
            if (PrinterServerAvailable)
            {
                ...
                else
                {
                    //Chuyen sang base64 dua xuong javascript.
                    string base64 = System.Convert.ToBase64String(data);
                    HtmlPage.Window.Invoke("printBase64StringUseAdobePrinter", base64, PrinterName, PrinterMode);
                } 
            }
            else
            {
        ...
        public override void PrintPdfDataInBase64String(string base64String)
        {
            if (PrinterServerAvailable)
            {
                ...
                {
                    HtmlPage.Window.Invoke("printBase64StringUseAdobePrinter", base64String, PrinterName, PrinterMode);
                }
            }
            else
            {
        ...
        public static ObservableCollection<PrinterInfo> GetAvailablePrinters()
        {
            ...
            if (PrinterServerAvailable)
            {
                ...
                {
                    json = (string)HtmlPage.Window.Invoke("getAllPrintersInJSON");
                }
                if (!string.IsNullOrWhiteSpace(json))
                {
                    json = json.Replace("\\", "\\\\");
