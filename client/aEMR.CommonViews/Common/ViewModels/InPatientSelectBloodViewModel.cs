...
{
    [Export(typeof(IInPatientSelectBlood)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class InPatientSelectBloodViewModel : Conductor<object>, IInPatientSelectBlood
    {
        ...
        public InPatientSelectBloodViewModel(IWindsorContainer container, INavigationService navigationService, ISalePosCaching salePosCaching)
        {
                        ...
                        contract.BeginGetMedicalServiceTypesByInOutType(outTypes,
                            Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    ...
                                    foreach (RefMedicalServiceType info in allItems.Where(x=>x.V_RefMedicalServiceTypes == (long)AllLookupValues.V_RefMedicalServiceTypes.MAU))
                                    {
                ...
                try
                {
                    using (var serviceFactory = new PatientRegistrationServiceClient())
                    {
                        ...
                        contract.BeginGetAllMedicalServiceItemsByType(serviceTypeID, CurrentInPatientAdmDisDetail == null ? null : CurrentInPatientAdmDisDetail.MedServiceItemPriceListID,
                            (long)AllLookupValues.V_RefMedicalServiceTypes.MAU,Globals.DispatchCallback((asyncResult) =>
                            {
