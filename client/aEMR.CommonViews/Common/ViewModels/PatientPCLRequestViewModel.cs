...
{
    [Export(typeof(IPatientPCLRequest)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class PatientPCLRequestViewModel : ViewModelBase, IPatientPCLRequest
        , IHandle<DbClickSelectedObjectEventWithKey<PatientPCLRequest, String>>
        , IHandle<ReLoadListPCLRequest>
        , IHandle<SetInPatientInfoAndRegistrationForPatientPCLRequest>
        , IHandle<DbClickSelectedObjectEventWithKeyToShowDetails<PatientPCLRequest, String>>
    {
        ...
        public PatientPCLRequestViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventAgr, ISalePosCaching salePosCaching)
        {
        ...
        private void CreateSubVM()
        {
            ...
            uc4.SearchCriteria.LoaiDanhSach = AllLookupValues.PatientPCLRequestListType.DANHSACHPHIEUYEUCAU;
            uc4.SearchCriteria.V_PCLMainCategory = (long)AllLookupValues.V_PCLMainCategory.Laboratory;
            UCPatientPCLRequestList = uc4;

            var uc5 = Globals.GetViewModel<IPatientPCLRequestEdit>();
            uc5.CallByPCLRequestViewModel = Common.LeftModuleActive.KHAMBENH_CLS_PHIEUYEUCAUXETNGHIEM;
        ...
        protected override void OnActivate()
        {
            ...
            ActivateSubVM();
            if (IsShowSummaryContent && Registration_DataStorage != null && Registration_DataStorage.CurrentPatient != null)
            {
                InitPatientInfo();
            }
            //▼====== #001
            if (Globals.LeftModuleActive == LeftModuleActive.KHAMBENH_CLS_PHIEUYEUCAUXETNGHIEM_NT)
            {
                var homevm = Globals.GetViewModel<IHome>();
                IInPatientOutstandingTask ostvm = Globals.GetViewModel<IInPatientOutstandingTask>();
                ostvm.WhichVM = SetOutStandingTask.PHIEU_YEU_CAU_XET_NGHIEM;
                homevm.OutstandingTaskContent = ostvm;
                homevm.IsExpandOST = true;
            }
            if (Globals.LeftModuleActive == LeftModuleActive.KHAMBENH_CLS_PHIEUYEUCAUXETNGHIEM)
            {
                var homevm = Globals.GetViewModel<IHome>();
        ...
        public IPatientPCLRequestEdit UCPatientPCLRequestEdit
        {
            get;
            set;
        }
        ...
        private void SetValueListPCLRequest_Common()
        {
            ...
            UCPatientPCLRequestList.SearchCriteria.V_PCLMainCategory = (long)AllLookupValues.V_PCLMainCategory.Laboratory;
        ...
        //▼====== #001
        public void Handle(SetInPatientInfoAndRegistrationForPatientPCLRequest message)
        {
            if (message != null)
            {
                InitPatientInfo();
