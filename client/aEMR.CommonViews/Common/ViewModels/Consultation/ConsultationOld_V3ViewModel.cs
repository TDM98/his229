...
* 20191007 TBL  #016:   BM 0017422: Bổ sung giờ bắt đầu và giờ kết thúc của quy trình khám bệnh vào bảng PatientRegistrationDetails
...
* 20200411 TBL  #018:   BM 0030109: Kiểm tra danh sách ICD10 có tồn tại ICD10 nào ngưng sử dụng khi bấm tạo mới dựa trên cũ
* 20200620 TBL  #019:   BM 0032126: Cho phép chỉnh sửa diễn tiến bệnh của đăng ký cũ đã khám bệnh
* 20210427 #020 TNHX:   Chọn ICD sao kèm theo nếu ICD chính có dấu găm
* 20210428 #021 TNHX:   Lấy hồ sơ bệnh án theo đợt điều trị
* 20210611 #022 TNHX:   346 Kiểm tra list ICD đã nhập theo quy tắc
* 20210701 #023 TNHX:   260 Thêm user bsi mượn vào chẩn đoán
* 20210710 #024 TNHX:   CHỉnh lại điều kiện kiểm tra mã bệnh kèm theo *. DO bsi chỉnh trực tiếp => bỏ sót kiểm tra
* 20211021 #025 TNHX:   CHỉnh lại điều kiện ràng buộc DHST chiều cao, cân nặng bắt buộc tất cả
* 20220318 #026 QTD:    Truyền thêm PtRegistrationID + V_RegistrationType cho popup Thông tin bệnh án
* 20220530 #027 BLQ: Kiểm tra thời gian thao tác của bác sĩ
* 20220608 #028 BLQ: Thêm dòng cảnh báo đã đầy cổng 
* 20220604 #029 QTD:    Disable ButtonControlsEnable khi tìm ĐK ngoại trú đã nhập viện
* 20220624 #030 BLQ: Truyền Triệu chứng/ Dấu hiệu lâm sàng và Lý do vào ngoại trú vào màn hình bệnh án điều trị ngoại trú 
* 20220625 #031 BLQ: 
*  + Thêm điều kiện khi chọn cách điều trị "Điều trị ngoại trú" thì mới hiện tích và danh sách điều trị
*  + Lấy ICD chính cuối cùng của đợt điều trị ngoại trú
*  + Lấy danh sách ICD cấu hình cho điều trị ngoại trú
*  20220729 #032 TNHX: 1482 - Thêm chặn không cho tạo giấy chuyển tuyến khi còn CLS chưa thực hiện
*  + 1754 : Lỗi khi thêm mới ICD10 làm bệnh chính là không xóa cái hiện tại khi thêm mã găm 
* 20220811 #033 DatTB: Chỉnh sửa màn hình hồ sơ ĐTNT
* + Thêm ShowDialog_V5 sử dụng Dictionary để truyền thêm title cho Dialog
* 20220812 #034 DatTB: Chỉnh sửa màn hình hồ sơ ĐTNT
* + Thêm hàm check toa thuốc để kiểm tra chỉnh sửa loại hình ĐTNT
* 20220915 #035 BLQ: Thêm điều kiện khi xóa đợt điều trị để lấy lại check điều trị ngoại trú
*                   + Ẩn nút check không cho người dùng tự check nữa
* 20221028 #036 BLQ: Truyền triệu chứng qua đề nghị nhập viện từ màn hình tiêu chí nhập viện
* 20221231 #037 BLQ: Truyền thông tin chẩn đoán vào màn hình Hồ sơ bệnh án
*                   + Thêm thông tin PatientID truyền qua phiếu tự khai
* 20230109 #038 DatTB: Thêm phiếu tự khai và cam kết điều trị
* 20230408 #039 BLQ: Thêm phần sắp xếp ICD khi đánh ICD găm
* 20230516 #040 QTD: Thêm ICD cho màn hình chỉ định dịch vụ
* 20230517 #041 DatTB: Khóa/mở nút giấy chuyển tuyến theo loại khám
* 20230603 #042 DatTB: Upgrade Devexpress: Cập nhật thêm thư viện using DevExpress.Data.Extensions;
* 20230624 #043 DatTB: Thay đổi điều kiện gàng buộc ICD
*/
namespace aEMR.Common.ViewModels
{
    [Export(typeof(IConsultationOld_V3)), PartCreationPolicy(CreationPolicy.NonShared)]
    public partial class ConsultationOld_V3ViewModel : ViewModelBase, IConsultationOld_V3
        ...
        , IHandle<SelectedDiagnosisTreatmentForConfirmSmallProceduce>
        ...
        , IHandle<AdmissionCriterion_Symptom_Select_Event>
        , IHandle<ConfirmOutPtTreatmentProgram>
        , IHandle<DeleteOutPtTreatmentProgramItem>
        , IHandle<PCLExamAccordingICD_Event>
    {
        ...
        public ConsultationOld_V3ViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            authorization();
            ...
            if (Globals.ServerConfigSection.CommonItems.LocationAllowPrenatalCertificates.ToString().Contains("|" + Globals.DeptLocation.LID.ToString() + "|"))
            {
                IsEnableGiayNDT = true;
            }
        }
        ...
        {
            CheckIsEnableAddPrescription();
            CheckInPtRegistration();
            if (DiagTrmtItem.V_TreatmentType != (long)AllLookupValues.V_TreatmentType.SmallProcedure)
            {
                Globals.EventAggregator.Publish(new CheckValidProcedure { IsHaveOneProcedure = true });
            }
            else if (Registration_DataStorage != null)
            {
                CheckValidProcedure(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID);
            }
            //▼====: #031
            NotifyOfPropertyChange(() => IsHasOldRegistration);
            //▲====: #031
            //▼==== #041
            if (DiagTrmtItem != null && DiagTrmtItem.V_TreatmentType > 0)
            {
                btGCTIsEnabled = DiagTrmtItem.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.Transfer;
            }
            //▲==== #041
        }
        private void CheckValidProcedure(long PtRegistrationID)
        {
            ...
            var t = new Thread(() =>
            {
                using (var serviceFactory = new ePMRsServiceClient())
                {
                    ...
                    contract.BeginCheckValidProcedure(PtRegistrationID, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            var results = contract.EndCheckValidProcedure(asyncResult);
                            Globals.EventAggregator.Publish(new CheckValidProcedure { IsHaveOneProcedure = results });
                        }
                        catch (Exception ex)
                        {
        ...
        private void CheckInPtRegistration()
        {
            ...
            {
                if (DiagTrmtItem.V_TreatmentType != 82501 && Registration_DataStorage != null && Registration_DataStorage.CurrentPatientRegistration.InPtRegistrationID != null)
                {
                    CheckStatusInPtRegistration_InPtRegistrationID(Convert.ToInt64(Registration_DataStorage.CurrentPatientRegistration.InPtRegistrationID));
                }
                else
                {
        ...
        private void CheckIsEnableAddPrescription()
        {
        ...
        public void InitPatientInfo(PatientRegistrationDetail PtRegDetail)
        {
            ...
            if (Registration_DataStorage != null && Registration_DataStorage.CurrentPatient != null && Registration_DataStorage.CurrentPatient.PatientID > 0)
            {
                if (PtRegDetail != null)
                {
                    NgayDK = PtRegDetail.RecCreatedDate;
                    DateStarted = PtRegDetail.DateStarted;
                    DateEnded = PtRegDetail.DateEnded;
                }
                //▼====== #010
                if (Registration_DataStorage.CurrentPatientRegistration != null)
                {
                ...
                //▲====== #010
                //▼===== #016
                //TBL: Khi tìm thấy đăng ký thì đi cập nhật giờ bắt đầu
                if (Registration_DataStorage.CurrentPatientRegistrationDetail != null && Registration_DataStorage.CurrentPatientRegistrationDetail.DateStarted == null)
                {
                    UpdateDateStarted_ForPatientRegistrationDetails(Registration_DataStorage.CurrentPatientRegistrationDetail.PtRegDetailID);
                }
                //▲===== #016
                if (CheckDangKyHopLe())
                {
                ...
                GetInjuryCertificates_ByPtRegID(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID, (long)AllLookupValues.RegistrationType.NGOAI_TRU);
                HiReportWarnVisible = Registration_DataStorage.CurrentPatientRegistration.HIReportID > 0;
                WarniningSapNhapVisible = !HiReportWarnVisible && Registration_DataStorage.CurrentPatientRegistration.IsAdmission;
                GetLastIDC10MainTreatmentProgram_ByPtRegID(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID, Registration_DataStorage.CurrentPatientRegistrationDetail.PtRegDetailID);
                GetVacationPrenatalCertificates_ByPtRegID(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID);
                GetVacationInsuranceCertificates_ByPtRegID(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID);
            }
            else
            {
                this.HideBusyIndicator();
        ...
        public bool IsInPtDiagnosis
        {
            get
            {
                ...
                return Registration_DataStorage != null && Registration_DataStorage.CurrentPatientRegistration != null
                    && Registration_DataStorage.CurrentPatientRegistration.BNTKSauXV.GetValueOrDefault()
                    && DiagTrmtItem != null && DiagTrmtItem.V_RegistrationType == AllLookupValues.RegistrationType.NOI_TRU;
            }
        }
        ...
        public bool FormEditorIsEnabled
        {
            ...
            set
            {
                ...
                NotifyOfPropertyChange(() => IsHasOldRegistration);
                if (IsEditingEnable != null)
                {
                    IsEditingEnable(FormEditorIsEnabled);
                }
            }
        }
        //▼====: #028
        private bool _HiReportWarnVisible = false;
        public bool HiReportWarnVisible
        {
            get
            {
                return _HiReportWarnVisible;
            }
            set
            {
                _HiReportWarnVisible = value;
                NotifyOfPropertyChange(() => HiReportWarnVisible);
            }
        }
        //▲====: #028
        ...
        private bool _DiagnosisFinalIsEnabled;
        ...
        public DateTime? DateStarted
        {
            ...
            {
                _DateStarted = value;
                NotifyOfPropertyChange(() => DateStarted);
            }
        }
        ...
        public DateTime? DateEnded
        {
            ...
            {
                _DateEnded = value;
                NotifyOfPropertyChange(() => DateEnded);
            }
        }
        #endregion
        ...
        private void ResetDefaultForBehaving()
        {
            ...
            if (RefMedRecTemplate != null && RefMedRecTemplate.Count > 0)
            {
            ...
            DiagTrmtItem.OrientedTreatment += "Bệnh tỉnh, tiếp xúc tốt, da niêm hồng, tim đều, phổi trong, bụng mềm";
        }
        ...
        private IEnumerator<IResult> AllCheck()
        {
            ...
            if (NeedICD10() && CheckedIsMain())
            {
                ...
                if (MainICD != null && Registration_DataStorage.ListRequiredSubDiseasesReferences != null)
                {
                ...
                if (requiredSubDiseasesReferences.Count() > 0)
                {
        ...
        public void AxAutoComplete_DropDownClosed(object sender, RoutedPropertyChangedEventArgs<bool> e)
        {
            ...
            if (refIDC10Item != null && Acb_ICD10_Code != null)
            {
                ...
                if (CheckCountIsMain())
                {
                    ...
                    if (String.IsNullOrEmpty(DiagTrmtItem.ReasonHospitalStay) && refIDC10List.Any(x => x.IsMain && x.DiseasesReference != null && x.DiseasesReference.IsActive))
                    {
                        DiagTrmtItem.ReasonHospitalStay += "Điều trị bệnh " + refIDC10List.Where(x => x.IsMain == true && x.DiseasesReference.IsActive == true).FirstOrDefault().DiseasesReference.DiseaseNameVN;
                    }
                }
        ...
        public void DiseaseName_DropDownClosed(object sender, RoutedPropertyChangedEventArgs<bool> e)
        {
            ...
            if (CheckCountIsMain())
            {
                ...
                if (String.IsNullOrEmpty(DiagTrmtItem.ReasonHospitalStay) && refIDC10List.Any(x => x.IsMain && x.DiseasesReference != null && x.DiseasesReference.IsActive))
                {
                    DiagTrmtItem.ReasonHospitalStay += "Điều trị bệnh " + refIDC10List.Where(x => x.IsMain && x.DiseasesReference.IsActive).FirstOrDefault().DiseasesReference.DiseaseNameVN;
                }
            }
            if (CheckExists(refIDC10Item))
            {
        ...
        public void Handle(SelectedRequireSubICDForDiagnosisTreatment message)
        {
            if (message != null && message.SubICDInfo != null)
            {
                if (message.MainICDIndex > 0)
                {
                    ...
                    //▼====: #032
                    // Cập nhật lại các ICD10 đang được tích là bệnh chính về bệnh kèm theo
                    if (!message.IsException)
                    {
                        foreach (var item in refIDC10List)
                        {
                            item.IsMain = false;
                        }
                    }
        ...
        public void ckbIsChecked_Click(object source, object sender)
        {
            ...
            if (String.IsNullOrEmpty(DiagTrmtItem.ReasonHospitalStay) && refIDC10List.Where(x => x.IsMain == true && x.DiseasesReference.IsActive == true).Count() > 0)
            {
                DiagTrmtItem.ReasonHospitalStay += "Điều trị bệnh " + refIDC10List.Where(x => x.IsMain == true && x.DiseasesReference.IsActive == true).FirstOrDefault().DiseasesReference.DiseaseNameVN;
            }
        }
        ...
        public bool CheckRequireSubICd(DiagnosisIcd10Items MainICD)
        {
            ...
            if (MainICD != null && Registration_DataStorage.ListRequiredSubDiseasesReferences != null)
            {
                foreach (var itemICD10 in Registration_DataStorage.ListRequiredSubDiseasesReferences.Where(x => x.MainICD10 == MainICD.ICD10Code))
                {
                    if (itemICD10.SubICDInfo != null && !tempICD10List.Contains(itemICD10.SubICDInfo.ICD10Code))
                    {
                        requiredSubDiseasesReferences.Add(itemICD10);
                    }
                    else
                    {
                        requiredSubDiseasesReferences = new ObservableCollection<RequiredSubDiseasesReferences>();
                        break;
                    }
                }
        ...
        public bool CheckRuleICd(DiagnosisIcd10Items MainICD = null)
        {
                    ...
                    if (itemICD10.ICD10Code != null && itemICD10.ICD10Code.Contains("*") && !itemICD10.IsMain)
                    {
                        ...
                        if (temp1 == null || temp1.Count() == 0)
                        {
                            ...
                            if (ruleDiseasesReferences.Count() == 1)
                            {
                                //▼====: #032
                                // Cập nhật lại các ICD10 đang được tích là bệnh chính về bệnh kèm theo
                                if (!ruleDiseasesReferences.FirstOrDefault().IsException)
                                {
                                    foreach (var item in refIDC10List)
                                    {
                                        item.IsMain = false;
            ...
            else if (MainICD.ICD10Code != null && MainICD.ICD10Code.Contains("*"))
            {
                ...
                if (Registration_DataStorage.ListRuleDiseasesReferences != null)
                {
                    ...
                    if (refIDC10List != null)
                    {
                        string tempICD10List = string.Join("; ", from item in refIDC10List
                                                                 where (!string.IsNullOrEmpty(item.ICD10Code) && item.DiseasesReference != null)
                                                                 select item.ICD10Code);
                        ObservableCollection<RuleDiseasesReferences> temp = Registration_DataStorage.ListRuleDiseasesReferences.Where(x => x.MainICD10 == MainICD.ICD10Code
                                                                                                            && tempICD10List.Contains(x.SubICD10)).ToObservableCollection();
                        if (temp == null || temp.Count() == 0)
                        {
                ...
                else if (ruleDiseasesReferences.Count() > 1)
                {
                    void onInitDlg(ISelectRuleICD typeInfo)
                    {
                        typeInfo.TitleForm = "Chọn mã bệnh kép kèm theo làm bệnh chính";
                        typeInfo.ListRuleDiseasesReferences = ruleDiseasesReferences;
                        typeInfo.MainICDIndex = MainICDIndex;
                    }
                    GlobalsNAV.ShowDialog<ISelectRuleICD>(onInitDlg, null, false, true, Globals.GetHalfHeightAndThreeFourthWidthDefaultDialogViewSize());
        ...
        public void lnkDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (item != null && item.ICD10Code != null && item.ICD10Code != "")
            {
                if (MessageBox.Show(eHCMSResources.Z0419_G1_CoMuonXoaMaICD10, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    ...
                    {
                        if (refIDC10List.Count == 2)
                        {
                            DiagTrmtItem.DiagnosisFinal = "";
                        }
                        else if (nSelIndex == refIDC10List.Count - 2)
                        {
        ...
        private bool CheckEmptyFields()
        {
            ...
            if (DiagTrmtItem.ReasonHospitalStay == null || DiagTrmtItem.ReasonHospitalStay.Trim() == "")
            {
                strWarningMsg += string.Format("{0} - ", eHCMSResources.Z3200_G1_LyDoVaoNgT);
            }
            if (Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID != null && IsOutPtTreatmentProgram
                && (DiagTrmtItem.Diagnosis == null || DiagTrmtItem.Diagnosis.Trim() == ""))
            {
                strWarningMsg += string.Format("{0} - ", eHCMSResources.G1785_G1_TrieuChungDHieuLS);
            }
        ...
        private IEnumerator<IResult> SaveNewDiagnosis()
        {
            ...
            //▼====: #031
            if (CS_DS.DiagTreatment.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.OutPtTreatment
                && Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID == null
                && IsOutPtTreatmentProgram)
            {
                Globals.ShowMessage("Bác sĩ chưa tick (nút) điều trị ngoại trú và chọn đợt điều trị", "Thông báo");
                yield break;
            }
            if (CS_DS.DiagTreatment.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.IssuedPrescription
                && Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID != null
                && IsOutPtTreatmentProgram)
            {
                Globals.ShowMessage("Bác sĩ chưa chọn đúng cách điều trị. Bệnh nhân đang nằm trong đợt điều trị", "Thông báo");
                yield break;
            }
            //▲====: #031
            if (CheckedIsMain() && NeedICD10())
            {
                //Chuyển code vào trong để kiểm tra có ICD trước tránh lỗi do không có ICD 
                //▼====: #031
                if (CS_DS.DiagTreatment.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.OutPtTreatment //IsOutPtTreatmentProgram //Lấy điều kiện theo cách điều trị để kiểm tra màn hình khám bệnh
                    && Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID != null
                    && !CheckIDC10MainInOutpatientTreatmentType())
                {
                    Globals.ShowMessage("ICD chính đang chọn chưa đúng theo quy định Điều trị ngoại trú." + Environment.NewLine +
                                        "ICD phải đúng trong danh sách quy định, ICD đúng loại hình điều trị." + Environment.NewLine +
                                        "Vui lòng liên hệ Phòng Kế Hoạch Tổng Hợp để được hỗ trợ", "Thông báo");
                    yield break;
                }
                //▼==== #043
                //string LastIDC10Main = "";
                //if (CS_DS.DiagTreatment.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.OutPtTreatment //IsOutPtTreatmentProgram //Lấy điều kiện theo cách điều trị để kiểm tra màn hình khám bệnh
                //    && Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID != null
                //    && !CheckLastIDC10MainTreatmentProgram(out LastIDC10Main))
                //{
                //    Globals.ShowMessage("ICD chính hiện tại đang không trùng khớp với ICD chính của đợt điều trị này. ICD chính của đợt hiện tại là: " + LastIDC10Main, "Thông báo");
                //    yield break;
                //}
                if (CS_DS.DiagTreatment.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.OutPtTreatment //IsOutPtTreatmentProgram //Lấy điều kiện theo cách điều trị để kiểm tra màn hình khám bệnh
                    && Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID != null
                    && !CheckLastOutpatientTreatmentTypeID())
                {
                    Globals.ShowMessage("ICD chính hiện tại đang không nằm trong Nhóm bệnh của hồ sơ này.", "Thông báo");
                    yield break;
                }
                if (CS_DS.DiagTreatment.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.OutPtTreatment //IsOutPtTreatmentProgram //Lấy điều kiện theo cách điều trị để kiểm tra màn hình khám bệnh
                    && Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID != null
                    && !CheckOutpatientTreatmentType_IsActive())
                {
                    Globals.ShowMessage("Mã ICD này đã tạm ngưng sử dụng. Vui lòng chọn Mã ICD khác hoặc liên hệ phòng Kế hoạch tổng hợp để được hỗ trợ!", "Thông báo");
                    yield break;
                }
                //▲==== #043
                //▲====: #031
        ...
        public void btSaveNewByOld()
        {
            ...
            DiagTrmtItem.ServiceRecID = null;
            DiagTrmtItem.PtRegDetailID = null;
            ...
            if (String.IsNullOrEmpty(DiagTrmtItem.ReasonHospitalStay) && refIDC10List.Where(x => x.IsMain == true && x.DiseasesReference.IsActive == true).FirstOrDefault() != null)
            {
                DiagTrmtItem.ReasonHospitalStay += "Điều trị bệnh " + refIDC10List.Where(x => x.IsMain == true && x.DiseasesReference.IsActive == true).FirstOrDefault().DiseasesReference.DiseaseNameVN;
            }
        ...
        public void MedicalFileInfoCmd()
        {
            ...
            GlobalsNAV.ShowDialog<IOutPtMedicalFileInfo>((aView) =>
            {
                aView.PtRegDetailID = Registration_DataStorage.CurrentPatientRegistrationDetail.PtRegDetailID;
                aView.OutPtTreatmentProgramID = (long)Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID;
                aView.V_RegistrationType = (long)Registration_DataStorage.CurrentPatientRegistration.V_RegistrationType;
                //▼====: #026
                aView.InPtRegistrationID = (long)Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID;
                //▲====: #026
                aView.OrientedTreatment = DiagTrmtItem.OrientedTreatment;
                //▼====: #030
                aView.Diagnosis = DiagTrmtItem.Diagnosis;
                aView.ReasonHospitalStay = DiagTrmtItem.ReasonHospitalStay;
                aView.MainDisease = GetDiseaseFromICDList(true);
                aView.IncludingDisease = GetDiseaseFromICDList(false);
                //▲====: #030
                //▼====: #037
                aView.IsChronic = Registration_DataStorage.CurrentPatientRegistration.IsChronic;
                aView.DiagTrmtItem = DiagTrmtItem;
                aView.PatientID = (long)Registration_DataStorage.CurrentPatientRegistration.PatientID;
                //▲====: #037
            });
            /*▲====: #012*/
        }
        //20220624 BLQ: Thêm hàm lấy chẩn đoán từ danh sách ICD
        private string GetDiseaseFromICDList(bool IsMain)
        {
            string Disease = "";
            ObservableCollection<DiagnosisIcd10Items> icd10Lists = refIDC10List.Where(x => x.DiagIcd10ItemID > 0 && (IsMain ? x.IsMain : !x.IsMain)).ToObservableCollection();
            if (icd10Lists != null && icd10Lists.Count > 0)
            {
                foreach (var item in icd10Lists)
                {
                    Disease += item.DiseasesReference.DiseaseNameVN + "; ";
                }
            }
            return string.IsNullOrEmpty(Disease) || Disease.Length < 2 ? Disease : Disease.Substring(0, Disease.Length - 2);
        }
        #endregion
        ...
        public void grdConsultation_LoadingRow(object sender, DataGridRowEventArgs e)
        {
            ...
            if (objRows != null)
            {
                ...
                //20200411 TBL: BM 0030109: Khi ICD10 ngưng sử dụng thì sẽ tô màu đỏ
        ...
        public void GetPaPerReferalFul(int V_TransferFormType, int PatientFindBy)
        {
            ...
            else
            {
                ...
                if (CurRegistration != null)
                {
                    ...
                    //2021-02-22 BaoLQ Do chức năng chặn lưu chẩn đoán khi chưa có giấy chuyển tuyến nên không lấy List ICD10 từ DiagTrmtItem được nên chuyển qua lấy từ refIDC10List
                    if (refIDC10List != null)
                    {
                        string tempICDList = String.Join(",", from item in refIDC10List
                                                              where (!string.IsNullOrEmpty(item.ICD10Code) && item.DiseasesReference != null)
                                                              select item.ICD10Code);
                        mEvent.Item.ICD10 = tempICDList;
                        mEvent.Item.ICD10Final = tempICDList;
                    }
                    if (DiagTrmtItem.DiagnosisFinal != null
                        //&& DiagTrmtItem.ICD10List != null
                        )
                    {
                    ...
                    //2021-02-17 Do chặn không cho lưu chẩn đoán khi chưa giấy chuyển tuyến nên hàm này sẽ lấy ngày khám của phiếu khám trước. Đổi lại lấy ngày hiện tại
                    ...
                    mEvent.Item.FromDate = Globals.GetCurServerDateTime();
                    //2021-02-17
                    mEvent.Item.ToDate = Globals.GetCurServerDateTime();
                    mEvent.Item.TransferDate = Globals.GetCurServerDateTime();
                }
            ...
            Action<IPaperReferalFull> onInitDlg = delegate (IPaperReferalFull TransferFromVm)
            {
                ...
                //▼==== #041
                TransferFromVm.IsHiReportOrDischarge = HiReportWarnVisible;
                //▲==== #041
        ...
        public void btGCT()
        {
            ...
            if (Registration_DataStorage != null && Registration_DataStorage.CurrentPatientRegistration != null)
            {
                ProcessCheckStatusPCLRequest(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID);
            }
            //▲====: #032
        }
        //private IEnumerator<IResult> CoroutineCheckTransferForm()
        //{
        //    var tempIsHaveTransferForm = CheckTransferForm();
        ...
        //▼====: #032
        public void ProcessCheckStatusPCLRequest(long PtRegistrationID)
        {
            this.ShowBusyIndicator(eHCMSResources.Z0840_G1_DangKTra);
            bool IsHavePCLRequestUnfinished = false;
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginCheckStatusPCLRequestBeforeCreateNew(PtRegistrationID, true, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                ...
                try
                {
                    using (var serviceFactory = new ePMRsServiceClient())
                    {
                        ...
                        Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                CurrentTransferForm = contract.EndGetTransferFormByPtRegistrationID(asyncResult);
                                ...
                                Globals.EventAggregator.Publish(new HaveTransferForm() { IsHaveTransferForm = IsHaveTransferForm });
                                GetPaPerReferalFul((int)AllLookupValues.V_TransferFormType.CHUYEN_Di, (int)AllLookupValues.PatientFindBy.NGOAITRU);
                            }
                            catch (Exception ex)
                            {
        ...
        public bool CheckValidDiagnosis()
        {
            ...
            if (ApplyCheckInPtRegistration && bCheckInPtRegistration)
            {
                MessageBox.Show("Có đăng ký nội trú !");
                return false;
            }
            if (Globals.curPhysicalExamination == null
                    || (Globals.curPhysicalExamination != null
                    && (Globals.curPhysicalExamination.Weight == null || Globals.curPhysicalExamination.Weight == 0
                           || Globals.curPhysicalExamination.Height == null || Globals.curPhysicalExamination.Height == 0)))
            {
                string ErrorMessages = string.Format("Bắt buộc nhập cân nặng/ chiều cao!", Globals.ServerConfigSection.CommonItems.AgeMustHasDHST / 365);
                void onInitDlg(IErrorBold confDlg)
                {
                    confDlg.ErrorTitle = eHCMSResources.K1576_G1_CBao;
                    confDlg.isCheckBox = false;
                    confDlg.SetMessage(ErrorMessages, "");
                    confDlg.FireOncloseEvent = true;
                }
                GlobalsNAV.ShowDialog<IErrorBold>(onInitDlg);
                return false;
            }
            if (Registration_DataStorage.CurrentPatient != null && ((TimeSpan)(Globals.GetCurServerDateTime() - Registration_DataStorage.CurrentPatient.DOB)).Days < Globals.ServerConfigSection.CommonItems.AgeMustHasDHST)
            {
                if (Globals.curPhysicalExamination == null
                    || (Globals.curPhysicalExamination != null
                           && (Globals.curPhysicalExamination.Weight == null || Globals.curPhysicalExamination.Weight == 0
                           || Globals.curPhysicalExamination.SpO2 == null || Globals.curPhysicalExamination.SpO2 == 0
                           || Globals.curPhysicalExamination.Temperature == null || Globals.curPhysicalExamination.Temperature == 0)))
                {
                    string ErrorMessages = string.Format("Trẻ em dưới {0} tuổi phải nhập cân nặng/ SpO2/ Nhiệt độ!", Globals.ServerConfigSection.CommonItems.AgeMustHasDHST / 365);
                    void onInitDlg(IErrorBold confDlg)
                    {
                        confDlg.ErrorTitle = eHCMSResources.K1576_G1_CBao;
                        confDlg.isCheckBox = false;
            ...
            else
            {
                if (Globals.curPhysicalExamination == null
                       || (Globals.curPhysicalExamination != null
                       && (Globals.curPhysicalExamination.Pulse == null || Globals.curPhysicalExamination.Pulse == 0
                              || Globals.curPhysicalExamination.SystolicPressure == null || Globals.curPhysicalExamination.SystolicPressure == 0
                              || Globals.curPhysicalExamination.DiastolicPressure == null || Globals.curPhysicalExamination.DiastolicPressure == 0)))
                {
                    string ErrorMessages = string.Format("Người bệnh trên {0} tuổi phải nhập huyết áp/ mạch!", Globals.ServerConfigSection.CommonItems.AgeMustHasDHST / 365);
                    void onInitDlg(IErrorBold confDlg)
                    {
                        confDlg.ErrorTitle = eHCMSResources.K1576_G1_CBao;
                        confDlg.isCheckBox = false;
            ...
            if (CheckedIsMain() && NeedICD10())
            {
                ...
                if (MainICD != null)
                {
                    if (!CheckRequireSubICd(MainICD))
                    {
                        return false;
                    }
                }
                // 20210611 Kiểm tra ICD chính là sao
                if (!CheckRuleICd())
                {
                    return false;
                }
                ...
                PatientRegistration CurRegistration = Registration_DataStorage.CurrentPatientRegistration;
                if (CurRegistration.PtInsuranceBenefit != null
                    && CurRegistration.PtInsuranceBenefit > 0 && refIDC10List != null
                    && refIDC10List.Any(x => x.ICD10Code != null
                    && x.DiseasesReference != null
                    && x.DiseasesReference.ICD10Code.Contains("Z10")))
                {
                    warningtask = new MessageWarningShowDialogTask(content, eHCMSResources.Z0339_G1_TiepTucLuuCDoan);
        ...
        public void ChangeStatesAfterUpdated(bool IsUpdate = false)
        {
            ...
            /*▲====: #004*/
            StateEdit();
            //20200408 TBL: BM 0029098: Nếu chẩn đoán cuối cùng là chẩn đoán của nội trú thì khi lưu thành công sẽ set lại thành của ngoại trú để ẩn cảnh báo chẩn đoán của nội trú
            if (DiagTrmtItem.V_RegistrationType == AllLookupValues.RegistrationType.NOI_TRU)
            {
                DiagTrmtItem.V_RegistrationType = AllLookupValues.RegistrationType.NGOAI_TRU;
                NotifyOfPropertyChange(() => IsInPtDiagnosis);
            }
        }
        ...
        public void btnAdmRequest()
        {
            ...
            if (Globals.curPhysicalExamination == null ||
                 ...
                 || Globals.curPhysicalExamination.SpO2 == null || Globals.curPhysicalExamination.SpO2 == 0))
            {
                MessageBox.Show("Bệnh nhân muốn nhập viện vui lòng nhập đủ DHST(chiều cao, cân nặng, mạch, nhiệt độ, huyết áp, SpO2). Mọi thắc mắc liên hệ KHTH", eHCMSResources.G0442_G1_TBao);
                return;
            }
            //▼====: #027
            if (!Globals.CheckDoctorContactPatientTime())
            ...
            //▲===== #015
            //Get danh sách icd kèm theo tiêu chuẩn nhập viện nếu chưa có
            GetListAdmissionCriterionAttachICD();
                            ...
                            Globals.DispatchCallback(asyncResult =>
                            {
                                try
                                {
                                    ...
                                    if (mOutRegistration != null && mOutRegistration.PtRegistrationID > 0)
                                    {
                                        ...
                                        {
                                            VM.InPtRegistrationID = Convert.ToInt64(Registration_DataStorage.CurrentPatientRegistration.InPtRegistrationID);
                                            VM.LoadAdmissionExamination(Convert.ToInt64(Registration_DataStorage.CurrentPatientRegistration.InPtRegistrationID), mRegistration.DiagnosisTreatment.OrientedTreatment, false);
                                        }, null, false, true, new Size(1200, 700));
                                        //▼==== #038
                                        //20230704 BaoLQ: Comment ra dời về màn hình nhập viện
                                        //GlobalsNAV.ShowDialog<ISelfDeclaration>((VM) =>
                                        //{
                                        //    VM.PtRegistrationID = Convert.ToInt64(Registration_DataStorage.CurrentPatientRegistration.InPtRegistrationID);
                                        //    VM.PatientID = Convert.ToInt64(Registration_DataStorage.CurrentPatientRegistration.PatientID);
        ...
        public void StateNew()
        {
            ...
            FormEditorIsEnabled = false;
            //20191024 TBL: Task #1140: Fix lỗi khi 1 BN có 2 đăng ký, đăng ký tạo chẩn đoán trước có cách điều trị là Nhập viện thì load đăng ký sau lên chưa có chẩn đoán nhưng vẫn hiện nút Đề nghị nhập viện
            btEditIsEnabled = false;
        ...
        public void StateEdit()
        {
            ...
            NotifyOfPropertyChange(() => FormEditorIsEnabled);
            //▼===== #029
            NotifyOfPropertyChange(() => IsEnableGiayCNTT);
            //▲===== #029
        }
        ...
        #endregion
        public void btnCreateServiceReq()
        {
            //▼===== 20191105 TTM:  #1244 Bổ sung thêm điều kiện Registration_DataStorage.PatientServiceRecordCollection.Count == 0
            //                      Vì nều PatientServiceRecordCollection.Count == 0 tức là đợt đăng ký này bệnh nhân chưa được chẩn đoán.
            if (DiagTrmtItem == null || DiagTrmtItem.DTItemID == 0 || Registration_DataStorage.PatientServiceRecordCollection.Count == 0)
            {
                Globals.ShowMessage(eHCMSResources.A0405_G1_Msg_InfoChuaCoCD, eHCMSResources.G0442_G1_TBao);
                return;
            }
            //▲===== 
            //▼====: #027
            ...
            var regVm = Globals.GetViewModel<IPatientRegistration_V2>();
            regVm.Registration_DataStorage = Registration_DataStorage;
            regVm.Diagnosis = DiagTrmtItem.DiagnosisFinal;
            regVm.CurRegistration = Registration_DataStorage.CurrentPatientRegistration;
            regVm.CurrentPatient = Registration_DataStorage.CurrentPatient;
            regVm.IsFromRequestDoctor = true;
            //▼====: #040
            regVm.ICD10List = DiagTrmtItem.ICD10List != null ? DiagTrmtItem.ICD10List.Trim().Replace(",", "; ") : "";
            //▲====: #040
            if (Globals.ServerConfigSection.ConsultationElements.CheckedTreatmentRegimen)
            {
                regVm.GetRefTreatmentRegimensAndDetail(Registration_DataStorage.CurrentPatientRegistrationDetail);
            }
            regVm.ApplyViewCase(RegistrationViewCase.RegistrationRequestView, DiagTrmtItem);
            GlobalsNAV.ShowDialog_V3(regVm, (aView) =>
            {
                //20191018 TBL: Không cần bắn sự kiện vì khi bắn sẽ nếu BN có 2 đăng ký thì sẽ hiện popup tìm đăng ký
                //Globals.EventAggregator.Publish(new ItemSelected<Patient> { Item = Registration_DataStorage.CurrentPatient });
                regVm.HandlePatientFromConsultation(Registration_DataStorage.CurrentPatient);
            });
        }
        private bool _IsShowButton = Globals.ServerConfigSection.OutRegisElements.IsPerformingTMVFunctionsA;
        public bool IsShowButton
        {
            get { return _IsShowButton; }
            set
            {
                if (_IsShowButton != value)
                {
                    _IsShowButton = value;
                    NotifyOfPropertyChange(() => IsShowButton);
                }
            }
        }
        public void btnCreatePackageServiceReq()
        {
            if (DiagTrmtItem == null || DiagTrmtItem.DTItemID == 0 || Registration_DataStorage.PatientServiceRecordCollection.Count == 0)
            {
                Globals.ShowMessage(eHCMSResources.A0405_G1_Msg_InfoChuaCoCD, eHCMSResources.G0442_G1_TBao);
        ...
        //▼===== #016
        private void UpdateDateStarted_ForPatientRegistrationDetails(long PtRegDetailID)
        {
            this.ShowBusyIndicator();
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginUpdateDateStarted_ForPatientRegistrationDetails(PtRegDetailID, Globals.DispatchCallback(asyncResult =>
                            {
                                try
                                {
                                    contract.EndUpdateDateStarted_ForPatientRegistrationDetails(out DateTime? DateStart, asyncResult);
                                    if (DateStart != null)
                                    {
        ...
        private bool IsProgramChangeInTreatmentChecked = false;
        ...
        {
            if (IsProgramChangeInTreatmentChecked)
            {
                IsProgramChangeInTreatmentChecked = false;
                e.Handled = true;
            ...
            if (Registration_DataStorage == null || Registration_DataStorage.CurrentPatientRegistration == null ||
                Registration_DataStorage.CurrentPatient == null || Registration_DataStorage.CurrentPatient.PatientID == 0)
            {
                e.Handled = true;
                return;
            }
            var IsInTreatmentProgram = (sender as CheckBox).IsChecked.GetValueOrDefault(false);
            if (IsInTreatmentProgram)
            {
                GlobalsNAV.ShowDialog<IOutPtTreatmentProgram>((DialogView) =>
                {
                    DialogView.CurrentRegistration = Registration_DataStorage.CurrentPatientRegistration;
                });
                NotifyViewDataChanged();
            }
            else if (Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID.GetValueOrDefault(0) > 0)
            {
                if (MessageBox.Show(eHCMSResources.Z2951_G1_Msg, eHCMSResources.G0442_G1_TBao, MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                {
                    this.ShowBusyIndicator();
                    var CurrentThread = new Thread(() =>
                    {
                        using (var CurrentFactory = new CommonUtilsServiceClient())
                        {
                            ...
                            CurrentContract.BeginUpdateTreatmentProgramIntoRegistration(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID, Registration_DataStorage.CurrentPatientRegistrationDetail.PtRegDetailID, null, Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    CurrentContract.EndUpdateTreatmentProgramIntoRegistration(out int OutPrescriptionsAmount, asyncResult);
                                    Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID = null;
                                    Registration_DataStorage.CurrentPatientRegistration.PrescriptionsAmount = OutPrescriptionsAmount;
                                }
                                catch (Exception ex)
                                {
                ...
                {
                    IsProgramChangeInTreatmentChecked = true;
                    e.Handled = true;
                    NotifyViewDataChanged();
                    return;
                }
        ...
        public void TreatmentProgramListButton()
        {
            //▼==== #033
            //GlobalsNAV.ShowDialog<IOutPtTreatmentProgram>((DialogView) =>
            //{
            //    DialogView.CurrentRegistration = Registration_DataStorage.CurrentPatientRegistration;
            //});
            IOutPtTreatmentProgram DialogView = Globals.GetViewModel<IOutPtTreatmentProgram>();
            DialogView.CurrentRegistration = Registration_DataStorage.CurrentPatientRegistration;
            //▼==== #034
            ...
            {
                DialogView.PtRegDetailID = Registration_DataStorage.CurrentPatientRegistrationDetail.PtRegDetailID;
            }
            //▲==== #034
            GlobalsNAV.ShowDialog_V5(DialogView, null, null, new Dictionary<string, object> { ["Title"] = eHCMSResources.Z3270_G1_DSHoSoDTNT });
            //▲==== #033
            NotifyViewDataChanged();
        }
        ...
        private bool _IsOutPtTreatmentProgram;
        ...
        public bool IsHasOldRegistration
        {
            get
            {
                return Registration_DataStorage != null
                    ...
                    //▼====: #031
                    && (FormEditorIsEnabled || Registration_DataStorage.CurrentPatientRegistration.HIReportID > 0)
                    && DiagTrmtItem != null
                    && DiagTrmtItem.V_TreatmentType == (long)AllLookupValues.V_TreatmentType.OutPtTreatment;
                //▲====: #031
            }
        }
        public void Handle(SelectedDiagnosisTreatmentForConfirmSmallProceduce message)
        {
            if (message != null && message.Icd10Items != null && message.DiagnosisTreatment != null)
            {
                DiagTrmtItem = message.DiagnosisTreatment;
                refIDC10List = message.Icd10Items.ToObservableCollection();
            }
        }
        //▼===== #018
        ...
        //▲===== #018
        //▼===== #019
        public void UpdateDiseaseCmd()
        {
            IUpdateDiseaseProgressionView aView = Globals.GetViewModel<IUpdateDiseaseProgressionView>();
            aView.DiagTrmtItem = DiagTrmtItem.DeepCopy();
            aView.DiagTrmtItem.DoctorComments = "";
            aView.DiagTrmtItem.OrientedTreatment = "";
            GlobalsNAV.ShowDialog_V3<IUpdateDiseaseProgressionView>(aView, null, null, false, false);
            if (aView.UpdateOK) //Nếu cập nhật thành công thì load lại chẩn đoán
            {
                GetDiagnosisTreatmentByPtID();
            }
        }

        private void GetDiagnosisTreatmentByPtID()
        {
            this.ShowBusyIndicator();
            var t = new Thread(() =>
            {
                ...
                {
                    var contract = serviceFactory.ServiceInstance;
                    contract.BeginGetDiagnosisTreatmentByPtID_V2(null, null, null, 1, false, 24001, null, Registration_DataStorage.CurrentPatientRegistrationDetail.PtRegDetailID, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            ...
                            {
                                DiagTrmtItem = results.ToObservableCollection()[0];
                            }
                            else
                            {
        ...
        private bool _HaveICDInAdmissionCriterion = false;
        public bool HaveICDInAdmissionCriterion
        {
            get
            {
                return _HaveICDInAdmissionCriterion;
            }
            set
            {
                if (_HaveICDInAdmissionCriterion != value)
                {
                    _HaveICDInAdmissionCriterion = value;
                    NotifyOfPropertyChange(() => HaveICDInAdmissionCriterion);
                }
            }
        }
        private void CheckAdmissionCriterion()
        {
            if (Globals.allAdmissionCriterionAttachICD == null || Globals.allAdmissionCriterionAttachICD.Where(x => x.ICD10Code == refIDC10List.Where(y => y.IsMain).FirstOrDefault().ICD10Code).Count() == 0)
            {
                GlobalsNAV.ShowDialog<IConsultationOld_InPt>((ConsultationVM) =>
                {
                    ...
                    //▼===== 20191011: Cờ để xác định màn hình chẩn đoán hiện tại đang được mở từ đề nghị nhập viện.
                    ...
                    ConsultationVM.gSelectedDoctorStaff.StaffID = DiagTrmtItem.DoctorStaffID;
                    if (DiagTrmtItem.DTItemID > 0 && ConsultationVM.InPtRegistrationID.GetValueOrDefault(0) == 0)
                    {
                        ...
                        AdmissionDiagTrmtItem.OrientedTreatment = "";// 2022-07-16 BLQ: Issue 1889 - Set về rỗng khi đề nghị nhập viện để cho người dùng tự nhập 
                ...
                }, null, false, true, new Size(1200, 900));
                //RegisterNewInPtRegistration();
            }
            else
            {
                GlobalsNAV.ShowDialog<IAdmissionCriterion_Symptom>((VM) =>
                {
                    VM.AdmissionCriterionID = Globals.allAdmissionCriterionAttachICD.Where(x => x.ICD10Code == refIDC10List.Where(y => y.IsMain).FirstOrDefault().ICD10Code).FirstOrDefault().AdmissionCriterionID;
                    VM.PtRegistrationID = Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID;
                    VM.PatientCode = Registration_DataStorage.CurrentPatient.PatientCode;
                    VM.InitInfo();
                }, null, false, true, new Size(1200, 500));
            }
        }
        public void GetListAdmissionCriterionAttachICD()
        {
            if (Globals.allAdmissionCriterionAttachICD != null)
            {
                CheckAdmissionCriterion();
                return;
            }
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetAllAdmissionCriterionAttachICD(Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var Result = contract.EndGetAllAdmissionCriterionAttachICD(asyncResult);
                                if (Result != null)
                                {
                                    Globals.allAdmissionCriterionAttachICD = Result;
                                    CheckAdmissionCriterion();
                                }
        ...
        public void Handle(AdmissionCriterion_Symptom_Select_Event message)
        {
            if (message != null && message.Result == true)
            {
                GlobalsNAV.ShowDialog<IConsultationOld_InPt>((ConsultationVM) =>
                {
                    ...
                    if (DiagTrmtItem.DTItemID > 0 && ConsultationVM.InPtRegistrationID.GetValueOrDefault(0) == 0)
                    {
                        var AdmissionDiagTrmtItem = DiagTrmtItem.DeepCopy();
                        //▼====: #036
                        AdmissionDiagTrmtItem.Diagnosis = message.Diagnosis;
                        //▲====: #036
                        AdmissionDiagTrmtItem.DTItemID = 0;
                        AdmissionDiagTrmtItem.OrientedTreatment = "";// 2022-07-16 BLQ: Issue 1889 - Set về rỗng khi đề nghị nhập viện để cho người dùng tự nhập 
                        AdmissionDiagTrmtItem.PatientServiceRecord.V_Behaving = (long)AllLookupValues.Behaving.YEU_CAU_NHAP_VIEN;
        ...
        //▼===== #029
        private bool _IsEnableGiayCNTT = true;
        public bool IsEnableGiayCNTT
        {
            get
            {
                return _IsEnableGiayCNTT;
            }
            set
            {
                if (_IsEnableGiayCNTT != value)
                {
                    _IsEnableGiayCNTT = value;
                    NotifyOfPropertyChange(() => IsEnableGiayCNTT);
                }
            }
        }

        private bool _WarniningSapNhapVisible = false;
        public bool WarniningSapNhapVisible
        {
            get
            {
                return _WarniningSapNhapVisible;
            }
            set
            {
                if (_WarniningSapNhapVisible != value)
                {
                    _WarniningSapNhapVisible = value;
                    NotifyOfPropertyChange(() => WarniningSapNhapVisible);
                }
            }
        }
        //▲===== #029
        //▼=====: #031
        private string _LastIDC10MainTreatmentProgram;
        public string LastIDC10MainTreatmentProgram
        {
            get
            {
                return _LastIDC10MainTreatmentProgram;
            }
            set
            {
                if (_LastIDC10MainTreatmentProgram != value)
                {
                    _LastIDC10MainTreatmentProgram = value;
                    NotifyOfPropertyChange(() => LastIDC10MainTreatmentProgram);
                }
            }
        }
        //▼===== #043
        private long _LastOutpatientTreatmentTypeID;
        public long LastOutpatientTreatmentTypeID
        {
            get
            {
                return _LastOutpatientTreatmentTypeID;
            }
            set
            {
                if (_LastOutpatientTreatmentTypeID != value)
                {
                    _LastOutpatientTreatmentTypeID = value;
                    NotifyOfPropertyChange(() => LastOutpatientTreatmentTypeID);
                }
            }
        }

        private ObservableCollection<OutpatientTreatmentTypeICD10Link> _ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID;
        public ObservableCollection<OutpatientTreatmentTypeICD10Link> ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID
        {
            get { return _ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID; }
            set
            {
                _ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID = value;
                NotifyOfPropertyChange(() => ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID);
            }
        }

        private void GetLastIDC10MainTreatmentProgram_ByPtRegID(long PtRegistrationID, long PtRegDetailID)
        {
            //this.ShowBusyIndicator();
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetLastIDC10MainTreatmentProgram_ByPtRegID(PtRegistrationID, PtRegDetailID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                long tempOTTypeID = 0;
                                bool result = contract.EndGetLastIDC10MainTreatmentProgram_ByPtRegID(out tempOTTypeID, asyncResult);
                                LastOutpatientTreatmentTypeID = tempOTTypeID;
                                OutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID(LastOutpatientTreatmentTypeID);
                            }
                            catch (Exception ex)
                            {
                                Globals.ShowMessage(ex.Message, eHCMSResources.T0432_G1_Error);
        ...
        public bool CheckLastOutpatientTreatmentTypeID()
        {
            if (refIDC10List == null
                || refIDC10List.Count == 0
                || LastOutpatientTreatmentTypeID == 0
                || Registration_DataStorage.CurrentPatientRegistration.OutpatientTreatmentTypeID == null)
            {
                return true;
            }
            string currentICD10Main = refIDC10List.Where(x => x.IsMain).FirstOrDefault().ICD10Code;
            if (ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID.Where(x => x.ICD10 == currentICD10Main).Count() == 0)
            {
                return false;
            }
            return true;
        ...
        public bool CheckOutpatientTreatmentType_IsActive()
        {
            if (refIDC10List == null || refIDC10List.Count == 0)
            {
                return true;
            }
            string currentICD10Main = refIDC10List.Where(x => x.IsMain).FirstOrDefault().ICD10Code;
            if (ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID.Where(x => x.ICD10 == currentICD10Main && x.IsActive).Count() == 0)
            {
                return false;
            }
            return true;
        ...
        public void OutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID(long OutpatientTreatmentTypeID)
        {
            var t = new Thread(() =>
            {
                try
                {
                    using (var serviceFactory = new ConfigurationManagerServiceClient())
                    {
                        ...
                        client.BeginOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID(OutpatientTreatmentTypeID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                ObjOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID = client.EndOutpatientTreatmentTypeICD10Link_ByOutpatientTreatmentTypeID(asyncResult).ToObservableCollection();
                            }
                            catch (FaultException<AxException> fault)
                            {
        ...
        //public bool CheckLastIDC10MainTreatmentProgram(out string LastIDC10Main)
        //{
        //    LastIDC10Main = "";
        //    if (refIDC10List == null
        //        || refIDC10List.Count == 0
        //        || string.IsNullOrEmpty(LastIDC10MainTreatmentProgram)
        //        || Registration_DataStorage.CurrentPatientRegistration.OutpatientTreatmentTypeID == null)
        //    {
        //        return true;
        //    }
        //    string currentICD10Main = refIDC10List.Where(x => x.IsMain).FirstOrDefault().ICD10Code;
        //    if (currentICD10Main != LastIDC10MainTreatmentProgram)
        //    {
        //        LastIDC10Main = LastIDC10MainTreatmentProgram;
        //        return false;
        //    }
        //    return true;
        //}
        //▲===== #043
        private List<OutpatientTreatmentType> _ListOutpatientTreatmentType;
        public List<OutpatientTreatmentType> ListOutpatientTreatmentType
        {
            get
            {
                return _ListOutpatientTreatmentType;
            }
            set
            {
                if (_ListOutpatientTreatmentType != value)
                {
                    _ListOutpatientTreatmentType = value;
                    NotifyOfPropertyChange(() => ListOutpatientTreatmentType);
                }
            }
        }
        private void GetAllOutpatientTreatmentType()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetAllOutpatientTreatmentType(Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                List<OutpatientTreatmentType> result = contract.EndGetAllOutpatientTreatmentType(asyncResult);
                                if (result != null && result.Count > 0)
                                {
                                    ListOutpatientTreatmentType = new List<OutpatientTreatmentType>();
                                    foreach (var item in result)
                                    {
                                        ListOutpatientTreatmentType.Add(item);
                                    }
                                }
                            }
                            catch (Exception ex)
        ...
        public bool CheckIDC10MainInOutpatientTreatmentType()
        {
            if (refIDC10List == null
                || refIDC10List.Count == 0
                || ListOutpatientTreatmentType == null
                || ListOutpatientTreatmentType.Count == 0
                || Registration_DataStorage.CurrentPatientRegistration.OutpatientTreatmentTypeID == null)
            {
                return true;
            }
            DiagnosisIcd10Items currentICD10Main = refIDC10List.Where(x => x.IsMain).FirstOrDefault();
            OutpatientTreatmentType CurrentOutpatientTreatment = ListOutpatientTreatmentType.Where(x => x.OutpatientTreatmentTypeID == Registration_DataStorage.CurrentPatientRegistration.OutpatientTreatmentTypeID).FirstOrDefault();
            if (CurrentOutpatientTreatment.ListICD10Code != null && CurrentOutpatientTreatment.ListICD10Code.Count > 0)
            {
                foreach (var item in CurrentOutpatientTreatment.ListICD10Code)
                {
                    if (item.IDCode == currentICD10Main.DiseasesReference.IDCode)
                    {
                        return true;
                    }
                }
        ...
        public void Handle(ConfirmOutPtTreatmentProgram message)
        {
            if (message != null && message.Result == true)
            {
                GetLastIDC10MainTreatmentProgram_ByPtRegID(Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID, Registration_DataStorage.CurrentPatientRegistrationDetail.PtRegDetailID);
                //▼=====: #037
                Registration_DataStorage.CurrentPatientRegistration.IsChronic = message.IsChronic;
                //▲=====: #037
            }
        }
        //▲=====: #031
        //▼=====: #035
        public void Handle(DeleteOutPtTreatmentProgramItem message)
        {
            if (message != null && message.Result == true)
            {
                Registration_DataStorage.CurrentPatientRegistration.OutPtTreatmentProgramID = null;
        ...
        public void Handle(PCLExamAccordingICD_Event message)
        {
            ...
            {
                DiagTrmtItem.V_TreatmentType = (long)AllLookupValues.V_TreatmentType.PCLExam;
                var regVm = Globals.GetViewModel<IPatientRegistration_V2>();
                regVm.Registration_DataStorage = Registration_DataStorage;
                regVm.Diagnosis = DiagTrmtItem.DiagnosisFinal;
                regVm.CurRegistration = Registration_DataStorage.CurrentPatientRegistration;
                regVm.CurrentPatient = Registration_DataStorage.CurrentPatient;
                regVm.IsFromPCLExamAccording = true;
                regVm.pCLExamTypes = message.ListPCLExamAccordingICD.ToObservableCollection();
                if (Globals.ServerConfigSection.ConsultationElements.CheckedTreatmentRegimen)
                {
                    regVm.GetRefTreatmentRegimensAndDetail(Registration_DataStorage.CurrentPatientRegistrationDetail);
                }
        ...
        private bool _IsEnableGiayNDT = false;
        public bool IsEnableGiayNDT
        {
            get
            {
                return _IsEnableGiayNDT;
            }
            set
            {
                if (_IsEnableGiayNDT != value)
                {
                    _IsEnableGiayNDT = value;
                    NotifyOfPropertyChange(() => IsEnableGiayNDT);
                }
            }
        }
        ...
        public void btGNDT()
        {
            if (Registration_DataStorage.CurrentPatientRegistration == null || Registration_DataStorage.CurrentPatientRegistration.Patient == null)
            {
                MessageBox.Show("Chưa chọn bệnh nhân", "Thông báo");
                return;
            }
           
            if (Registration_DataStorage.CurrentPatientRegistration.Patient.Gender != "F")
            {
                MessageBox.Show("Bệnh nhân nam không thể tạo phiếu nghĩ dưỡng thai", "Thông báo");
                return;
            }
            if (DiagTrmtItem == null || DiagTrmtItem.DTItemID == 0 || !mEditConsultationState)
            {
                MessageBox.Show("Bệnh nhân chưa có chẩn đoán!", "Thông báo");
                return;
            }
            Action<IVacationInsuranceCertificate> onInitDlg = delegate (IVacationInsuranceCertificate vacationInsuranceCertificate)
            {
                ...
                vacationInsuranceCertificate.IsOpenFromConsult = true;
                vacationInsuranceCertificate.IsPrenatal = true;
                vacationInsuranceCertificate.TitleForm = eHCMSResources.Z3115_G1_ChungNhanNDT;
                vacationInsuranceCertificate.PtRegistration = Registration_DataStorage.CurrentPatientRegistration;
                vacationInsuranceCertificate.PtRegistration.DiagnosisTreatment = DiagTrmtItem;
        ...
        public void btGNVHBH()
        {
            ...
            Action<IVacationInsuranceCertificate> onInitDlg = delegate (IVacationInsuranceCertificate vacationInsuranceCertificate)
            {
                ...
                vacationInsuranceCertificate.PtRegistration = Registration_DataStorage.CurrentPatientRegistration;
                vacationInsuranceCertificate.PtRegistration.DiagnosisTreatment = DiagTrmtItem;
                vacationInsuranceCertificate.PtRegistration.DiagnosisTreatment.DiagnosisFinal =
                    string.Join("; ", from item in refIDC10List
                                      where (!string.IsNullOrEmpty(item.ICD10Code) && item.DiseasesReference != null && item.IsMain && item.DiseasesReference.IsLongTermIllness)
                                      select item.DiseasesReference.DiseaseNameVN + "(" + item.ICD10Code + ")")
                    + string.Join("; ", from item in refIDC10List
                                        where (!string.IsNullOrEmpty(item.ICD10Code) && item.DiseasesReference != null && item.IsMain && !item.DiseasesReference.IsLongTermIllness)
                                        select item.DiseasesReference.DiseaseNameVN)
                    + "; " + string.Join("; ", from item in refIDC10List
                                        where (!string.IsNullOrEmpty(item.ICD10Code) && item.DiseasesReference != null && !item.IsMain)
                                        select item.DiseasesReference.DiseaseNameVN);
