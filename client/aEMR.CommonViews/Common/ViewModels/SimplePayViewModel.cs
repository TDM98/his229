 ...
 * 20230712 #019 TNHX: 3323 Chỉnh lại cách gọi API khi gửi dữ liệu qua PAC service gateway
 */
                    ...
                    if (aPatientPCLRequestList.Count > 0)
                    {
                        foreach (PatientPCLRequest temp in aPatientPCLRequestList)
                        {
                            ...
                            if (!string.IsNullOrEmpty(Globals.ServerConfigSection.CommonItems.PACLocalServiceGatewayUrl))
                            {
                                ...
                                if (item != null && item.patientPCLReqID != null)
                                {
                                    try
                                    {
        ...
        private IEnumerator<IResult> DoPayForRegisNew_CombiningTwoStoreProcs()
        {
            ...
            if (exError == null)
            {
                ...
                if (!IsConfirmHIView)
                {
                    ...
                    {
                        Globals.EventAggregator.Publish(new PhieuChiDinhForRegistrationCompleted()
                        {
                            Registration = savedRegisReturn,
                            RegDetailsList = Globals.ServerConfigSection.CommonItems.IsEnablePrintReceiptAndRequest ? paidRegDetailsList : new List<PatientRegistrationDetail>(),
                            PCLRequestList = paidPclRequestList ?? new List<PatientPCLRequest>()
                        });
                    }
