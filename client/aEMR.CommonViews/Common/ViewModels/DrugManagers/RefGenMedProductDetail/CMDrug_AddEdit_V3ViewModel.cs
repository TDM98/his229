/*
 * 12/03/2020 TTM: Bổ sung trường IsNotVAT (Không thuế) vào màn hình.
 */
using System;
...
* 20181002 #006 TBL:    BM 0000067. Rang buoc truong duong dung.
* 20181006 #007 TTM:    Thay đổi cách lấy giá trị cột từ e.Column.GetValue(FrameworkElement.NameProperty).ToString()
*                       => e.Column.Equals("Tên Grid".GetColumnByName("Tên cột") vì cách cũ bị sai => Lấy giá trị cột không đc.
* 20181101 #008 TBL:    BM 0003228. Thay doi truong Ten hoat chat tu TextBox thanh AutoComplete
* 20190916 #009 TBL:    BM 0014339: Khi tạo mới dựa trên cũ xóa những thông tin của thầu 
* 20191006 #010 TNHX: [Bitrix 896] Insert/ Update ConIndicatorDrugsRelToMedCondXML where Insert new Drug + refactor code
* 20191007 #011 TNHX: [Bitrix 896] Check valid Drug Info
* 20191012 #012 TBL:    Trường hợp khi cập nhật danh mục mà xóa giá trị của DispenseVolume và Dung tích thì set về = 0 để khi lưu có thể kiểm tra được
* 20230322 #013 QTD:    Thêm dữ liệu 130
*/
namespace aEMR.Common.ViewModels
{
    [Export(typeof(ICMDrug_AddEdit_V3)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class CMDrug_AddEdit_V3ViewModel : ViewModelBase, ICMDrug_AddEdit_V3
        , IHandle<DrugDeptCloseSearchSupplierEvent>
        , IHandle<MedDeptContraEvent>
        , IHandle<V_RouteOfAdministrationEvent>
    {
        /*▼====: #003*/
        ...
        public void authorization()
        {
            ...
            mSuaThuoc_KhoaDuoc = Globals.CheckOperation(Globals.listRefModule, (int)eModules.mKhoaDuoc,
                                                         (int)eKhoaDuoc.mSuaThuoc,(int)oKhoaDuocEx.mSuaThuoc_KhoaDuoc);
            mSuaThuoc_DuocLamSang = Globals.CheckOperation(Globals.listRefModule, (int)eModules.mKhoaDuoc,
                                                         (int)eKhoaDuoc.mSuaThuoc,(int)oKhoaDuocEx.mSuaThuoc_DuocLamSang);
        }
        private bool _mSuaThuoc_KhoaDuoc = true;
        private bool _mSuaThuoc_DuocLamSang = true;
        public bool mSuaThuoc_KhoaDuoc
        {
            get
            {
                return _mSuaThuoc_KhoaDuoc;
            }
            set
            {
                if (_mSuaThuoc_KhoaDuoc
                 == value)
                    return;
                _mSuaThuoc_KhoaDuoc
                 = value;
                NotifyOfPropertyChange(() => mSuaThuoc_KhoaDuoc
                );
            }
        }  
        public bool mSuaThuoc_DuocLamSang
        {
            get
            {
                return _mSuaThuoc_DuocLamSang;
            }
            set
            {
                if (_mSuaThuoc_DuocLamSang
                 == value)
                    return;
                _mSuaThuoc_DuocLamSang
                 = value;
                NotifyOfPropertyChange(() => mSuaThuoc_DuocLamSang
                );
            }
        }
        public void btAddNewFromOldValue()
        {
            ...
            ObjRefGenMedProductDetails_Current.RefGenMedDrugDetails.TCKTAndTCCNGroup = "";
            ObjRefGenMedProductDetails_Current.RefGenMedDrugDetails.V_GroupTypeForReport20 = 0;
            ObjRefGenMedProductDetails_Current.TLThanhToan = 0;
            ObjRefGenMedProductDetails_Current.MaxHIPay = 0;
            ObjRefGenMedProductDetails_Current.HITTypeID = 0;
            ObjRefGenMedProductDetails_Current.BiddingHospital = "";
            ObjRefGenMedProductDetails_Current.BidDecisionNumAndOrdinalNum = "";
            ObjRefGenMedProductDetails_Current.BidEffectiveDate = "";
        ...
        private ObservableCollection<Lookup> _V_CatDrugTypes;
        public ObservableCollection<Lookup> V_CatDrugTypes
        {
            get { return _V_CatDrugTypes; }
            set
            {
                if (_V_CatDrugTypes != value)
                    _V_CatDrugTypes = value;
                NotifyOfPropertyChange(() => V_CatDrugTypes);
            }
        }
        ...
        public CMDrug_AddEdit_V3ViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg, ISalePosCaching salePosCaching)
        {
            ...
            InstructionOrdinalTypeCollection = Globals.AllLookupValueList.Where(x => x.ObjectTypeID == (long)LookupValues.V_InstructionOrdinalType).ToObservableCollection();
            #endregion
            ...
            //20180802 TTM: Chỉ lấy CatDrugType thuộc loại dùng chung và của khoa dược.
            V_CatDrugTypes = Globals.AllLookupValueList.Where(x => (x.ObjectTypeID == (long)LookupValues.V_CatDrugType && x.LookupID != 82203)).ToObservableCollection();
            ...
            allV_RouteOfAdministration_Edit = new ObservableCollection<RouteOfAdministrationContactDrug>();
            V_RouteOfAdministration_Edit = new EntitiesEdit<Lookup>();
            ...
            /*▲====: #004*/
            //▼====: #013
            DrugFormulationMethodCollection = Globals.AllLookupValueList.Where(x => x.ObjectTypeID == (long)LookupValues.V_DrugFormulationMethod).ToObservableCollection();
            V_PaymentSourceCollection = Globals.AllLookupValueList.Where(x => x.ObjectTypeID == (long)LookupValues.V_PaymentSource).ToObservableCollection();
            //▲====: #013
            authorization();
        }
        ...
        private IEnumerator<IResult> GetAllReferenceValues()
        {
            ...
            GetAllRefGeneric();
            /*▲====: #008*/
            GetAllRefGenMedProductDetail(V_MedProductType);
        }
        ...
        //20180802 TTM: Lấy giá trị combobox ra để tạo cảnh báo
        ComboBox cbb_Type;
        public void cbb_Type_Loaded(object sender, RoutedEventArgs e)
        {
            cbb_Type = sender as ComboBox;
        }
        ...
        private ObservableCollection<RefGenMedProductDetails> _RefGenMedProductDetaillst;
        public ObservableCollection<RefGenMedProductDetails> RefGenMedProductDetaillst
        {
            get
            {
        ...
        private ObservableCollection<RefGenMedProductDetails> _RefGenMedProductDetail;
        public ObservableCollection<RefGenMedProductDetails> RefGenMedProductDetaill
        {
            get
            {
            ...
            set
            {
                if (_RefGenMedProductDetail != value)
                {
                    ...
                    NotifyOfPropertyChange(() => RefGenMedProductDetaill);
                }
            }
        }
        private ObservableCollection<Lookup> _InstructionOrdinalTypeCollection;
        public ObservableCollection<Lookup> InstructionOrdinalTypeCollection
        {
            get
            {
                return _InstructionOrdinalTypeCollection;
            }
            set
            {
                if (_InstructionOrdinalTypeCollection == value)
                {
                    return;
                }
                _InstructionOrdinalTypeCollection = value;
                NotifyOfPropertyChange(() => InstructionOrdinalTypeCollection);
            }
        }
        #endregion
        ...
        private bool CheckData()
        {
            ...
            else
            {
                ...
                if (ObjRefGenMedProductDetails_Current.InsuranceCover == true)
                {
                    ...
                    //▼====: #011 Không quan trọng tạm thời bỏ kiểm tra
        ...
        public ObservableCollection<ContraIndicatorMedProductsRelToMedCond> createListNew()
        {
            ...
            foreach (var item in RefMedicalConditionType_Edit.NewObject)
            {
                ...
                p.IsWarning = item.IsWarning;
                lstNew.Add(p);
            }
            return lstNew;
        }
        public ObservableCollection<RouteOfAdministrationContactDrug> createListROADelete()
        {
            var lstNew = new ObservableCollection<RouteOfAdministrationContactDrug>();
            foreach (var item in V_RouteOfAdministration_Edit.DeleteObject)
            {
                foreach (var contra in allV_RouteOfAdministration_Edit)
                {
                    if (item.LookupID == contra.RouteOfAdministration.LookupID)
                    {
                        lstNew.Add(contra);
                    }
                }
        ...
        public ObservableCollection<RouteOfAdministrationContactDrug> createListROANew()
        {
            var lstNew = new ObservableCollection<RouteOfAdministrationContactDrug>();
            foreach (var item in V_RouteOfAdministration_Edit.NewObject)
            {
                RouteOfAdministrationContactDrug p = new RouteOfAdministrationContactDrug();
                //p.RefGenMedProductDetails = ObjRefGenMedProductDetails_Current;
                p.GenMedProductID = ObjRefGenMedProductDetails_Current.GenMedProductID;
                p.RouteOfAdministration = item;
                lstNew.Add(p);
            }
            return lstNew;
        }
        ...
        public void btSave()
        {
            if (CheckValid(ObjRefGenMedProductDetails_Current))
            {
                ...
                //20191009 TBL: Vì sai logic nên cho dù chọn nhóm là DT và DĐ cũng set Volume = 0
                ...
                {
                    IsWaitingSave = true;
                    RefGenMedProductDetails_Save_New(ObjRefGenMedProductDetails_Current);
                }
            }
        }
                    ...
                    using (var serviceFactory = new PharmacyGenMedDeptDetailsServiceClient())
                    {
                        ...
                        , lstUpdate, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                ...
                                if (results)
                                {
                                    //20190328 TBL: Comment cau thong bao lai vi khi luu thanh cong client hien thi 2 popup thong bao
        ...
        private void InsertDeleteUpdateRouteOfAdministrationContactDrugXML(IList<RouteOfAdministrationContactDrug> lstInsert
                                                  ...
                                                  , IList<RouteOfAdministrationContactDrug> lstUpdate)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginInsertDeleteUpdateRouteOfAdministrationContactDrugXML(lstInsert, lstDelete
                        , lstUpdate, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                bool results = contract.EndInsertDeleteUpdateRouteOfAdministrationContactDrugXML(asyncResult);
                                if (results)
                                {
                                    //20190328 TBL: Comment cau thong bao lai vi khi luu thanh cong client hien thi 2 popup thong bao
                                    //MessageBox.Show(eHCMSResources.A0279_G1_Msg_InfoCNhatOK);
        ...
        private void RefGenMedProductDetails_Save_New(RefGenMedProductDetails Obj)
        {
            //Lấy thêm StaffID để ghi log
            Obj.StaffID = Globals.LoggedUserAccount.Staff.StaffID;
            ...
            //20180802 TTM: Lấy giá trị combobox ra để tạo cảnh báo
            if (cbb_Type.SelectedValue == null)
            {
                MessageBox.Show(eHCMSResources.Z2258_G1_ChuaChonPhanLoai);
                IsWaitingSave = false;
                return;
            }
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginRefGenMedProductDetails_Save_New(Obj, RefMedicalConditionType_Edit.TempObject, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                string Res = "";
                                long GenMedProductID;
                                contract.EndRefGenMedProductDetails_Save_New(out Res, out GenMedProductID, asyncResult);
                                /*▼====: #003*/
                                    ...
                                    case "Update-1":
                                        {
                                            ...
                                            InsertDeleteUpdateRouteOfAdministrationContactDrugXML(createListROANew(),createListROADelete(), null);
                                            break;
                                        }
                                    case "Insert-0":
                                        {
                                    ...
                                    case "Insert-1":
                                        {
                                            ...
                                            MessageBox.Show(eHCMSResources.A0468_G1_Msg_InfoLuuOK, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OK);
                                            //TryClose();
                                            // add new ConIndicatorDrugsRelToMedCondXML
                                            InsertDeleteUpdateConIndicatorDrugsRelToMedCondXML(createListNew(), createListDelete(), null);
                                            InsertDeleteUpdateRouteOfAdministrationContactDrugXML(createListROANew(), createListROADelete(), null);
                                            break;
                                        }
        ...
        {
            if (!(temp is RefGenMedProductDetails u))
            {
                return false;
            }
            string error = "";
            ...
            if ((u.SelectedGeneric == null || u.SelectedGeneric.DrugClassID <= 0)
                && u.V_MedProductType != (long) AllLookupValues.MedProductType.NUTRITION)
            {
            ...
            // 20200116 TNHX: Cột này trên giao diện đã không còn. thêm điều kiện để tranh báo lỗi đối với thuốc k BHYT
            if (u.InsuranceCover.GetValueOrDefault() && (u.HIPaymentPercent == null || u.HIPaymentPercent <= 0 || u.HIPaymentPercent > 1))
            {
            ...
            if (u.V_ProductScope == 0 && u.InsuranceCover == true)
            {
                error += "Chưa chọn phạm vi!";
            }
            if (!string.IsNullOrEmpty(error))
            {
                MessageBox.Show(error, eHCMSResources.G0442_G1_TBao);
        ...
        private ObservableCollection<RouteOfAdministrationContactDrug> _allV_RouteOfAdministration_Edit;
        public ObservableCollection<RouteOfAdministrationContactDrug> allV_RouteOfAdministration_Edit
        {
            get
            {
                return _allV_RouteOfAdministration_Edit;
            }
            set
            {
                if (_allV_RouteOfAdministration_Edit == value)
                    return;
                _allV_RouteOfAdministration_Edit = value;
                NotifyOfPropertyChange(() => allV_RouteOfAdministration_Edit);
            }
        }
                        ...
                        try
                        {
                            ...
                            if (results != null)
                            {
                                ...
                                foreach (var p in results)
                                {
                                    ...
                                    if (p.RefMedicalConditionType == null)
                                    {
        ...
        public void GetRouteOfAdministrationList(long DrugROAID, long DrugID)
        {
            Globals.EventAggregator.Publish(new BusyEvent { IsBusy = true, Message = eHCMSResources.Z0125_G1_DangXuLi });
            var t = new Thread(() =>
            {
                    ...
                    contract.BeginGetRouteOfAdministrationList(DrugROAID, DrugID, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            ...
                            if (results != null)
                            {
                                ...
                                foreach (var p in results)
                                {
                                    allV_RouteOfAdministration_Edit.Add(p);
                                    ...
                                    V_RouteOfAdministration_Edit = new EntitiesEdit<Lookup>(obsRMCT);
                                }
                                NotifyOfPropertyChange(() => allContraIndicatorDrugsRelToMedCond);
                            }
                        }
        ...
        public void AddRouteOfAdministration()
        {
            Action<IRouteOfAdministrationContactDrug> onInitDlg = delegate (IRouteOfAdministrationContactDrug MedConditionVM)
            {
                MedConditionVM.V_RouteOfAdministration_Edit = V_RouteOfAdministration_Edit;
                MedConditionVM.NewDrug = ObjRefGenMedProductDetails_Current;
                this.ActivateItem(MedConditionVM);
            };
            GlobalsNAV.ShowDialog<IRouteOfAdministrationContactDrug>(onInitDlg);
        }
        ...
        public void Handle(V_RouteOfAdministrationEvent message)
        {
            if (message != null)
            {
                V_RouteOfAdministration_Edit = message.V_RouteOfAdministration_Edit;
            }
        }
        ...
        /*▲====: #004*/
        private void GetAllRefGenMedProductDetail(long V_MedProductType)
        {
            this.DlgShowBusyIndicator(eHCMSResources.Z0125_G1_DangXuLi);
            var t = new Thread(() =>
            {
                ...
                {
                    var contract = serviceFactory.ServiceInstance;
                    contract.BeginGetAllRefGenMedProductDetail(V_MedProductType, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            IList<RefGenMedProductDetails> results = contract.EndGetAllRefGenMedProductDetail(asyncResult);
                            if (results != null)
                            {
                                RefGenMedProductDetaillst = results.ToObservableCollection();
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        AutoCompleteBox Auto_Code;
        public void Auto_Code_Loaded(object sender, RoutedEventArgs e)
        {
            Auto_Code = sender as AutoCompleteBox;
        }
        public void Code_DropDownClosed(object sender, RoutedPropertyChangedEventArgs<bool> e)
        {
            AutoCompleteBox axCode = sender as AutoCompleteBox;
            if (ObjRefGenMedProductDetails_Current != null)
            {
                if (axCode != null && axCode.SelectedItem != null)
                {
                    RefGenMedProductDetails item = axCode.SelectedItem as RefGenMedProductDetails;
                    if (item != null && item.GenMedProductID > 0)
                    {
                        ObjRefGenMedProductDetails_Current.LastID = item.GenMedProductID;
                        ObjRefGenMedProductDetails_Current.LastCode = item.Code;
                    }
                    else
                    {
                        ObjRefGenMedProductDetails_Current.LastID = 0;
                        ObjRefGenMedProductDetails_Current.LastCode = "";
                    }
                }
                else
                {
        ...
        private bool _UseBidDetailOnInward = Globals.ServerConfigSection.MedDeptElements.UseBidDetailOnInward;
        public bool UseBidDetailOnInward
        {
            ...
            set
            {
                ...
                NotifyOfPropertyChange(() => UseBidDetailOnInward);
            }
        }
        //▼===== #012
        public void txt_Volume_LostFocus(object sender, RoutedEventArgs e)
        {
            if (ObjRefGenMedProductDetails_Current == null)
            {
                return;
            ...
            if (!string.IsNullOrEmpty(txt))
            {
                ...
                ObjRefGenMedProductDetails_Current.Volume = value > 0 ? value : 0;
            }
        ...
        public void DispenseVolume_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txt))
            {
                double value = 0;
                double.TryParse(txt, out value);
                ObjRefGenMedProductDetails_Current.DispenseVolume = value > 0 ? value : 0;
            }
            else //TBL: Trường hợp xóa thì set = 0 
            {
                ObjRefGenMedProductDetails_Current.DispenseVolume = 0;
            }
        }
        //▲===== #012
        ...
        //▼===== #013
        private ObservableCollection<Lookup> _DrugFormulationMethodCollection;
        public ObservableCollection<Lookup> DrugFormulationMethodCollection
        {
            get
            {
                return _DrugFormulationMethodCollection;
            }
            set
            {
                _DrugFormulationMethodCollection = value;
                NotifyOfPropertyChange(() => DrugFormulationMethodCollection);
            }
        }
        ...
        private ObservableCollection<Lookup> _V_PaymentSourceCollection;
        public ObservableCollection<Lookup> V_PaymentSourceCollection
        {
            get
            {
                return _V_PaymentSourceCollection;
            }
            set
            {
                _V_PaymentSourceCollection = value;
                NotifyOfPropertyChange(() => V_PaymentSourceCollection);
            }
        }
        //▲===== #013

        //#Issue 1074
        //private string _GenericNameCode;
        //public string GenericNameCode
        //{
        //    get
        //    {
        //        return _GenericNameCode;
        //    }
        //    set
        //    {
        //        _GenericNameCode = value;
        //        NotifyOfPropertyChange(() => GenericNameCode);
        //    }
        //}

        //public void SetGenericNameCode()
        //{
        //    if (ObjRefGenMedProductDetails_Current != null && ObjRefGenMedProductDetails_Current.SelectedGeneric != null)
        //    {
        //        GenericNameCode = ObjRefGenMedProductDetails_Current.SelectedGeneric.FaName + " - " + ObjRefGenMedProductDetails_Current.SelectedGeneric.DrugClassCode;
        //    }
        //}
    }
}
