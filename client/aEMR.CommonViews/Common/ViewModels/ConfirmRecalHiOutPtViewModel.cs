...
* 20200618 #010 TTM:    BM 0039277: Fix lỗi bệnh nhân có 2 toa thuốc 1 dv 1 bh, toa dịch vụ ra sau thì toa bảo hiểm sẽ bị bỏ qua không kiểm tra ngày thuốc.
* 20200628 #011 TTM:    Thêm điều kiện chặn nếu như đăng ký đã được xác nhận thì không cho phép huỷ phiếu xuất.
* 20200811 #012 TNHX:   Print PhieuNhanThuoc base on config PhieuNhanThuocPrintingModeInConfirmHIView. Tạm thời chưa làm chức năng in tự động
* 20200818 #013 BLQ:   Không tự hủy phiếu xuất khi hủy xác nhận BH qua ngày
* 20220511 #014 QTD:    Get OutcashAdvanceID cuối cùng để in biên lai
* 20220521 #015 BaoLQ: Thêm chức năng in toa thuốc khi xem mẫu 12
* 20220903 #016 BaoLQ: Thêm function In mẫu 12
*                      Chỉnh lại nút Mẫu 01/KCB xem/in và in theo cấu hình
* 20221003 #017 DatTB: Thêm điều kiện không in biên lai 0đ quầy duyệt toa BHYT
* 20221205 #018 QTD:  Đẩy cổng XML tự động khi xác nhận QLBH ngoại trú
* 20221217 #019 TNHX: Đẩy toa thuốc điện tử khi xác nhận QLBH ngoại trú
* 20230108 #020 TNHX: 994 Sửa lỗi + chỉnh câu 1 hình mới
* 20230420 #021 TNHX: Bỏ chạy tự động trên HIS chuyển qua service
*/

namespace aEMR.Pharmacy.ViewModels
{
    [Export(typeof(IConfirmRecalHiOutPt)), PartCreationPolicy(CreationPolicy.NonShared)]
    public partial class ConfirmRecalHiOutPtViewModel : Common.ViewModels.PharmacyHandlePayCompletedViewModelBase, IConfirmRecalHiOutPt
        ...
        , IHandle<RemoveItem<InPatientBillingInvoice>>
    {
        ...
        private IConfirmRecalHiOutPtView _currentView;
        protected override void OnViewLoaded(object view)
        {
            base.OnViewLoaded(view);
            _currentView = view as IConfirmRecalHiOutPtView;
        }

        [ImportingConstructor]
        public ConfirmRecalHiOutPtViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            authorization();
            //==== #001
            ...
            //==== #001
            GetStaffLogin();
            ListPrescriptionIsNotSold = new ObservableCollection<Prescription>();
            RefeshData();
            SetDefaultForStore();
        }
        ...
        {
            string error = "";
            if (PatientInfo.LatestRegistration != null && Globals.IsLockRegistration(PatientInfo.LatestRegistration.RegLockFlag, "bán thuốc"))
            {
                return;
            }
            if (SelectedOutInvoice == null)
            ...
            error += CheckThuocBiTrungHoatChatBaoHiemConThuoc();
            ...
            // QMS Service
            // BaoLQ
            //if (Globals.ServerConfigSection.CommonItems.ApplyQMSAPI)
            //{
        ...
        public void btnMoney(object sender, RoutedEventArgs e)
        {
            if (PatientInfo != null && PatientInfo.LatestRegistration != null 
                && Globals.IsLockRegistration(PatientInfo.LatestRegistration.RegLockFlag, "thu tiền")
                && !Globals.ServerConfigSection.CommonItems.IsApplyAutoCreateHIReportWhenConfirmHI)
            {
                return;
            }
            //if (Convert.ToInt32(Globals.ConfigList[(int)AppConfigKeys.ConfigItemKey.PharmacyCountMoneyIndependent]) == 1)
            ...
            //▼===== Trở về như cũ luôn hiển thị, nếu chưa xác nhận thì không hiển thị popup
            //if (PatientInfo != null 
            //    && PatientInfo.LatestRegistration != null
            //    && PatientInfo.LatestRegistration.PatientTransaction != null
            //    && PatientInfo.LatestRegistration.PatientTransaction.PatientTransactionDetails != null
            //    && PatientInfo.LatestRegistration.PatientTransaction.PatientTransactionDetails.All(x => x.IsPaided))
            //{
            //    return;
            //}
            //▲=====
        ...
        private void LoadInfoCommon()
        {
            if (SelectedPrescription != null)
            {
                ...
                strNgayDung = "";
                ((ConfirmRecalHiOutPtView)this.GetView()).AutoDrug_Text.Text = "";
                ((ConfirmRecalHiOutPtView)this.GetView()).chbBH.IsChecked = false;
                //---------------------------------------------------------
                SelectedOutInvoice = null;
                SelectedOutInvoice = new OutwardDrugInvoice();
                SelectedOutInvoice.OutDate = Globals.ServerDate.Value;
        ...
        private IEnumerator<IResult> DoGetInfoPatientPrescription()
        {
            ...
            if (IsConfirmHIView)
            {
                var loadRegTask = new LoadRegistrationInfoTask((long)SelectedPrescription.PtRegistrationID, true);
                ...
                {
                    PatientInfo = loadRegTask.Registration.Patient;
                    PatientInfo.CurrentHealthInsurance = loadRegTask.Registration.HealthInsurance;
            ...
            if (IsConfirmHIView && !SeccondLoad)
            {
                spGetInBatchNumberAndPrice_ByPresciption(0, StoreID, isobject, true, SelectedPrescription.PtRegistrationID);
            }
            else if (Globals.ServerConfigSection.CommonItems.EnableHIStore && !IsConfirmHIView
        ...
        {
            Action<IPrescriptionSearch> onInitDlg = delegate (IPrescriptionSearch proAlloc)
            {
                ...
                using (var serviceFactory = new ePrescriptionsServiceClient())
                {
                    ...
                    contract.BeginSearchPrescription(SearchCriteria, PageIndex, PageSize, true, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            ...
                            {
                                //▼=====  Dù tìm được 1 hay nhiều toa thuốc thì luôn luôn bung popup lên để người dùng lựa chọn, không tự động chọn nếu chỉ
                                //        tìm thấy 1 toa thuốc.
        ...
        public void SearchPrescriptionToSell()
        {
            ...
            if (PatientInfo != null && PatientInfo.LatestRegistration != null)
            {
                SearchCriteria.PtRegistrationID = PatientInfo.LatestRegistration.PtRegistrationID;
                SearchPrescription(0, 20);
            }
            else
            {
        ...
        public void LoadPrescriptionDetail(Prescription aPrescription, bool SeccondLoadToSell = false)
        {
            SelectedPrescription = aPrescription;
            SeccondLoad = SeccondLoadToSell;
        ...
        public void Handle(PharmacyCloseSearchPrescriptionEvent message)
        {
            if (message != null)
            {
                LoadPrescriptionDetail(message.SelectedPrescription as Prescription, (message.SelectedPrescription as Prescription).IsSold);
            }
        }
        #endregion
        ...
        public void SearchInvoiceForSeccondSell()
        {
            SearchCriteria = new PrescriptionSearchCriteria();
            if (PatientInfo != null && PatientInfo.LatestRegistration != null)
            {
                SearchOutwardPrescriptionForSeccondSell(0, Globals.PageSize, PatientInfo.LatestRegistration.PtRegistrationID);
            }
            else
            {
                MessageBox.Show(eHCMSResources.A0635_G1_Msg_InfoKhCoDKDeThaoTac);
        ...
        private IEnumerator<IResult> DoGetInfoPatientInvoice(bool isCallByPayViewModel = false, bool aIsComfirmHICalled = false)
        {
            ...
            //▼===== 20191112 TTM:  Chuyển LoadPatientInfoByRegistrationTask xuống vì không cần thiết do LoadRegistrationInfoTask khi vào màn hình xác nhận đã load đầy đủ thông tin tương tự như LoadPatientInfoByRegistrationTask
            //                      mà còn chi tiết và đầy đủ hơn nên để lại thì nó sẽ bị xuống service 2 lần để thực hiện 1 việc.
            if (IsConfirmHIView)
            {
                ...
                if (OldPaymentContent != null && OldServiceContent != null && OldPclContent != null && loadRegTask.Registration != null)
                {
                    ...
                    OldPclContent.PCLRequests = PatientInfo.LatestRegistration.PCLRequests.Where(x => x.RecordState != RecordState.DELETED && x.PatientPCLRequestIndicators != null).ToObservableCollection();
                    OldBillingContent.UpdateItemList(PatientInfo.LatestRegistration.InPatientBillingInvoices == null || !PatientInfo.LatestRegistration.InPatientBillingInvoices.Any(x => x.PaidTime != null && x.RecordState != RecordState.DELETED) ? null : PatientInfo.LatestRegistration.InPatientBillingInvoices.Where(x => x.PaidTime != null && x.RecordState != RecordState.DELETED).ToList());
                    ////▼===== 20200710 TTM:  Fix lỗi nếu đăng ký điều trị ngoại trú xác nhận sẽ không không hiển thị nút thu tiền
                    ////                      Nguyên nhân là biding bằng CanGetMoney của OutwardDrugInvoice vì CanGetMoney chỉ set true khi chưa trả tiền.
                    ////                      Mà đăng ký điều trị ngoại trú đã trả tiền thuốc từ ngày đầu tiên. Nên loại bỏ ra và sử dụng biến mới
                    ////                      IsEnableGetMoney điều kiện tổng tiền bệnh nhân phải trả - tổng bệnh nhân đã trả != 0 (Anh Tuân kêu).
                    //if (loadRegTask != null && loadRegTask.Registration != null && loadRegTask.Registration.ConfirmHIStaffID > 0)
                    //{
                    //    IsEnableGetMoney = false;
                    //    if (loadRegTask.Registration.PayableSum != null && (loadRegTask.Registration.PayableSum.TotalPatientPayment - loadRegTask.Registration.PayableSum.TotalCashAdvanceAmount) != 0)
                    //    {
                    //        IsEnableGetMoney = true;
                    //    }
                    //}
                    ////▲=====
                }
                    ...
                    contract.BeginGetOutwardDrugDetailsByOutwardInvoice_V2(SelectedOutInvoice.outiID, IsHIOutPt, OutiIDArray, PtRegistrationID, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            ...
                            if (results != null)
                            {
                                ...
                                {
                                    var PrintMode = Globals.ServerConfigSection.CommonItems.PhieuNhanThuocPrintingModeInConfirmHIView;
                                    switch (PrintMode)
                                    {
                                        case 1:
                                            btnPreview();
                        ...
                        try
                        {
                            ...
                            if (results != null && results.Count > 0)
                            {
                                if (results.Count > 1 && this.IsConfirmHIView && results.Select(x => x.PtRegistrationID).Distinct().Count() == 1)
                                {
                                    ...
                                    SelectedOutInvoice = (results.OrderBy(x => x.V_OutDrugInvStatus).ThenBy(x => x.PaidTime).ThenByDescending(x => x.IsHICount).ThenByDescending(x => x.StoreID)).FirstOrDefault();
        ...
        private void SearchOutwardPrescriptionForSeccondSell(int PageIndex, int PageSize, long PtRegistrationID = 0)
        {
            int Total = 0;
            this.ShowBusyIndicator();
            if (SearchInvoiceCriteria == null)
        ...
        {
            var printingMode = Globals.ServerConfigSection.CommonItems.Temp12PrintingMode;
            switch (printingMode)
            {
                case 1:
                    btnPreview12();
        ...
        public void btnPreview12()
        {
            if (SelectedOutInvoice == null) return;
            //▼====: #015
            if (Globals.ServerConfigSection.CommonItems.PrintPrescriptionWithTemp12 && Globals.ServerConfigSection.Hospitals.HospitalCode == "96160")
            {
                GetPrecriptionsByPtRegistrationID();
            }
            //▲====: #015
            Action<IReportDocumentPreview> onInitDlg = delegate (IReportDocumentPreview proAlloc)
            {
        ...
        public void btnPrint12Silent()
        {
            ...
            if (Globals.ServerConfigSection.CommonItems.ApplyTemp12Version6556)
            {
                PrintTemp12_6556Silent(SelectedOutInvoice.PtRegistrationID.GetValueOrDefault(0));
            }
            else
            {
                PrintTemp12Silent(SelectedOutInvoice.PtRegistrationID.GetValueOrDefault(0));
            }
        }
        private void PrintTemp12Silent(long PtRegistrationID)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetXRpt_Temp12InPdfFormat(0, PtRegistrationID, Globals.GetCurServerDateTime(), Globals.GetCurServerDateTime(), false
                            , Globals.LoggedUserAccount != null && Globals.LoggedUserAccount.Staff != null ? Globals.LoggedUserAccount.Staff.FullName : "" , 0 , "Khoa Khám Bệnh"
                            , (long)AllLookupValues.RegistrationType.NGOAI_TRU
                            , Globals.FirstCharToUpper(Globals.ServerConfigSection.CommonItems.ReportHospitalName.ToLower())
                            , Globals.FirstCharToUpper(Globals.ServerConfigSection.CommonItems.ReportHospitalAddress.ToLower())
                            , Globals.FirstCharToUpper(Globals.ServerConfigSection.CommonItems.ReportDepartmentOfHealth.ToLower())
                            , false
                            , Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    var results = contract.EndGetXRpt_Temp12InPdfFormat(asyncResult);
        ...
        private void PrintTemp12_6556Silent(long PtRegistrationID)
        {
            this.ShowBusyIndicator(eHCMSResources.Z1544_G1_DangIn);
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetXRpt_Temp12_6556InPdfFormat(0, PtRegistrationID, Globals.GetCurServerDateTime(), Globals.GetCurServerDateTime(), false
                            , Globals.LoggedUserAccount != null && Globals.LoggedUserAccount.Staff != null ? Globals.LoggedUserAccount.Staff.FullName : "" , 0 , "Khoa Khám Bệnh"
                            , (long)AllLookupValues.RegistrationType.NGOAI_TRU
                            , Globals.FirstCharToUpper(Globals.ServerConfigSection.CommonItems.ReportHospitalName.ToLower())
                            , Globals.FirstCharToUpper(Globals.ServerConfigSection.CommonItems.ReportHospitalAddress.ToLower())
                            ...
                            , Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    var results = contract.EndGetXRpt_Temp12_6556InPdfFormat(asyncResult);
                                    var printEvt = new ActiveXPrintEvt(this, PrinterType.IN_PHIEU, results, ActiveXPrintType.ByteArray, 1);
                                    Globals.EventAggregator.Publish(printEvt);
                                }
                                catch (Exception ex)
        ...
        //▲====: #016
        //▼====: #015
        private void GetPrecriptionsByPtRegistrationID()
        {
            if (SelectedOutInvoice == null) return;
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetPrescriptionByServiceRecID(SelectedOutInvoice.SelectedPrescription.PatientID.GetValueOrDefault(0)
                            , null
                            , SelectedOutInvoice.PtRegistrationID.GetValueOrDefault(0)
                            , null
                            , Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    var results = contract.EndGetPrescriptionByServiceRecID(asyncResult);
                                    if(results != null)
                                    {
                                        foreach (var item in results)
                                        {
                                            item.PrescriptionDetails = ReadPrescriptionDetailFromString(item.PrescriptDetailsStr);
                                            PrintPrecriptions(item);
                                        }
                                    }
                                }
                                catch (Exception ex)
        ...
        public void PrintPrecriptions(Prescription PrescriptionsForPrint)
        {
            if (PrescriptionsForPrint.IssueID <= 0)
            {
                MessageBox.Show(eHCMSResources.A0668_G1_Msg_InfoKhCoToaThuocDeIn, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OK);
                return;
            }
            int numOfCopies = 0;
            bool IsPsychotropicDrugs = false;
            bool IsFuncfoodsOrCosmetics = false;
            bool HasPharmacyDrug = false;
            ...
            //foreach (var PsychotropicDrugs in PrescriptionsForPrint.PrescriptionDetails)
            //{
            ...
            //foreach (var FuncfoodsOrCosmetics in PrescriptionsForPrint.PrescriptionDetails)
            //{
            ...
            if (PrescriptionsForPrint == null || PrescriptionsForPrint.PrescriptionDetails == null || PrescriptionsForPrint.PrescriptionDetails.Count == 0)
                parTypeOfForm = 0;
            //foreach (var FuncPharmacyDrugs in PrescriptionsForPrint.PrescriptionDetails)
            //{
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetXRptEPrescriptionInPdfFormat(PrescriptionsForPrint.IssueID, IsPsychotropicDrugs, IsFuncfoodsOrCosmetics
        ...
        public void Handle(PayForRegistrationCompletedAtConfirmHIView message)
        {
            if (message != null && IsActive)
            {
                ...
                else
                {
                    ...
                    {
                        if (message.Registration != null && message.Registration.PatientTransaction != null && message.Registration.PatientTransaction.PatientCashAdvances != null)
                        {
                            if (message.Registration.PtRegistrationID != 0)
                            {
                                GetLastOutPatientCashAdvance(message.Registration.PtRegistrationID, true, message);
                            }
                        }
                    }
                    //▲====: #014
        ...
        private void CancelCurrentInvoice()
        {
            ...
            if (PatientInfo != null && PatientInfo.LatestRegistration != null && Globals.IsLockRegistration(PatientInfo.LatestRegistration.RegLockFlag, "hủy phiếu xuất"))
            {
                return;
            }
            if (Globals.ServerConfigSection.PharmacyElements.PharmacyCountMoneyIndependent == 1)
        ...
        {
            if (PatientInfo != null && PatientInfo.LatestRegistration != null && Globals.IsLockRegistration(PatientInfo.LatestRegistration.RegLockFlag, "cập nhật phiếu"))
            {
                return;
            }
        ...
        {
            if (PatientInfo != null && PatientInfo.LatestRegistration != null && Globals.IsLockRegistration(PatientInfo.LatestRegistration.RegLockFlag, "hoàn tiền"))
            {
                return;
            }
            //if (Convert.ToInt32(Globals.ConfigList[(int)AppConfigKeys.ConfigItemKey.PharmacyCountMoneyIndependent]) == 1)
        ...
        public void btnCancelConfirmHIBenefit()
        {
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginCancelConfirmHIBenefit(PatientInfo.LatestRegistration.PtRegistrationID, Globals.LoggedUserAccount.Staff.StaffID,
                            Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (PatientInfo != null && PatientInfo.LatestRegistration != null)
                                        {
                                            ...
                                            NotifyOfPropertyChange(() => IsEnableCancelBillingButton);
                                            //▼===== 20200625 TTM:  Chỉ có những bệnh nhân không phải là điều trị ngoại trú khi huỷ xác nhận mới tự động huỷ phiếu
                                            //                      xuất luôn
                                            if ((PatientInfo.LatestRegistration.OutPtTreatmentProgramID == null
                                                || PatientInfo.LatestRegistration.OutPtTreatmentProgramID == 0)
                                                //▼===== 013
                                                && SelectedOutInvoice.OutDate.Date == Globals.GetCurServerDateTime().Date)
                                                //▲===== 013
                                            {
                                                CancelCurrentInvoice();
                                            }
        ...
        //▼===== 20192311 TTM: Comment ra vì code này không còn xài từ lâu rồi.
        //public Dictionary<long, IList<OutwardDrug>> SavedOutwardDrugInvoice = new Dictionary<long, IList<OutwardDrug>>();
        ...
        public void btnFinalization()
        {
            ...
            GlobalsNAV.ShowDialog<IEditOutPtTransactionFinalization>((IEditOutPtTransactionFinalization aView) =>
            {
                ...
                {
                    TaxMemberName = PatientInfo.FullName,
                    TaxMemberAddress = PatientInfo.PatientStreetAddress
                    ,
                        ...
                        contract.BeginRecalRegistrationHIBenefit(aRegistration.PtRegistrationID, Globals.LoggedUserAccount.Staff.StaffID
                            ...
                            , Globals.DispatchCallback((asyncResult) =>
                            {
                                ...
                                try
                                {
                                    ...
                                    if (retval && GlobalsNAV.IsQMSEnable() && null != aRegistration.Patient)
                                    {
                                        OrderDTO orderDTO = GlobalsNAV.UpdateOrder(aRegistration.Patient, OrderDTO.DONE_STATUS);
                                    }
                                    GlobalsNAV.ShowMessagePopup(eHCMSResources.Z2387_G1_XacNhanThanhCong);
                                    //▼====: #020
                                    //▼====: #018
                                    //if (Globals.ServerConfigSection.CommonItems.IsApplyAutoCreateHIReportWhenConfirmHI 
                                    //    && aRegistration.V_RegistrationType == AllLookupValues.RegistrationType.NGOAI_TRU && (aRegistration.OutPtTreatmentProgramID == null || aRegistration.OutPtTreatmentProgramID == 0))
                                    //{
                                    ...
                                    //▲====: #020
                                    //▼====: #019
                                    if (Globals.ServerConfigSection.CommonItems.IsApplyAutoCreateDTDTReportWhenConfirmHI)
                                    {
        ...
        #region New method.
        //▼===== 20191126 Code gom các hàm load chuẩn bị dữ liệu để thực hiện xuất phiếu xuất thành 1 lần đi về db duy nhất
        private ObservableCollection<Prescription> _ListPrescriptionForConfirm;
        public ObservableCollection<Prescription> ListPrescriptionForConfirm
        {
            get
            {
                return _ListPrescriptionForConfirm;
            }
            set
            {
                if (_ListPrescriptionForConfirm != value)
                {
                    _ListPrescriptionForConfirm = value;
                    NotifyOfPropertyChange(() => ListPrescriptionForConfirm);
                }
            }
        }
        private ObservableCollection<Prescription> _ListPrescriptionIsNotSold;
        public ObservableCollection<Prescription> ListPrescriptionIsNotSold
        {
            get
            {
                return _ListPrescriptionIsNotSold;
            }
            set
            {
                if (_ListPrescriptionIsNotSold != value)
                {
                    _ListPrescriptionIsNotSold = value;
                    NotifyOfPropertyChange(() => ListPrescriptionIsNotSold);
                }
            }
        }
        public void PrepareDataForConfirmRegistration(ObservableCollection<Prescription> aPrescription, PatientRegistration cPatientRegistration)
        {
            if (cPatientRegistration == null && cPatientRegistration.Patient == null || aPrescription == null && aPrescription.Count == 0)
            {
                MessageBox.Show(eHCMSResources.Z2439_G1_DuLieuDauVaoKhongDungQD);
                return;
            }
            ListPrescriptionForConfirm = aPrescription;
            SelectedPrescription = aPrescription.First();
            //20191123 TTM: vì lý do biến PatientInfo không được set lại mà được sử dụng nên set thẳng từ cPatientRegistration sử dụng luôn.
            PatientInfo = cPatientRegistration.Patient;
            PatientInfo.LatestRegistration = cPatientRegistration;

            //20191123 TTM: Kiểm tra xem toa thuốc đã xuất ra phiếu xuất chưa nếu đã xuất rồi thì searchinvoice.
            int count = 0;
            if (ListPrescriptionForConfirm != null && ListPrescriptionForConfirm.Count > 0)
            {
                foreach (Prescription PrescriptionDetail in ListPrescriptionForConfirm)
                {
                    if (PrescriptionDetail.IsSold)
                    {
                        count++;
                    }
                }
                if (count == ListPrescriptionForConfirm.Count())
                {
                    ApplySearchInvoiceCriteriaValues(null, cPatientRegistration.PtRegistrationID);
                    SearchInvoiceOld();
                    return;
                }
            }
            Coroutine.BeginExecute(GetDataForConfirmTask(cPatientRegistration.PtRegistrationID));
        }
        
        private IEnumerator<IResult> GetDataForConfirmTask(long PtRegistrationID)
        {
            if (ListPrescriptionForConfirm == null && ListPrescriptionForConfirm.Count <= 0)
            {
                MessageBox.Show(eHCMSResources.Z2439_G1_DuLieuDauVaoKhongDungQD);
            ...
            Prescription FirstPrescriptionOnList = new Prescription();
            if (ListPrescriptionIsNotSold != null && ListPrescriptionIsNotSold.Count > 0)
            {
                FirstPrescriptionOnList = ListPrescriptionIsNotSold.First();
            }

            //FirstPrescriptionOnList = ListPrescriptionForConfirm.First();

            if (FirstPrescriptionOnList != null)
            {
                SelectedSellVisitor = null;
                strNgayDung = "";
                ((ConfirmRecalHiOutPtView)this.GetView()).AutoDrug_Text.Text = "";
                ...
                SelectedOutInvoice = new OutwardDrugInvoice();
                SelectedOutInvoice.SelectedPrescription = FirstPrescriptionOnList;
                SelectedOutInvoice.PrescriptID = FirstPrescriptionOnList.PrescriptID;
                SelectedOutInvoice.PtRegistrationID = FirstPrescriptionOnList.PtRegistrationID;
                SelectedPrescription.IssuedDateTime = FirstPrescriptionOnList.IssuedDateTime;
                HideShowColumnDelete();
                NotifyOfPropertyChange(() => VisibilityName);
                NotifyOfPropertyChange(() => VisibilityCode);
            }
            ...
            //    && PatientInfo.CurrentHealthInsurance.ValidDateTo >= Globals.ServerDate.Value.Date
            //    && SelectedOutInvoice != null && SelectedOutInvoice.SelectedPrescription != null
            //    && SelectedOutInvoice.SelectedPrescription.Issue_HisID > 0 && !SelectedOutInvoice.SelectedPrescription.IsSold)
            //{
            //    TimeSpan Time = PatientInfo.CurrentHealthInsurance.ValidDateTo.GetValueOrDefault() - Globals.GetCurServerDateTime().Date;
            //    DayRpts = Convert.ToInt32(Time.TotalDays + 1);
            //    Visibility = Visibility.Collapsed;
            //    IsHIPatient = true;
            //}
            ...
            {
                IsHIPatient = false;
                foreach (var PrescriptionDetail in ListPrescriptionIsNotSold)
                {
                    if (PrescriptionDetail.Issue_HisID > 0)
                    {
                        TimeSpan Time = PatientInfo.CurrentHealthInsurance.ValidDateTo.GetValueOrDefault() - Globals.GetCurServerDateTime().Date;
                        DayRpts = Convert.ToInt32(Time.TotalDays + 1);
                        Visibility = Visibility.Collapsed;
            ...
            //▼===== 20200702 TTM:  Vì nguyên nhân đối với các ca không có toa thuốc sẽ không load thông tin thuốc tạo ra phiếu xuất để thực hiện
            //                      => nó sẽ mặc định hiển thị các dịch vụ đã thực hiện toa rỗng và xác nhận.
            //                      Do dữ liệu anh Tuấn chuyển sang chỉ có thông tin đăng ký => Không có chi tiết dịch vụ => không refresh được màn hình
            //                      Nên giữ thông tin của các trước đó lên.
            //                      Khúc code này kiểm tra xem toa thuốc của bệnh nhân có phải toa rỗng không nếu là toa rỗng thì load lại đăng ký để lấy được
            //                      danh sách dịch vụ mà hiển thị cho chính xác.
            if (SelectedOutInvoice != null && SelectedOutInvoice.SelectedPrescription != null && SelectedOutInvoice.SelectedPrescription.IsEmptyPrescription)
            {
                var loadRegTask = new LoadRegistrationInfoTask((long)SelectedPrescription.PtRegistrationID, true);
                yield return loadRegTask;
                if (OldPaymentContent != null && OldServiceContent != null && OldPclContent != null && loadRegTask.Registration != null)
                ...
                else if (OldPaymentContent != null && OldServiceContent != null && OldPclContent != null && OldPaymentContent != null)
                {
                    OldPaymentContent.InitViewForPayments(null);
                    OldServiceContent.UpdateServiceItemList(null);
                    OldPclContent.PCLRequests = null;
            ...
            if (IsHIPatient)
            {
                if (SubDate(FirstPrescriptionOnList.IssuedDateTime.Value.Date, DateServer) >= Globals.ServerConfigSection.HealthInsurances.MaxDaySellPrescriptInsurance)
                {
                    MessageBox.Show(eHCMSResources.Z0895_G1_BHYTKgTraTienCHoToa);
                    IsHIPatient = false;
                }
                if (!IsNotCountInsurance)
                {
                    isobject = 1;
                }
            }

            //▼===== Luôn load toa chưa trả tiền để thực hiện.
            if (ListPrescriptionIsNotSold != null && ListPrescriptionIsNotSold.Count > 0)
            {
                GetOutwardDrugAndPrice_ByListPresciption(ListPrescriptionIsNotSold, StoreID, isobject, (long)SelectedOutInvoice.PtRegistrationID);
            }

            //GetOutwardDrugAndPrice_ByListPresciption(ListPrescriptionForConfirm, StoreID, isobject, (long)SelectedOutInvoice.PtRegistrationID);
            NotifyOfPropertyChange(() => ValueQuyenLoiBH);
            yield break;
        }
        public void GetOutwardDrugAndPrice_ByListPresciption(ObservableCollection<Prescription> ListPrescription, long aStoreID, short aIsObject, long PtRegistrationID)
        {
            if (SelectedOutInvoice == null)
            {
                return;
            ...
            isLoadingDetail = true;
            DateTime LastDaySoldHI = DateTime.MinValue;
            ObservableCollection<GetDrugForSellVisitor> GetDrugForSellVisitorListByPrescriptIDForConfirm = new ObservableCollection<GetDrugForSellVisitor>();
            this.ShowBusyIndicator();
            var t = new Thread(() =>
            {
                using (var serviceFactory = new PharmacyInwardDrugServiceClient())
                {
                    var contract = serviceFactory.ServiceInstance;
                    SelectedOutInvoice.OutwardDrugs = new ObservableCollection<OutwardDrug>();
                    contract.BeginGetOutwardDrugAndPrice_ByListPresciption(ListPrescription, aStoreID, aIsObject, IsHIOutPt, PtRegistrationID, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            var results = contract.EndGetOutwardDrugAndPrice_ByListPresciption(out LastDaySoldHI, out GetDrugForSellVisitorListByPrescriptIDForConfirm, asyncResult);
                            if (results != null)
                            {
                                if (SelectedOutInvoice != null)
                                {
                                    if (LastDaySoldHI != null)
                                    {
                                        NgayBHGanNhat = LastDaySoldHI;
                                    }
                                    if (GetDrugForSellVisitorListByPrescriptIDForConfirm != null && GetDrugForSellVisitorListByPrescriptIDForConfirm.Count > 0)
                                    {
                                        GetDrugForSellVisitorListByPrescriptID = GetDrugForSellVisitorListByPrescriptIDForConfirm;
                                    }
                                    SelectedOutInvoice.OutwardDrugs = results.ToObservableCollection();
                                    OutwardDrugsCopy = null;
                                    SumTotalPrice();
        ...
        //▼===== 20191209: Bổ sung code lấy dữ liệu cho các toa chưa trả tiền để thực hiện liên tục. Ví dụ: A đã trả tền sau đó ra tiếp toa B 
        //                 thì load toa B để thực hiện tiếp.
        public void PrepareDataForConfirmRegistration_New(ObservableCollection<Prescription> aPrescription, PatientRegistration cPatientRegistration)
        {
            if (cPatientRegistration == null && cPatientRegistration.Patient == null || aPrescription == null && aPrescription.Count == 0)
            {
                MessageBox.Show(eHCMSResources.Z2439_G1_DuLieuDauVaoKhongDungQD);
            ...
            //20191123 TTM: Kiểm tra xem toa thuốc đã xuất ra phiếu xuất chưa nếu đã xuất rồi thì searchinvoice.
            int count = 0;
            //20191209 TTM: Bổ sung danh sách các toa thuốc chưa IsSold = true
            ListPrescriptionIsNotSold = new ObservableCollection<Prescription>();
            ...
            if (ListPrescriptionForConfirm != null && ListPrescriptionForConfirm.Count > 0)
            {
                foreach (Prescription PrescriptionDetail in ListPrescriptionForConfirm)
                {
                    ...
                    {
                        ListPrescriptionIsNotSold.Add(PrescriptionDetail);
                    }
                }
                if (count == ListPrescriptionForConfirm.Count())
                {
        ...
        //▲===== 
        //private bool _IsEnableGetMoney = false;
        //public bool IsEnableGetMoney
        //{
        //    get { return _IsEnableGetMoney; }
        //    set
        //    {
        //        if (_IsEnableGetMoney != value)
        //        {
        //            _IsEnableGetMoney = value;
        //            NotifyOfPropertyChange(() => IsEnableGetMoney);
        //        }
        //    }
        //}

        private bool _SeccondLoad = false;
        public bool SeccondLoad
        {
            get { return _SeccondLoad; }
            set
            {
                if (_SeccondLoad != value)
                {
                    _SeccondLoad = value;
                    NotifyOfPropertyChange(() => SeccondLoad);
                }
            }
        }
        ...
        public void UpdateOrder(OrderDTO requestOrderDTO)
        {
            ...
            //    || String.IsNullOrEmpty(requestOrderDTO.GetPutPath()))
            //{
            //    return;
            //}
            //try
            ...
            //    var result = GlobalsNAV.PutRESTServiceJSon_New(Globals.ServerConfigSection.CommonItems.QMS_API_Url,
            //        eHCMSResources.Z3115_G1_PublishOrder, requestOrderDTO.GetPutPath(), requestOrderDTO);
            //    if (result != null)
            //    {
        ...
        //▼====: #014
        private void GetLastOutPatientCashAdvance(long PtRegistrationID, bool isGetLast, PayForRegistrationCompletedAtConfirmHIView _message)
        {
            var t = new Thread(() =>
            {
                using (var serviceFactory = new BillingPaymentWcfServiceLibClient())
                {
                    var contract = serviceFactory.ServiceInstance;
                    contract.BeginGetLastOutPatientCashAdvance(PtRegistrationID, isGetLast, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            lastOutPatientCashAdvance = contract.EndGetLastOutPatientCashAdvance(asyncResult);
                        }
                        catch (Exception ex)
                        {
                            MessageBox.Show(ex.Message, eHCMSResources.T0432_G1_Error, MessageBoxButton.OK);
                        ...
                        {
                            if(lastOutPatientCashAdvance != null && (lastOutPatientCashAdvance.PaymentAmount > 0 || lastOutPatientCashAdvance.PaymentAmount > 0)) //==== #017
                            {
                                _message.Registration.PatientTransaction.PatientCashAdvances.Add(lastOutPatientCashAdvance);
                                ProcessPayCompletedEvent(_message);
                            }
                        }
                    }), null);
                }
        ...
        private OutPatientCashAdvance _lastOutPatientCashAdvance;
        public OutPatientCashAdvance lastOutPatientCashAdvance
        {
            get
            {
                return _lastOutPatientCashAdvance;
            }
            set
            {
                if (_lastOutPatientCashAdvance != value)
                {
                    _lastOutPatientCashAdvance = value;
                }
                NotifyOfPropertyChange(() => lastOutPatientCashAdvance);
            }
        }
        //▲====: #014
        private List<Prescription> _Prescriptions_Current;
        public List<Prescription> Prescriptions_Current
        {
            get
            {
                return _Prescriptions_Current;
            }
            set
            {
                if (_Prescriptions_Current != value)
                {
                    _Prescriptions_Current = value;
                    NotifyOfPropertyChange(() => Prescriptions_Current);
                }
            }
        }
        ...
        private void GetAllPrescription_BaoHiemConThuoc()
        {
            ...
            GetAllPrescription_BaoHiemConThuoc_ByPatientID(PatientInfo.PatientID
                , 0
                , true);
                ...
                using (var serviceFactory = new ePrescriptionsServiceClient())
                {
                    ...
                    contract.BeginGetPrescriptionDetailsByPrescriptID(prescriptID, false, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            ...
                            {
                                ListPrescriptionDetailBaoHiemConThuocCungChuyenKhoa = null;
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private string CheckThuocBiTrungHoatChatBaoHiemConThuoc()
        {
            //TBL: Neu bat cau hinh thi moi kiem tra hoat chat
            if (!Globals.ServerConfigSection.ConsultationElements.CheckToaThuocBiTrungTheoHoatChatVaNgayThuocBaoHiem)
            {
                return "";
            }
            ObservableCollection<PrescriptionDetail> PrescriptionDetail = new ObservableCollection<PrescriptionDetail>();
            foreach (var item in SelectedOutInvoice.OutwardDrugs)
            {
                PrescriptionDetail.Add(item.PrescriptionDetailObj);
            }
            StringBuilder sb = new StringBuilder();
            string err = "";
            if (CheckToaThuocBiTrungBaoHiemConThuoc_TheoHoatChat(ListPrescriptionDetailBaoHiemConThuocCungChuyenKhoa, PrescriptionDetail, out err))
            {
                return string.Format("BN có thừa thuốc đợt điều trị cũ:" + Environment.NewLine + err + Environment.NewLine
                    + "Vui lòng kiểm tra phiếu thu tiền của BN, nếu BN đã đóng tiền thuốc thừa rồi, nhấn đồng ý lưu, tiến hành duyệt và" + Environment.NewLine
                    + "cấp phát bình thường. Nếu BN chưa đóng tiền thừa, vui lòng hướng dẫn BN gặp kế toán thu đóng tiền thuốc thừa. Trân trọng!");
            }
            return "";
        }

        private bool CheckToaThuocBiTrungBaoHiemConThuoc_TheoHoatChat(List<IList<PrescriptionDetail>> ListPrescriptionDetailBaoHiemConThuoc, IList<PrescriptionDetail> pNeedSave, out string msg)
        {
            string ChangePrice = "";
            StringBuilder sb = new StringBuilder();
            msg = "";
            if (pNeedSave != null && pNeedSave.Count > 0)
            {
                ...
                {
                    if (!CheckThuocBiTrungBaoHiemConThuoc_TheoHoatChat(ListPrescriptionDetailBaoHiemConThuoc, prescriptDetaiil, out ChangePrice) && prescriptDetaiil.SelectedDrugForPrescription != null && prescriptDetaiil.SelectedDrugForPrescription.GenericName != null)
                    {
        ...
        private bool CheckThuocBiTrungBaoHiemConThuoc_TheoHoatChat(List<IList<PrescriptionDetail>> ListPrescriptionDetail, PrescriptionDetail pNeedSave, out string ChangePrice)
        {
            ChangePrice = "";
            if (ListPrescriptionDetail != null && ListPrescriptionDetail.Count > 0)
            {
                foreach (IList<PrescriptionDetail> listdetail_i in ListPrescriptionDetail)
                {
                    foreach (var item in listdetail_i)
                    {
                        if (item.PrescriptID != pNeedSave.PrescriptID && item.DrugID != 0 && item.DrugID != null && item.SelectedDrugForPrescription != null 
                            && pNeedSave.SelectedDrugForPrescription != null 
                            && item.SelectedDrugForPrescription.GenericID == pNeedSave.SelectedDrugForPrescription.GenericID 
                            && item.BeOfHIMedicineList )//&& pNeedSave.BeOfHIMedicineList)
                        {
        ...
        private void CreateHIReportOutInPtFileTaskAuto(GenericCoRoutineTask genTask, object aRegistrationIDList)
        {
            var t = new Thread(() =>
            {
                try
        ...
        {
            if (PatientInfo == null || PatientInfo.LatestRegistration == null || PatientInfo.LatestRegistration.HIReportID != 0) yield break;
            ...
            var mCreateHIReportFileTask = new GenericCoRoutineTask(CreateHIReportOutInPtFileTaskAuto, mTitle);
            ...
            if (mHealthInsuranceReport != null)
            {
                if (mHealthInsuranceReport.HIReportID > 0)
                {
                    ...
                    PatientInfo.LatestRegistration.HIReportID = mHealthInsuranceReport.HIReportID;
                    PatientInfo.LatestRegistration.ReportAppliedCode = mHealthInsuranceReport.ReportAppliedCode;
                    PatientInfo.LatestRegistration.V_ReportStatus = new Lookup();
                    PatientInfo.LatestRegistration.V_ReportStatus.LookupID = mHealthInsuranceReport.V_ReportStatus;
                    PatientInfo.LatestRegistration.V_ReportStatus.ObjectValue = eHCMSResources.Z2333_G1_XacNhanBCHoanTat;
                }
        ...
        private IEnumerator<IResult> CreateDTDT_Routine()
        {
            ...
            if (mDTDTReport != null)
            {
                ...
                if (mDTDTReport.DTDTReportOutPt > 0)
                {
                    ...
                    PatientInfo.LatestRegistration.DTDTReportID = mDTDTReport.DTDTReportID;
                }
            }
