...
{
    [Export(typeof(IInPatientSelectBed)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class InPatientSelectBedViewModel : Conductor<object>, IInPatientSelectBed
    {
        ...
        public InPatientSelectBedViewModel(IWindsorContainer container, INavigationService navigationService, ISalePosCaching salePosCaching)
        {
                        ...
                        contract.BeginGetMedicalServiceTypesByInOutType(outTypes,
                            Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    ...
                                    foreach (RefMedicalServiceType info in allItems.Where(x=>x.V_RefMedicalServiceTypes == (long)AllLookupValues.V_RefMedicalServiceTypes.GIUONGBENH))
                                    {
        ...
        public void GetAllMedicalServiceItemsByType(RefMedicalServiceType serviceType = null)
        {
            this.ShowBusyIndicator(eHCMSResources.Z0992_G1_DangLayDSCacDV);
            var t = new Thread(() =>
            {
                AxErrorEventArgs error = null;
                try
                {
                    using (var serviceFactory = new PatientRegistrationServiceClient())
                    {
                        ...
                        contract.BeginGetAllMedicalServiceItemsByType(serviceTypeID, CurrentInPatientAdmDisDetail == null ? null : CurrentInPatientAdmDisDetail.MedServiceItemPriceListID,
                            (long)AllLookupValues.V_RefMedicalServiceTypes.GIUONGBENH, Globals.DispatchCallback((asyncResult) =>
                            {
