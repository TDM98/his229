...
{
    [Export(typeof(ISummary_V3)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class Summary_V3ViewModel : ViewModelBase, ISummary_V3
        ...
        , IHandle<LoadGeneralInfoPageEvent>
    {
        ...
        public Summary_V3ViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            CreateSubVM();
        ...
        {
            UCConsultations = Globals.GetViewModel<IConsultations_V3>();
            UCConsultations.IsShowSummaryContent = false;
            UCConsultations.IsEditingEnable += (aIsEnable) =>
            {
                ConsultationOldIsEditingEnable = aIsEnable;
            };
            UCePrescriptions = Globals.GetViewModel<IePrescriptions>();
            UCePrescriptions.IsShowSummaryContent = false;
            //UCePrescriptions.IsChildControl = true;
            UCConsultations.CS_DS = CS_DS;
            UCePrescriptions.CS_DS = CS_DS;
            UCConsultations.IsInTreatmentProgramChangedCallback = new IsInTreatmentProgramChanged(() =>
            {
                if (UCePrescriptions != null)
                {
                    UCePrescriptions.NotifyViewDataChanged();
                }
            });
        }
        ...
        public IConsultations_V3 UCConsultations { get; set; }
        ...
        private bool _ConsultationOldIsEditingEnable = false;
        public bool ConsultationOldIsEditingEnable
        {
            get
            {
                return _ConsultationOldIsEditingEnable;
            }
            set
            {
                if (_ConsultationOldIsEditingEnable == value)
                {
                    return;
                }
                _ConsultationOldIsEditingEnable = value;
                NotifyOfPropertyChange(() => ConsultationOldIsEditingEnable);
            }
        }
        public bool IsOutPtTreatmentProgram
        {
            get
            {
                return UCConsultations != null && UCConsultations.IsOutPtTreatmentProgram;
            }
            set
            {
                if (UCConsultations == null)
                ...
                UCConsultations.IsOutPtTreatmentProgram = value;
            }
        }
        #endregion
        #region Handles
        ...
        public void Handle(LoadGeneralInfoPageEvent message)
        {
            if (message != null)
            {
                InitConsultationInfo(message.Patient, null);
            }
        }
        ...
        public void InitConsultationInfo(Patient patientInfo, PatientRegistrationDetail PtRegDetail)
        {
            ...
            {
                InitData(patientInfo);
                UCConsultations.InitPatientInfo(PtRegDetail);
                UCePrescriptions.InitPatientInfo();
            }
            NotifyOfPropertyChange(() => CS_DS);
        }
        ...
        public void hpkEditPhysicalExam()
        {
            Action<IcwPhysiscalExam> onInitDlg = delegate (IcwPhysiscalExam proAlloc)
            {
                ...
                {
                    if (PtPhyExamItem.PtRegistrationID != Registration_DataStorage.CurrentPatientRegistration.PtRegistrationID)
                    {
        ...
        {
            UCConsultations.NotifyViewDataChanged();
        }
        ...
        public ObservableCollection<DiagnosisICD9Items> refICD9List
        {
            get
            {
                return UCConsultations.refICD9List;
            }
            set
            {
                UCConsultations.refICD9List = value;
            }
        }
        public long Compare2ICD9List()
        {
            return UCConsultations.Compare2ICD9List();
        }
        ...
        public ObservableCollection<Resources> SelectedResourceList
        {
            get
            {
                return UCConsultations.SelectedResourceList;
            }
            set
            {
                UCConsultations.SelectedResourceList = value;
            }
        }
        //▲===== #004
    }
