...
{
    [Export(typeof(IInstructionList)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class InstructionListViewModel : ViewModelBase, IInstructionList
    {
        ...
        public InstructionListViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
        }
                ...
                using (var mFactory = new ePMRsServiceClient())
                {
                    ...
                    mContract.BeginGetDiagnosisTreatment_InPt_ByPtRegID(PtRegistrationID, null, null, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            ...
                            {
                                DiagnosisTreatmentCollection = mReturnCollection.Where(x=>x.IntPtDiagDrInstructionID != 0).ToObservableCollection();
                            }
                        }
                        catch (Exception ex)
                        {
