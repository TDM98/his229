...
{
    [Export(typeof(IMedCondition)),PartCreationPolicy(CreationPolicy.NonShared)]
    public class MedConditionViewModel : Conductor<object>, IMedCondition
    {
        ...
        public void butSave()
        {
            Globals.EventAggregator.Publish(new PharContraEvent() { refMedicalConditionType_Edit = RefMedicalConditionType_Edit });
            TryClose();
                ...
                using (var serviceFactory = new PharmacyDrugServiceClient())
                {
                    ...
                    contract.BeginDeleteRefMedicalConditions(MCID, MCTypeID, Globals.LoggedUserAccount.StaffID.Value, Globals.DispatchCallback((asyncResult) =>
                    {
                        ...
                        try
                        {
                            ...
                            //    Globals.ShowMessage("Xóa thành công!", "");
                            //}
                        }
                        catch (Exception ex)
                        {
