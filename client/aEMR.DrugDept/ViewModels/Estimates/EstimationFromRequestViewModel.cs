...
/*
* 20230104 #001 QTD:  Tạo màn hình dự trù mới
* 20230630 #002 QTD:  Thêm báo cáo dự trù tổng hợp theo khoa/phòng
*/
namespace aEMR.DrugDept.ViewModels
{
    [Export(typeof(IEstimationFromRequest)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class EstimationFromRequestViewModel : ViewModelBase, IEstimationFromRequest
        , IHandle<DrugDeptCloseSearchEstimationEvent>
    {
        ...
        public EstimationFromRequestViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            Authorization();
                        ...
                        contract.BeginDrugDeptEstimationForPO_CheckExists(V_EstimateType, DateOfEstimation, V_MedProductType, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    GetEstimationDrugDept_FromRequest();
        ...
        public void btnOK(object sender, RoutedEventArgs e)
        {
            ...
            {
                GetEstimationDrugDept_FromRequest();
                //if (((EstimationFromRequestViewModel)GetView()).dpkEstimateDate.SelectedDate != null)
                //{
        ...
        private void DrugDeptEstimationForPODelete(long DrugDeptEstimatePoID)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginDrugDeptEstimationForPO_Delete(DrugDeptEstimatePoID, true, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
        ...
        public void btnPreview()
        {
            void onInitDlg(IDrugDeptReportDocumentPreview proAlloc)
            {
                ...
                {
                    proAlloc.LyDo = "TỔNG HỢP DỰ TRÙ VĂN PHÒNG PHẨM";
                }
                else if (V_MedProductType == (long)AllLookupValues.MedProductType.VATTU_TIEUHAO)
                {
                    proAlloc.LyDo = "TỔNG HỢP DỰ TRÙ VẬT TƯ TIÊU HAO";
                }
                else
                {
                ...
                proAlloc.EstimationFromRequest = true;
            }
        ...
        private void GetEstimationDrugDept_FromRequest()
        {
            this.ShowBusyIndicator(eHCMSResources.K2887_G1_DangXuLy);
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetEstimation_V2(V_MedProductType, FromDate, ToDate, CurrentDrugDeptEstimationForPO, false, true
        ...
        public void btnPrintByDept()
        {
            void onInitDlg(IDrugDeptReportDocumentPreview proAlloc)
            {
                ...
                proAlloc.eItem = ReportName.DRUGDEPT_ESTIMATION_FOR_DEPT;
                proAlloc.StaffFullName = GetStaffLogin().FullName;
                proAlloc.EstimationFromRequest = true;
            }
            GlobalsNAV.ShowDialog<IDrugDeptReportDocumentPreview>(onInitDlg);
