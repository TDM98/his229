...
{
    [Export(typeof(IEstimationDrugDeptByBid)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class EstimationDrugDeptByBidViewModel : ViewModelBase, IEstimationDrugDeptByBid
        , IHandle<DrugDeptCloseSearchEstimationEvent>
    {
        ...
        public EstimationDrugDeptByBidViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            Authorization();
            ...
            var t = new Thread(() =>
            {
                try
                {
                    using (var serviceFactory = new PharmacyEstimattionServiceClient())
                    {
                        ...
                        contract.BeginDrugDeptEstimationForPO_FullOperator_ByBid(V_MedProductType, CurrentDrugDeptEstimationForPO, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                DrugDeptEstimationForPO EstimateOut;
                                long results = contract.EndDrugDeptEstimationForPO_FullOperator_ByBid(out EstimateOut, asyncResult);
                                EstimationDetailsDeleted.Clear();
                                SetRadioButtonCompleted();
                                if (results >= 0)
                                {
        ...
        private void DeleteEstimationDetails(object item)
        {
            if (CurrentDrugDeptEstimationForPO.EstimationDetails != null)
            {
                ...
                if (CurrentDrugDeptEstimationForPO.EstimationDetails.Any(x => x != p && x.GenMedProductID == p.GenMedProductID))
                {
                    var CurrentExistsItem = CurrentDrugDeptEstimationForPO.EstimationDetails.First(x => x != p && x.GenMedProductID == p.GenMedProductID);
                    AddEstimatedQtyFromDeleteItem(CurrentExistsItem, p);
                }
                CurrentDrugDeptEstimationForPO.EstimationDetails.Remove(p);
                LoadDataGrid();
            }
        ...
        private void DrugDeptEstimationForPoDetail_ByParentID(long DrugDeptEstimateID)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginDrugDeptEstimationForPoDetail_ByParentIDAndByBid(DrugDeptEstimateID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndDrugDeptEstimationForPoDetail_ByParentIDAndByBid(asyncResult);
        ...
        private void GetEstimationForMonthDrugDept()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetEstimationForMonthByBid(V_MedProductType, CurrentDrugDeptEstimationForPO, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndGetEstimationForMonthByBid(asyncResult);
                                CurrentDrugDeptEstimationForPO.EstimationDetails = results.ToObservableCollection();
                                if (CurrentDrugDeptEstimationForPO.EstimationDetails != null)
                                {
                                    for (int i = 0; i < CurrentDrugDeptEstimationForPO.EstimationDetails.Count; i++)
        ...
        {
            if (CurrentDrugDeptEstimationForPO != null && CurrentDrugDeptEstimationForPO.DateOfEstimation != null)
            {
        ...
        public void btnMergePreview()
        {
            ...
            {
                //▼======= #002
                proAlloc.ID = this.CurrentDrugDeptEstimationForPO.DrugDeptEstimatePoID;
        ...
        private void GetRefGenericDrugDetail_Auto(string BrandName, int PageIndex, int PageSize)
        {
            ...
            var t = new Thread(() =>
            {
                ...
                {
                    var contract = serviceFactory.ServiceInstance;
                    contract.BeginRefGenMedProductDetails_SearchAutoPaging(false, BrandName, null, V_MedProductType
                        , RefGenDrugCatID_1, PageSize, PageIndex, null
                ...
                if (items != null && items.Count() > 0)
                {
                    if (MessageBox.Show(eHCMSResources.Z0565_G1_CoChacXoaHangDaChon, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                    {
                        foreach (DrugDeptEstimationForPoDetail p in items)
                        {
                            ...
                            if (CurrentDrugDeptEstimationForPO.EstimationDetails.Any(x => !items.Contains(x) && x.GenMedProductID == p.GenMedProductID))
                            {
                                var CurrentExistsItem = CurrentDrugDeptEstimationForPO.EstimationDetails.First(x => !items.Contains(x) && x.GenMedProductID == p.GenMedProductID);
                                AddEstimatedQtyFromDeleteItem(CurrentExistsItem, p);
                            }
                        }
        ...
        private void AddEstimatedQtyFromDeleteItem(DrugDeptEstimationForPoDetail CurrentExistsItem, DrugDeptEstimationForPoDetail RemoveItem)
        {
            CurrentExistsItem.OutQtyPrevFirstMonth += RemoveItem.OutQtyPrevFirstMonth;
            CurrentExistsItem.OutQtyPrevSecondMonth += RemoveItem.OutQtyPrevSecondMonth;
            CurrentExistsItem.OutQtyPrevThirdMonth += RemoveItem.OutQtyPrevThirdMonth;
            CurrentExistsItem.OutQtyPrevFourthMonth += RemoveItem.OutQtyPrevFourthMonth;
            CurrentExistsItem.RemainQty += RemoveItem.RemainQty;
            CurrentExistsItem.EstimatedQty_F += RemoveItem.EstimatedQty_F;
            CurrentExistsItem.OutAverageQty += RemoveItem.OutAverageQty;
            CurrentExistsItem.Priority += RemoveItem.Priority;
            CurrentExistsItem.OutQtyLastTwelveMonths += RemoveItem.OutQtyLastTwelveMonths;
            CurrentExistsItem.ToDateOutQty += RemoveItem.ToDateOutQty;
            CurrentExistsItem.QtyPackaging += RemoveItem.QtyPackaging;
            CurrentExistsItem.AdjustedQty += RemoveItem.AdjustedQty;
            CurrentExistsItem.TotalPrice += RemoveItem.TotalPrice;
        }
