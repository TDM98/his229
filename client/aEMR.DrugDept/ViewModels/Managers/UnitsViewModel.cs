...
{
    [Export(typeof(IDrugDeptUnits)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class UnitsViewModel : Conductor<object>, IDrugDeptUnits
    {
        ...
        public void authorization()
        {
            ...
            bTim = Globals.CheckAuthorization(Globals.listRefModule, (int)eModules.mKhoaDuoc
                                               , (int)eKhoaDuoc.mQuanLyDonViTinh,
                                               (int)oKhoaDuocEx.mQuanLyDonViTinh_Tim, (int)ePermission.mView);
            bThem = Globals.CheckAuthorization(Globals.listRefModule, (int)eModules.mKhoaDuoc
                                               , (int)eKhoaDuoc.mQuanLyDonViTinh,
                                               (int)oKhoaDuocEx.mQuanLyDonViTinh_Them, (int)ePermission.mView);
            bChinhSua = Globals.CheckAuthorization(Globals.listRefModule, (int)eModules.mKhoaDuoc
                                               , (int)eKhoaDuoc.mQuanLyDonViTinh,
                                               (int)oKhoaDuocEx.mQuanLyDonViTinh_ChinhSua, (int)ePermission.mView);
            ...
            var t = new Thread(() =>
                                   {
                                       using (var serviceFactory = new PharmacyUnitsServiceClient())
                                       {
                                           ...
                                           contract.BeginAddNewDrugDeptUnit(SelectedUnit, Globals.DispatchCallback((asyncResult) =>
                                           {

                                               try
                                               {
                                                   contract.EndAddNewDrugDeptUnit(asyncResult);
        ...
        public void UpdateUnit(RefUnit SelectedUnit)
        {
            ...
            var t = new Thread(() =>
            {
                using (var serviceFactory = new PharmacyUnitsServiceClient())
                {
                    ...
                    contract.BeginUpdateDrugDeptUnit(SelectedUnit, Globals.DispatchCallback((asyncResult) =>
                    {

                        try
                        {
                            contract.EndUpdateDrugDeptUnit(asyncResult);
        ...
        public void DeleteUnit(long UnitID)
        {
            ...
            var t = new Thread(() =>
            {
                using (var serviceFactory = new PharmacyUnitsServiceClient())
                {
                    ...
                    contract.BeginDeleteDrugDeptUnitByID(UnitID, Globals.LoggedUserAccount.StaffID, Globals.DispatchCallback((asyncResult) =>
                    {

                        try
                        {
                            contract.EndDeleteDrugDeptUnitByID(asyncResult);
        ...
        public void SearchUnits(int PageIndex, int PageSize)
        {
            ...
            var t = new Thread(() =>
            {
                ...
                {
                    var contract = serviceFactory.ServiceInstance;
                    contract.BeginSearchDrugDeptUnit(UnitName, PageIndex, PageSize, true, Globals.DispatchCallback((asyncResult) =>
                    {

                        try
                        {
                            var results = contract.EndSearchDrugDeptUnit(out totalCount, asyncResult);
                            if (results != null)
                            {
                                Units.Clear();
        ...
        {
            if (MessageBox.Show(string.Format(eHCMSResources.Z0476_G1_BanMuonXoa, eHCMSResources.K3966_G1_Dvt), eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
            {
