...
{
    [Export(typeof(IRefUnitsUse_SearchPaging)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class RefUnitsUse_SearchPagingViewModel : Conductor<object>, IRefUnitsUse_SearchPaging
    {
        ...
        protected override void OnViewLoaded(object view)
        {
            ...
            _currentView = view as IRefUnitsUse_SearchPagingView;
        }
        IRefUnitsUse_SearchPagingView _currentView;

        [ImportingConstructor]
        public RefUnitsUse_SearchPagingViewModel(IWindsorContainer container, INavigationService navigationService, ISalePosCaching salePosCaching)
        {
                ...
                try
                {
                    using (var serviceFactory = new PharmacyGenMedDeptDetailsServiceClient())
                    {
                        ...
                        client.BeginRefUnits_SearchPaging(criteria, pageIndex, pageSize, bCountTotal, Globals.DispatchCallback((asyncResult) =>
                        {
                            ...
                            //var view = this.GetView() as IRefUnitsUse_SearchPagingView;
        ...
        public void SelectionChangedCmd(object sender, SelectionChangedEventArgs e)
        {
            if (e.AddedItems != null && e.AddedItems.Count > 0)
            {
                ...
                Globals.EventAggregator.Publish(new RefUnitsUse_SearchPagingView_Selected_Event() { ObjectSelected = ObjRefUnits_Selected });
            }
            else
            {
                Globals.EventAggregator.Publish(new RefUnitsUse_SearchPagingView_Selected_Event() { ObjectSelected = null });
            }
        }
