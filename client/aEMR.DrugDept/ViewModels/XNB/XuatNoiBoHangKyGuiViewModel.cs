 ...
 * 20221111 #013 QTD:   Lọc kho cho màn hình xuất trả hàng ký gửi
 * 20221115 #014 QTD:   Cho chọn loại xuất và chỉ lấy Xuất nội bộ + Xuất huỷ hàng cho màn hình xuất hàng ký gửi
 */
namespace aEMR.DrugDept.ViewModels
{
    [Export(typeof(IXuatNoiBoHangKyGui)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class XuatNoiBoHangKyGuiViewModel : ViewModelBase, IXuatNoiBoHangKyGui, IHandle<DrugDeptCloseSearchRequestEvent>
        ...
        , IHandle<DrugDeptCloseSearchRequestForHIStoreEvent>
    {
        ...
        public XuatNoiBoHangKyGuiViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            authorization();
        ...
        private void RefeshData()
        {
            ...
            ClearData();
            GetInStoreBySelectedStored(null);
            TotalPrice = 0;
            TotalPriceNotVAT = 0;
        ...
        private void SetDefaultForStore()
        {
            if (StoreCbx != null && StoreCbx.Count > 0)
            {
                ...
                GetInStoreBySelectedStored(selectedStore);
            }
        }
        ...
        private IEnumerator<IResult> DoGetStoreToSell()
        {
            ...
            var StoreTemp = paymentTypeTask.LookupList.Where(x => V_MedProductType != 0
                                                            ...
                                                            //▼===== #013
                                                            && ((!IsReturn && x.IsConsignment)|| (IsReturn && x.IsMain && !x.IsConsignment && x.IsConsignmentReturn))).ToObservableCollection();
                                                            //▲===== #013
            StoreCbx = Globals.CheckDrugMedStoreWareHouse(StoreTemp);

            var InStoreTemp = paymentTypeTask.LookupList.Where(x => V_MedProductType != 0
                                                           && x.StoreTypeID == (long)AllLookupValues.StoreType.STORAGE_DRUGDEPT
                                                           && x.ListV_MedProductType != null
                                                           && x.ListV_MedProductType.Contains(V_MedProductType.ToString())
                                                           //▼===== #013
                                                           && ((!IsReturn &&  !x.IsConsignment) || (IsReturn && x.IsConsignment))).ToObservableCollection();
                                                            //▲===== #013
            InStoreCbx = Globals.CheckDrugMedStoreWareHouse(InStoreTemp);
            if (StoreCbx == null || StoreCbx.Count <= 0)
            {
        ...
        private void ListDisplayAutoComplete()
        {
            var hhh = from hd in RefGenMedProductDetailsList
                      group hd by new { hd.GenMedProductID, hd.BrandName, hd.SelectedUnit.UnitName, hd.RequestQty, hd.Code, hd.ProductCodeRefNum, hd.inviID , hd.InvID } into hdgroup
                      ...
                      select new
                      {
                          ...
                          inviID = hdgroup.Key.inviID,
                          InvID = hdgroup.Key.InvID
                      };
        ...
        private void SearchRefGenMedProductDetails(string Name, bool? IsCode)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetDrugForSellVisitorAutoComplete_ForRequestDrugDept_ForConsignment(IsSearchByGenericName, IsCost, Name, StoreID
                            ...
                            , Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndGetDrugForSellVisitorAutoComplete_ForRequestDrugDept_ForConsignment(asyncResult);
        ...
        {
            if (!CheckDrugInv(value))
            {
                MessageBox.Show("Chỉ chọn được thuốc/VT cùng phiếu nhập NCC");
                return;
            }
        ...
        private void GetInStoreBySelectedStored(RefStorageWarehouseLocation selectedStore)
        {
            // 20220617 BLQ Gán kho nhận theo kho xuất
            if (StoreCbx != null && StoreCbx.Count > 0)
            {
                string tempListV_MedProductType = selectedStore == null ? StoreCbx.Where(x => x.StoreID == StoreID).FirstOrDefault().ListV_MedProductType : selectedStore.ListV_MedProductType;
                bool tempIsVATCreditStorage = selectedStore == null ? StoreCbx.Where(x => x.StoreID == StoreID).FirstOrDefault().IsVATCreditStorage : selectedStore.IsVATCreditStorage;
                long tempV_GroupTypes = selectedStore == null ? StoreCbx.Where(x => x.StoreID == StoreID).FirstOrDefault().V_GroupTypes : selectedStore.V_GroupTypes;
                if (InStoreCbx != null && InStoreCbx.Where(x => x.V_GroupTypes == tempV_GroupTypes).Count() > 0)
                {
                    RefStorageWarehouseLocation inSelectedStored = InStoreCbx.Where(x => x.V_GroupTypes == tempV_GroupTypes).FirstOrDefault();
                    SelectedOutInvoice.OutputToID = inSelectedStored.StoreID;
                    SelectedOutInvoice.FullName = inSelectedStored.swhlName;
                    SelectedOutInvoice.Address = "";
                    SelectedOutInvoice.NumberPhone = "";
                    //▼====: #001
                    if (Globals.ServerConfigSection.CommonItems.EnableHIStore)
                    {
                        OutputToStoreID = inSelectedStored.StoreID;
                    }
                    else
                    {
                ...
                {
                    MessageBox.Show("Không tìm thấy kho nhận.");
                }
            }
        }
        ...
        public void LoadRefOutputType()
        {
            if (!IsBalanceView)
            {
                ...
                RefOutputTypes = Globals.RefOutputType.Where(x => x.IsSelected == true 
                    && (x.TypID == (long)AllLookupValues.RefOutputType.XUATNOIBO || x.TypID == (long)AllLookupValues.RefOutputType.HUYHANG)).ToObservableCollection();
            }
            else
            {
        ...
        //▲====: #010
        private bool CheckDrugInv(RefGenMedProductDetails productDetails)
        {
            if (SelectedOutInvoice.OutwardDrugMedDepts != null && SelectedOutInvoice.OutwardDrugMedDepts.Count > 0)
            {
                foreach (var item in SelectedOutInvoice.OutwardDrugMedDepts)
                {
                    if(productDetails.inviID != item.RefGenericDrugDetail.inviID)
                    {
                        return false;
                    }
                }
        ...
        public bool IsReturn
        {
            ...
            set
            {
                ...
                {
                    _IsReturn = value;
                    NotifyOfPropertyChange(() => IsReturn);
                }
            }
        }
    }
