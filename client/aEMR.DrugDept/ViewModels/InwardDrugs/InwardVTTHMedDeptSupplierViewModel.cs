...
namespace aEMR.DrugDept.ViewModels
{
    ...
    public class InwardVTTHMedDeptSupplierViewModel : ViewModelBase, IVTTHMedDeptInwardSupplier
        ...
        , IHandle<DrugDeptCloseEditInwardEvent>
    {
        ...
        public InwardVTTHMedDeptSupplierViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventAggregator, ISalePosCaching salePosCaching)
        {
            ...
            InwardVTTHMedDeptList = new PagedSortableCollectionView<InwardDrugMedDept>();
            InwardVTTHMedDeptList.OnRefresh += new EventHandler<RefreshEventArgs>(InwardVTTHMedDeptList_OnRefresh);
            InwardVTTHMedDeptList.PageSize = Globals.PageSize;
        ...
        void InwardVTTHMedDeptList_OnRefresh(object sender, RefreshEventArgs e)
        {
            InwardVTTHDetails_ByID(inviID, InwardVTTHMedDeptList.PageIndex, InwardVTTHMedDeptList.PageSize);
        }
        ...
        private InwardDrugMedDeptInvoice _CurrentInwardVTTHMedDeptInvoice;
        public InwardDrugMedDeptInvoice CurrentInwardVTTHMedDeptInvoice
        {
            get
            {
                return _CurrentInwardVTTHMedDeptInvoice;
            }
            set
            {
                if (_CurrentInwardVTTHMedDeptInvoice != value)
                {
                    _CurrentInwardVTTHMedDeptInvoice = value;
                    ...
                    NotifyOfPropertyChange(() => CurrentInwardVTTHMedDeptInvoice);
                }
            }
        }
        ...
        private PagedSortableCollectionView<InwardDrugMedDept> _InwardVTTHMedDeptList;
        public PagedSortableCollectionView<InwardDrugMedDept> InwardVTTHMedDeptList
        {
            get
            {
                return _InwardVTTHMedDeptList;
            }
            set
            {
                if (_InwardVTTHMedDeptList != value)
                {
                    _InwardVTTHMedDeptList = value;
                    NotifyOfPropertyChange(() => InwardVTTHMedDeptList);
                }
            }
        }
        ...
        private InwardDrugMedDept _CurrentInwardVTTHMedDept;
        public InwardDrugMedDept CurrentInwardVTTHMedDept
        {
            get
            {
                return _CurrentInwardVTTHMedDept;
            }
            set
            {
                if (_CurrentInwardVTTHMedDept != value)
                {
                    _CurrentInwardVTTHMedDept = value;
                    NotifyOfPropertyChange(() => CurrentInwardVTTHMedDept);
                }
            }
        }
                    ...
                    using (var serviceFactory = new CommonService_V2Client())
                    {
                        ...
                        contract.BeginGetAllCurrency(value, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                ...
                                CurrentInwardVTTHMedDeptInvoice.SelectedCurrency = Currencies.FirstOrDefault();
                            }
                            catch (Exception ex)
                            {
        ...
        private IEnumerator<IResult> DoGetStore_MedDept()
        {
            ...
            {
                CurrentInwardVTTHMedDeptInvoice.SelectedStorage = StoreCbx.FirstOrDefault();
            }
            else
            {
        ...
        {
            CurrentInwardVTTHMedDeptInvoice = new InwardDrugMedDeptInvoice
            {
            ...
            CurrentInwardVTTHMedDeptInvoice.SelectedCurrency.CurrencyID = (long)AllLookupValues.CurrencyTable.VND;
            ...
            IsHideAuSupplier = true;
            if (InwardVTTHMedDeptList != null)
            {
                InwardVTTHMedDeptList.Clear();
            }
            DeepCopyInvoice();
            InitDetailInward();
            TotalPercent = 0;
        ...
        {
            CurrentInwardInvoiceCopy = CurrentInwardVTTHMedDeptInvoice.DeepCopy();
        }
        ...
        private void DeleteInwardInvoice()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginDeleteInwardVTTHMedDeptInvoice(CurrentInwardVTTHMedDeptInvoice.inviID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndDeleteInwardVTTHMedDeptInvoice(asyncResult);
                                if (results == 0)
                                {
                                    InitializeInvoice();
        ...
        {
            if (CurrentInwardVTTHMedDeptInvoice.SelectedCurrency == null || CurrentInwardVTTHMedDeptInvoice.SelectedCurrency.CurrencyID == 0)
            {
            ...
            if (CurrentInwardVTTHMedDeptInvoice.SelectedCurrency.CurrencyID != (long)AllLookupValues.CurrencyTable.VND)
            {
                if (CurrentInwardVTTHMedDeptInvoice.ExchangeRates <= 0)
                {
            ...
            this.ShowBusyIndicator(eHCMSResources.Z0125_G1_DangXuLi);
            CurrentInwardVTTHMedDeptInvoice.TypID = TypID;
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginUpdateInwardVTTHMedDeptInvoice(CurrentInwardVTTHMedDeptInvoice, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndUpdateInwardVTTHMedDeptInvoice(asyncResult);
                                if (results == 0)
                                {
                                    DeepCopyInvoice();
                                    CheckIsPercentAll();
                                    Load_InwardVTTHDetails(CurrentInwardVTTHMedDeptInvoice.inviID);
                                    InwardVTTHMedDeptInvoice_GetListCost(CurrentInwardVTTHMedDeptInvoice.inviID);
        ...
        public void BtnEdit(object sender, RoutedEventArgs e)
        {
            if (MessageBox.Show(string.Format(eHCMSResources.Z0583_G1_CoMuonChinhSuaKg, eHCMSResources.Z0578_G1_PhNhap), eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
            {
                if (CheckValidate())
                {
                    //▼===== #001
                    string ErrorMsg = "";
                    if (!Globals.CheckLimitCharaterOfInwardInvoice(out ErrorMsg, CurrentInwardVTTHMedDeptInvoice.InvInvoiceNumber, CurrentInwardVTTHMedDeptInvoice.SerialNumber))
                    {
                        MessageBox.Show(ErrorMsg, eHCMSResources.G0449_G1_TBaoLoi);
                        return;
                    }
                    //▲===== #001
                    if (CurrentInwardVTTHMedDeptInvoice.SelectedStorage != null)
                    {
                        CurrentInwardVTTHMedDeptInvoice.StoreID = CurrentInwardVTTHMedDeptInvoice.SelectedStorage.StoreID;
                    }
                    if (CurrentInwardVTTHMedDeptInvoice.SelectedCurrency != null)
                    {
                        CurrentInwardVTTHMedDeptInvoice.CurrencyID = CurrentInwardVTTHMedDeptInvoice.SelectedCurrency.CurrencyID;
                    }
        ...
        {
            CurrentInwardVTTHMedDeptInvoice.TotalPriceNotVAT = TongTienSPChuaVAT;
            CurrentInwardVTTHMedDeptInvoice.TotalDiscountOnProduct = CKTrenSP;//tong ck tren hoan don
            CurrentInwardVTTHMedDeptInvoice.TotalDiscountInvoice = CKTrenSP + TongCKTrenHoaDon;
            CurrentInwardVTTHMedDeptInvoice.TotalPrice = TongTienSPChuaVAT - (CKTrenSP + TongCKTrenHoaDon);
            CurrentInwardVTTHMedDeptInvoice.TotalHaveCustomTax = TongTienHoaDonCoThueNK;
            CurrentInwardVTTHMedDeptInvoice.TotalPriceVAT = TongTienHoaDonCoVAT;
            CurrentInwardVTTHMedDeptInvoice.TotalVATDifferenceAmount = TotalVATDifferenceAmount;
            CurrentInwardVTTHMedDeptInvoice.TotalVATAmountActual = CurrentInwardVTTHMedDeptInvoice.TotalVATAmount + CurrentInwardVTTHMedDeptInvoice.TotalVATDifferenceAmount;
        }

        private void Load_InwardVTTHDetails(long ID)
        {
            inviID = ID;
            InwardVTTHMedDeptList.PageIndex = 0;
            InwardVTTHDetails_ByID(CurrentInwardVTTHMedDeptInvoice.inviID, 0, Globals.PageSize);
        }

        private void InwardVTTHDetails_ByID(long ID, int PageIndex, int PageSize)
        {
            this.ShowBusyIndicator(eHCMSResources.Z0125_G1_DangXuLi);
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetInwardVTTHMedDept_ByIDInvoice(ID, PageSize, PageIndex, true, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndGetInwardVTTHMedDept_ByIDInvoice(out int total, out decimal TongTienSPChuaVAT, out decimal CKTrenSP
                                ...
                                InwardVTTHMedDeptList.Clear();
                                InwardVTTHMedDeptList.TotalItemCount = total;
                                if (results != null)
                                {
                                    foreach (InwardDrugMedDept p in results)
                                    {
                                        InwardVTTHMedDeptList.Add(p);
                                    }
                                }
        ...
        {
            if (CurrentInwardVTTHMedDeptInvoice.SelectedSupplier != null)
            {
                PharmacyPurchaseOrder_BySupplierID(CurrentInwardVTTHMedDeptInvoice.SelectedSupplier.SupplierID);
            }
            else
            {
        ...
        private void GetInwardInvoiceDrugByID(long ID)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetInwardVTTHMedDeptInvoice_ByID(ID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                CurrentInwardVTTHMedDeptInvoice = contract.EndGetInwardVTTHMedDeptInvoice_ByID(asyncResult);
                                DeepCopyInvoice();
                            }
                            catch (Exception ex)
                            {
        ...
        private void AddInwardInvoiceDrug()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginAddInwardVTTHMedDeptInvoice(CurrentInwardVTTHMedDeptInvoice, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndAddInwardVTTHMedDeptInvoice(out long ID, asyncResult);
                                if (results == 0)
                                {
        ...
        {
            if (CurrentInwardVTTHMedDeptInvoice == null)
            {
                return false;
            }
            return CurrentInwardVTTHMedDeptInvoice.Validate();
        }
        ...
        public void BtnAdd(object sender, RoutedEventArgs e)
        {
            if (CheckValidate())
            {
                ...
                if (CurrentInwardVTTHMedDeptInvoice.VAT < 1 && CurrentInwardVTTHMedDeptInvoice.VAT > 0)
                {
                    Globals.ShowMessage(eHCMSResources.K0262_G1_VATKhongHopLe, eHCMSResources.G0442_G1_TBao);
                    return;
                }
                if (CurrentInwardVTTHMedDeptInvoice.DiscountingByPercent < 1 && CurrentInwardVTTHMedDeptInvoice.DiscountingByPercent > 0)
                {
                ...
                CurrentInwardVTTHMedDeptInvoice.CurrencyID = CurrentInwardVTTHMedDeptInvoice.SelectedCurrency.CurrencyID;
                CurrentInwardVTTHMedDeptInvoice.V_MedProductType = V_MedProductType;
        ...
        {
            if (CurrentInwardVTTHMedDept == null || CurrentInwardVTTHMedDept.GenMedProductID == null || CurrentInwardVTTHMedDept.GenMedProductID <= 0)
            {
                return;
            }

            GetRefGenMedProductDetails(CurrentInwardVTTHMedDept.GenMedProductID.GetValueOrDefault(), CurrentInwardVTTHMedDept.GenMedVersionID);
        }
        ...
        {
            CurrentInwardVTTHMedDeptInvoice.IsForeign = false;
        }
        public void rdtNgoaiNuoc_Checked(object sender, RoutedEventArgs e)
        {
            CurrentInwardVTTHMedDeptInvoice.IsForeign = true;
        }
        ...
        {
            if (CurrentInwardVTTHMedDeptInvoice == null || CurrentInwardVTTHMedDeptInvoice.SelectedSupplier == null || CurrentInwardInvoiceCopy == null || CurrentInwardInvoiceCopy.SelectedSupplier == null)
            {
                MessageBox.Show(string.Format("{0}!", eHCMSResources.Z0585_G1_PhNhapKgDuTTin), eHCMSResources.G0442_G1_TBao, MessageBoxButton.OK);
                return false;
            }

            if (CurrentInwardVTTHMedDeptInvoice.SelectedSupplier.SupplierID != CurrentInwardInvoiceCopy.SelectedSupplier.SupplierID)
            {
                MessageBox.Show(string.Format("{0}!", eHCMSResources.Z0586_G1_NCCDaThayDoi), eHCMSResources.G0442_G1_TBao, MessageBoxButton.OK);
                return false;
            }

            if (CurrentInwardVTTHMedDeptInvoice.CurrencyID != CurrentInwardInvoiceCopy.CurrencyID || CurrentInwardVTTHMedDeptInvoice.ExchangeRates != CurrentInwardInvoiceCopy.ExchangeRates || CurrentInwardVTTHMedDeptInvoice.VAT != CurrentInwardInvoiceCopy.VAT || CurrentInwardVTTHMedDeptInvoice.IsForeign != CurrentInwardInvoiceCopy.IsForeign || CurrentInwardVTTHMedDeptInvoice.Notes != CurrentInwardInvoiceCopy.Notes)
            {
                if (MessageBox.Show(eHCMSResources.Z0587_G1_CoMuonCNhatHD, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    ...
                    {
                        CurrentInwardVTTHMedDeptInvoice = CurrentInwardInvoiceCopy;
                    }
                }
                else
                {
        ...
        private void InwardDrug_InsertList()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginInwardVTTHMedDept_InsertList(DrugDeptPurchaseOrderDetailList.ToList(), CurrentInwardVTTHMedDeptInvoice.inviID, DrugDeptPoID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                contract.EndInwardVTTHMedDept_InsertList(asyncResult);
        ...
        private void AddBlankRow()
        {
            if (CheckBlankRow())
            {
                ...
                //    p.VAT = CurrentInwardVTTHMedDeptInvoice.VAT - 1;
                //}
        ...
        {
            long? SupplierID = 0;
            if (CurrentInwardVTTHMedDeptInvoice != null && CurrentInwardVTTHMedDeptInvoice.SelectedSupplier != null)
            {
                SupplierID = CurrentInwardVTTHMedDeptInvoice.SelectedSupplier.SupplierID;
            }
            int totalCount = 0;
            var t = new Thread(() =>
            {
        ...
        private void DeleteInwardDrug(long InID)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginDeleteInwardVTTHMedDept(InID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndDeleteInwardVTTHMedDept(asyncResult);
                                if (results == 0)
                                {
                                    Load_InwardVTTHDetails(CurrentInwardVTTHMedDeptInvoice.inviID);
                                    InwardVTTHMedDeptInvoice_GetListCost(CurrentInwardVTTHMedDeptInvoice.inviID);
        ...
        {
            if (CurrentInwardVTTHMedDeptInvoice.CanEditAndDelete)
            {
                if (MessageBox.Show(eHCMSResources.A0118_G1_Msg_ConfXoaDong, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    if (CurrentInwardVTTHMedDept != null)
                    {
                        DeleteInwardDrug(CurrentInwardVTTHMedDept.InID);
                    }
                }
            }
            else
        ...
        {
            CurrentInwardDrugMedDeptCopy = CurrentInwardVTTHMedDept.DeepCopy();
        }

        public void lnkEdit_Click(object sender, RoutedEventArgs e)
        {
            if (CurrentInwardVTTHMedDeptInvoice.DrugDeptSupplierPaymentReqID != null && CurrentInwardVTTHMedDeptInvoice.DrugDeptSupplierPaymentReqID > 0)
            {
            ...
            {
                if (CurrentInwardVTTHMedDeptInvoice.SelectedCurrency != null && CurrentInwardVTTHMedDeptInvoice.SelectedCurrency.CurrencyID != (long)AllLookupValues.CurrencyTable.VND)
                {
                ...
                proAlloc.CurrentInwardDrugMedDeptCopy = CurrentInwardVTTHMedDept.DeepCopy();
                proAlloc.SetValueForProperty();

                if (CurrentInwardVTTHMedDeptInvoice.CheckedPoint)
                {
        ...
        private void CheckEnableSupplier()
        {
            if ((DrugDeptPurchaseOrderDetailList != null && DrugDeptPurchaseOrderDetailList.Count > 0
                ...
                || (InwardVTTHMedDeptList != null && InwardVTTHMedDeptList.Count > 0))
            {
        ...
        public void OpenPopUpSearchInwardInvoice(IList<InwardDrugMedDeptInvoice> results, int Totalcount)
        {
            ...
            void onInitDlg(IDrugDeptInwardDrugSupplierSearch proAlloc)
            {
                ...
                {
                    proAlloc.strHienThi = eHCMSResources.Z2521_G1_VTTH.ToUpper();
                }
                else
                {
        ...
        private void SearchInwardInvoiceVTTH(int PageIndex, int PageSize)
        {
            if (SearchCriteria == null)
            {
                return;
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginSearchInwardVTTHMedDeptInvoice(SearchCriteria, TypID, V_MedProductType, PageIndex, PageSize, true, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndSearchInwardVTTHMedDeptInvoice(out int Totalcount, asyncResult);
                                if (results != null && results.Count > 0)
                                {
                                    if (results.Count > 1)
                                    {
                                    ...
                                    {
                                        CurrentInwardVTTHMedDeptInvoice = results.FirstOrDefault();
                                        InitDetailInward();
                                        DeepCopyInvoice();
        ...
        public void tbx_Search_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                SearchInwardInvoiceVTTH(0, Globals.PageSize);
            }
        }
        ...
        public void Handle(DrugDeptCloseSearchSupplierEvent message)
        {
            ...
            {
                CurrentInwardVTTHMedDeptInvoice.SelectedSupplier = message.SelectedSupplier as DrugDeptSupplier;
                PharmacyPurchaseOrder_BySupplierID();
            }
        }
        ...
        public void btnPreview()
        {
            Action<IDrugDeptReportDocumentPreview> onInitDlg = delegate (IDrugDeptReportDocumentPreview proAlloc)
            {
                ...
                if (CurrentInwardVTTHMedDeptInvoice != null && CurrentInwardVTTHMedDeptInvoice.SelectedStaff != null)
                {
                    proAlloc.StaffFullName = CurrentInwardVTTHMedDeptInvoice.SelectedStaff.FullName;
                }
            };
        ...
        public void btnPrint()
        {
            ...
            var t = new Thread(() =>
            {
                using (var serviceFactory = new ReportServiceClient())
                {
                    ...
                    contract.BeginGetInWardVTTHInPdfFormat(CurrentInwardVTTHMedDeptInvoice.inviID, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
        ...
        public void Handle(DrugDeptCloseSearchInwardIncoiceEvent message)
        {
            if (message != null && this.IsActive)
            {
                CurrentInwardVTTHMedDeptInvoice = message.SelectedInwardInvoice as InwardDrugMedDeptInvoice;
                InitDetailInward();
                DeepCopyInvoice();
                LoadInfoThenSelectedInvoice();
        ...
        private void InwardVTTHMedDeptInvoice_GetListCost(long inviID)
        {
            this.ShowBusyIndicator(eHCMSResources.Z0125_G1_DangXuLi);
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginInwardVTTHMedDeptInvoice_GetListCost(inviID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndInwardVTTHMedDeptInvoice_GetListCost(asyncResult);
                                CurrentInwardVTTHMedDeptInvoice.CostTableMedDeptLists = results.ToObservableCollection();
                                SumTotalPercent();
                            }
                            catch (Exception ex)
                            {
        ...
        {
            TotalPercent = 0;
            if (CurrentInwardVTTHMedDeptInvoice != null && CurrentInwardVTTHMedDeptInvoice.CostTableMedDeptLists != null)
            {
                for (int i = 0; i < CurrentInwardVTTHMedDeptInvoice.CostTableMedDeptLists.Count; i++)
                {
                    TotalPercent += CurrentInwardVTTHMedDeptInvoice.CostTableMedDeptLists[i].Rates;
                }
            }
        }
        #endregion
        ...
        public void ActualQD_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                if (CurrentInwardVTTHMedDeptInvoice != null && CurrentInwardVTTHMedDeptInvoice.ExchangeRates != 0)
                {
                    try
                    {
                        CurrentInwardVTTHMedDeptInvoice.TotalPriceActual_QĐ = Convert.ToDecimal((sender as TextBox).Text);
                    }
                    catch
                    {
                        Globals.ShowMessage(eHCMSResources.A0559_G1_Msg_InfoGTriKhHopLe, eHCMSResources.T0074_G1_I);
                    }
                    CurrentInwardVTTHMedDeptInvoice.TotalPriceActual = CurrentInwardVTTHMedDeptInvoice.TotalPriceActual_QĐ / (decimal)CurrentInwardVTTHMedDeptInvoice.ExchangeRates;
                }
            }
        }
        ...
        public void Actual_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                if (CurrentInwardVTTHMedDeptInvoice != null)
                {
                    try
                    {
                        CurrentInwardVTTHMedDeptInvoice.TotalPriceActual = Convert.ToDecimal((sender as TextBox).Text);
                    }
                    catch
                    {
                        Globals.ShowMessage(eHCMSResources.A0559_G1_Msg_InfoGTriKhHopLe, eHCMSResources.T0074_G1_I);
