...
{
    [Export(typeof(IThanhTrungMedDeptInwardSupplier)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class InwardThanhTrungMedDeptSupplierViewModel : ViewModelBase, IThanhTrungMedDeptInwardSupplier
        ...
        , IHandle<DrugDeptCloseEditInwardEvent>
    {
        ...
        public InwardThanhTrungMedDeptSupplierViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventAggregator, ISalePosCaching salePosCaching)
        {
            ...
            InwardThanhTrungMedDeptList = new PagedSortableCollectionView<InwardDrugMedDept>();
            InwardThanhTrungMedDeptList.OnRefresh += new EventHandler<RefreshEventArgs>(InwardThanhTrungMedDeptList_OnRefresh);
            InwardThanhTrungMedDeptList.PageSize = Globals.PageSize;
        ...
        void InwardThanhTrungMedDeptList_OnRefresh(object sender, RefreshEventArgs e)
        {
            InwardThanhTrungDetails_ByID(inviID, InwardThanhTrungMedDeptList.PageIndex, InwardThanhTrungMedDeptList.PageSize);
        }
        ...
        private InwardDrugMedDeptInvoice _CurrentInwardThanhTrungMedDeptInvoice;
        public InwardDrugMedDeptInvoice CurrentInwardThanhTrungMedDeptInvoice
        {
            get
            {
                return _CurrentInwardThanhTrungMedDeptInvoice;
            }
            set
            {
                if (_CurrentInwardThanhTrungMedDeptInvoice != value)
                {
                    _CurrentInwardThanhTrungMedDeptInvoice = value;
                    ...
                    NotifyOfPropertyChange(() => CurrentInwardThanhTrungMedDeptInvoice);
                }
            }
        }
        ...
        private PagedSortableCollectionView<InwardDrugMedDept> _InwardThanhTrungMedDeptList;
        public PagedSortableCollectionView<InwardDrugMedDept> InwardThanhTrungMedDeptList
        {
            get
            {
                return _InwardThanhTrungMedDeptList;
            }
            set
            {
                if (_InwardThanhTrungMedDeptList != value)
                {
                    _InwardThanhTrungMedDeptList = value;
                    NotifyOfPropertyChange(() => InwardThanhTrungMedDeptList);
                }
            }
        }
        ...
        private InwardDrugMedDept _CurrentInwardThanhTrungMedDept;
        public InwardDrugMedDept CurrentInwardThanhTrungMedDept
        {
            get
            {
                return _CurrentInwardThanhTrungMedDept;
            }
            set
            {
                if (_CurrentInwardThanhTrungMedDept != value)
                {
                    _CurrentInwardThanhTrungMedDept = value;
                    NotifyOfPropertyChange(() => CurrentInwardThanhTrungMedDept);
                }
            }
        }
        ...
        private long _V_MedProductType = (long)AllLookupValues.MedProductType.THANHTRUNG;
                    ...
                    using (var serviceFactory = new CommonService_V2Client())
                    {
                        ...
                        contract.BeginGetAllCurrency(value, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                ...
                                CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency = Currencies.FirstOrDefault();
                            }
                            catch (Exception ex)
                            {
        ...
        private IEnumerator<IResult> DoGetStore_MedDept()
        {
            ...
            {
                CurrentInwardThanhTrungMedDeptInvoice.SelectedStorage = StoreCbx.FirstOrDefault();
            }
            else
            {
        ...
        {
            CurrentInwardThanhTrungMedDeptInvoice = new InwardDrugMedDeptInvoice
            {
            ...
            CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency.CurrencyID = (long)AllLookupValues.CurrencyTable.VND;
            ...
            IsHideAuSupplier = true;
            if (InwardThanhTrungMedDeptList != null)
            {
                InwardThanhTrungMedDeptList.Clear();
            }
            DeepCopyInvoice();
            InitDetailInward();
            TotalPercent = 0;
        ...
        {
            CurrentInwardInvoiceCopy = CurrentInwardThanhTrungMedDeptInvoice.DeepCopy();
        }
        ...
        private void DeleteInwardInvoice()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginDeleteInwardThanhTrungMedDeptInvoice(CurrentInwardThanhTrungMedDeptInvoice.inviID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndDeleteInwardThanhTrungMedDeptInvoice(asyncResult);
                                if (results == 0)
                                {
                                    InitializeInvoice();
        ...
        {
            if (CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency == null || CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency.CurrencyID == 0)
            {
            ...
            if (CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency.CurrencyID != (long)AllLookupValues.CurrencyTable.VND)
            {
                if (CurrentInwardThanhTrungMedDeptInvoice.ExchangeRates <= 0)
                {
            ...
            this.ShowBusyIndicator(eHCMSResources.Z0125_G1_DangXuLi);
            CurrentInwardThanhTrungMedDeptInvoice.TypID = TypID;
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginUpdateInwardThanhTrungMedDeptInvoice(CurrentInwardThanhTrungMedDeptInvoice, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndUpdateInwardThanhTrungMedDeptInvoice(asyncResult);
                                if (results == 0)
                                {
                                    DeepCopyInvoice();
                                    CheckIsPercentAll();
                                    Load_InwardThanhTrungDetails(CurrentInwardThanhTrungMedDeptInvoice.inviID);
                                    InwardThanhTrungMedDeptInvoice_GetListCost(CurrentInwardThanhTrungMedDeptInvoice.inviID);
        ...
        public void BtnEdit(object sender, RoutedEventArgs e)
        {
            if (MessageBox.Show(string.Format(eHCMSResources.Z0583_G1_CoMuonChinhSuaKg, eHCMSResources.Z0578_G1_PhNhap), eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
            {
                if (CheckValidate())
                {
                    //▼===== #001
                    string ErrorMsg = "";
                    if (!Globals.CheckLimitCharaterOfInwardInvoice(out ErrorMsg, CurrentInwardThanhTrungMedDeptInvoice.InvInvoiceNumber, CurrentInwardThanhTrungMedDeptInvoice.SerialNumber))
                    {
                        MessageBox.Show(ErrorMsg, eHCMSResources.G0449_G1_TBaoLoi);
                        return;
                    }
                    //▲===== #001
                    if (CurrentInwardThanhTrungMedDeptInvoice.SelectedStorage != null)
                    {
                        CurrentInwardThanhTrungMedDeptInvoice.StoreID = CurrentInwardThanhTrungMedDeptInvoice.SelectedStorage.StoreID;
                    }
                    if (CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency != null)
                    {
                        CurrentInwardThanhTrungMedDeptInvoice.CurrencyID = CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency.CurrencyID;
                    }
        ...
        {
            CurrentInwardThanhTrungMedDeptInvoice.TotalPriceNotVAT = TongTienSPChuaVAT;
            CurrentInwardThanhTrungMedDeptInvoice.TotalDiscountOnProduct = CKTrenSP;//tong ck tren hoan don
            CurrentInwardThanhTrungMedDeptInvoice.TotalDiscountInvoice = CKTrenSP + TongCKTrenHoaDon;
            CurrentInwardThanhTrungMedDeptInvoice.TotalPrice = TongTienSPChuaVAT - (CKTrenSP + TongCKTrenHoaDon);
            CurrentInwardThanhTrungMedDeptInvoice.TotalHaveCustomTax = TongTienHoaDonCoThueNK;
            CurrentInwardThanhTrungMedDeptInvoice.TotalPriceVAT = TongTienHoaDonCoVAT;
            CurrentInwardThanhTrungMedDeptInvoice.TotalVATDifferenceAmount = TotalVATDifferenceAmount;
            CurrentInwardThanhTrungMedDeptInvoice.TotalVATAmountActual = CurrentInwardThanhTrungMedDeptInvoice.TotalVATAmount + CurrentInwardThanhTrungMedDeptInvoice.TotalVATDifferenceAmount;
        }

        private void Load_InwardThanhTrungDetails(long ID)
        {
            inviID = ID;
            InwardThanhTrungMedDeptList.PageIndex = 0;
            InwardThanhTrungDetails_ByID(CurrentInwardThanhTrungMedDeptInvoice.inviID, 0, Globals.PageSize);
        }

        private void InwardThanhTrungDetails_ByID(long ID, int PageIndex, int PageSize)
        {
            this.ShowBusyIndicator(eHCMSResources.Z0125_G1_DangXuLi);
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetInwardThanhTrungMedDept_ByIDInvoice(ID, PageSize, PageIndex, true, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndGetInwardThanhTrungMedDept_ByIDInvoice(out int total, out decimal TongTienSPChuaVAT, out decimal CKTrenSP
                                ...
                                InwardThanhTrungMedDeptList.Clear();
                                InwardThanhTrungMedDeptList.TotalItemCount = total;
                                if (results != null)
                                {
                                    foreach (InwardDrugMedDept p in results)
                                    {
                                        InwardThanhTrungMedDeptList.Add(p);
                                    }
                                }
        ...
        {
            if (CurrentInwardThanhTrungMedDeptInvoice.SelectedSupplier != null)
            {
                PharmacyPurchaseOrder_BySupplierID(CurrentInwardThanhTrungMedDeptInvoice.SelectedSupplier.SupplierID);
            }
            else
            {
        ...
        private void GetInwardInvoiceDrugByID(long ID)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetInwardThanhTrungMedDeptInvoice_ByID(ID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                CurrentInwardThanhTrungMedDeptInvoice = contract.EndGetInwardThanhTrungMedDeptInvoice_ByID(asyncResult);
                                DeepCopyInvoice();
                            }
                            catch (Exception ex)
                            {
        ...
        private void AddInwardInvoiceDrug()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginAddInwardThanhTrungMedDeptInvoice(CurrentInwardThanhTrungMedDeptInvoice, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndAddInwardThanhTrungMedDeptInvoice(out long ID, asyncResult);
                                if (results == 0)
                                {
        ...
        {
            if (CurrentInwardThanhTrungMedDeptInvoice == null)
            {
                return false;
            }
            return CurrentInwardThanhTrungMedDeptInvoice.Validate();
        }
        ...
        public void BtnAdd(object sender, RoutedEventArgs e)
        {
            if (CheckValidate())
            {
                ...
                if (CurrentInwardThanhTrungMedDeptInvoice.VAT < 1 && CurrentInwardThanhTrungMedDeptInvoice.VAT > 0)
                {
                    Globals.ShowMessage(eHCMSResources.K0262_G1_VATKhongHopLe, eHCMSResources.G0442_G1_TBao);
                    return;
                }
                if (CurrentInwardThanhTrungMedDeptInvoice.DiscountingByPercent < 1 && CurrentInwardThanhTrungMedDeptInvoice.DiscountingByPercent > 0)
                {
                ...
                CurrentInwardThanhTrungMedDeptInvoice.CurrencyID = CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency.CurrencyID;
                CurrentInwardThanhTrungMedDeptInvoice.V_MedProductType = V_MedProductType;
        ...
        {
            if (CurrentInwardThanhTrungMedDept == null || CurrentInwardThanhTrungMedDept.GenMedProductID == null || CurrentInwardThanhTrungMedDept.GenMedProductID <= 0)
            {
                return;
            }

            GetRefGenMedProductDetails(CurrentInwardThanhTrungMedDept.GenMedProductID.GetValueOrDefault(), CurrentInwardThanhTrungMedDept.GenMedVersionID);
        }
        ...
        {
            CurrentInwardThanhTrungMedDeptInvoice.IsForeign = false;
        }
        public void rdtNgoaiNuoc_Checked(object sender, RoutedEventArgs e)
        {
            CurrentInwardThanhTrungMedDeptInvoice.IsForeign = true;
        }
        ...
        {
            if (CurrentInwardThanhTrungMedDeptInvoice == null || CurrentInwardThanhTrungMedDeptInvoice.SelectedSupplier == null || CurrentInwardInvoiceCopy == null || CurrentInwardInvoiceCopy.SelectedSupplier == null)
            {
                MessageBox.Show(string.Format("{0}!", eHCMSResources.Z0585_G1_PhNhapKgDuTTin), eHCMSResources.G0442_G1_TBao, MessageBoxButton.OK);
                return false;
            }

            if (CurrentInwardThanhTrungMedDeptInvoice.SelectedSupplier.SupplierID != CurrentInwardInvoiceCopy.SelectedSupplier.SupplierID)
            {
                MessageBox.Show(string.Format("{0}!", eHCMSResources.Z0586_G1_NCCDaThayDoi), eHCMSResources.G0442_G1_TBao, MessageBoxButton.OK);
                return false;
            }

            if (CurrentInwardThanhTrungMedDeptInvoice.CurrencyID != CurrentInwardInvoiceCopy.CurrencyID || CurrentInwardThanhTrungMedDeptInvoice.ExchangeRates != CurrentInwardInvoiceCopy.ExchangeRates || CurrentInwardThanhTrungMedDeptInvoice.VAT != CurrentInwardInvoiceCopy.VAT || CurrentInwardThanhTrungMedDeptInvoice.IsForeign != CurrentInwardInvoiceCopy.IsForeign || CurrentInwardThanhTrungMedDeptInvoice.Notes != CurrentInwardInvoiceCopy.Notes)
            {
                if (MessageBox.Show(eHCMSResources.Z0587_G1_CoMuonCNhatHD, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    ...
                    {
                        CurrentInwardThanhTrungMedDeptInvoice = CurrentInwardInvoiceCopy;
                    }
                }
                else
                {
        ...
        private void InwardDrug_InsertList()
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginInwardThanhTrungMedDept_InsertList(DrugDeptPurchaseOrderDetailList.ToList(), CurrentInwardThanhTrungMedDeptInvoice.inviID, DrugDeptPoID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                contract.EndInwardThanhTrungMedDept_InsertList(asyncResult);
        ...
        private void AddBlankRow()
        {
            if (CheckBlankRow())
            {
                ...
                //    p.VAT = CurrentInwardThanhTrungMedDeptInvoice.VAT - 1;
                //}
        ...
        {
            long? SupplierID = 0;
            if (CurrentInwardThanhTrungMedDeptInvoice != null && CurrentInwardThanhTrungMedDeptInvoice.SelectedSupplier != null)
            {
                SupplierID = CurrentInwardThanhTrungMedDeptInvoice.SelectedSupplier.SupplierID;
            }
            int totalCount = 0;
            var t = new Thread(() =>
            {
        ...
        private void DeleteInwardDrug(long InID)
        {
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginDeleteInwardThanhTrungMedDept(InID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                int results = contract.EndDeleteInwardThanhTrungMedDept(asyncResult);
                                if (results == 0)
                                {
                                    Load_InwardThanhTrungDetails(CurrentInwardThanhTrungMedDeptInvoice.inviID);
                                    InwardThanhTrungMedDeptInvoice_GetListCost(CurrentInwardThanhTrungMedDeptInvoice.inviID);
        ...
        {
            if (CurrentInwardThanhTrungMedDeptInvoice.CanEditAndDelete)
            {
                if (MessageBox.Show(eHCMSResources.A0118_G1_Msg_ConfXoaDong, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    if (CurrentInwardThanhTrungMedDept != null)
                    {
                        DeleteInwardDrug(CurrentInwardThanhTrungMedDept.InID);
                    }
                }
            }
            else
        ...
        {
            CurrentInwardDrugMedDeptCopy = CurrentInwardThanhTrungMedDept.DeepCopy();
        }

        public void lnkEdit_Click(object sender, RoutedEventArgs e)
        {
            if (CurrentInwardThanhTrungMedDeptInvoice.DrugDeptSupplierPaymentReqID != null && CurrentInwardThanhTrungMedDeptInvoice.DrugDeptSupplierPaymentReqID > 0)
            {
            ...
            {
                if (CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency != null && CurrentInwardThanhTrungMedDeptInvoice.SelectedCurrency.CurrencyID != (long)AllLookupValues.CurrencyTable.VND)
                {
                ...
                proAlloc.CurrentInwardDrugMedDeptCopy = CurrentInwardThanhTrungMedDept.DeepCopy();
                proAlloc.SetValueForProperty();

                if (CurrentInwardThanhTrungMedDeptInvoice.CheckedPoint)
                {
        ...
        private void CheckEnableSupplier()
        {
            if ((DrugDeptPurchaseOrderDetailList != null && DrugDeptPurchaseOrderDetailList.Count > 0
                ...
                || (InwardThanhTrungMedDeptList != null && InwardThanhTrungMedDeptList.Count > 0))
            {
        ...
        public void OpenPopUpSearchInwardInvoice(IList<InwardDrugMedDeptInvoice> results, int Totalcount)
        {
            ...
            void onInitDlg(IDrugDeptInwardDrugSupplierSearch proAlloc)
            {
                ...
                proAlloc.V_MedProductType = V_MedProductType;
                 if (V_MedProductType == (long)AllLookupValues.MedProductType.THANHTRUNG)
                {
                    proAlloc.strHienThi = eHCMSResources.Z2500_G1_ThanhTrung.ToUpper();
                }
                else
                {
        ...
        private void SearchInwardInvoiceThanhTrung(int PageIndex, int PageSize)
        {
            if (SearchCriteria == null)
            {
                return;
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginSearchInwardThanhTrungMedDeptInvoice(SearchCriteria, TypID, V_MedProductType, PageIndex, PageSize, true, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndSearchInwardThanhTrungMedDeptInvoice(out int Totalcount, asyncResult);
                                if (results != null && results.Count > 0)
                                {
                                    if (results.Count > 1)
                                    {
                                    ...
                                    {
                                        CurrentInwardThanhTrungMedDeptInvoice = results.FirstOrDefault();
                                        InitDetailInward();
                                        DeepCopyInvoice();
        ...
        public void tbx_Search_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                SearchInwardInvoiceThanhTrung(0, Globals.PageSize);
            }
        }
        ...
        public void Handle(DrugDeptCloseSearchSupplierEvent message)
        {
            ...
            {
                CurrentInwardThanhTrungMedDeptInvoice.SelectedSupplier = message.SelectedSupplier as DrugDeptSupplier;
                PharmacyPurchaseOrder_BySupplierID();
            }
        }
        ...
        public void btnPreview()
        {
            Action<IDrugDeptReportDocumentPreview> onInitDlg = delegate (IDrugDeptReportDocumentPreview proAlloc)
            {
                ...
                {
                    proAlloc.eItem = ReportName.DRUGDEPT_INWARD_THANH_TRUNG_SUPPLIER;
                }
                else
                {
                    proAlloc.eItem = ReportName.DRUGDEPT_INWARD_THANH_TRUNG_SUPPLIER_TRONGNUOC;
                }

                if (V_MedProductType == (long)AllLookupValues.MedProductType.THANHTRUNG )
                {
                    proAlloc.LyDo = eHCMSResources.Z2503_G1_PhieuNhapKhoThanhTrung.ToUpper();
                }
                else
                {
                    proAlloc.LyDo = "";
                }
                if (CurrentInwardThanhTrungMedDeptInvoice != null && CurrentInwardThanhTrungMedDeptInvoice.SelectedStaff != null)
                {
                    proAlloc.StaffFullName = CurrentInwardThanhTrungMedDeptInvoice.SelectedStaff.FullName;
                }
            };
        ...
        public void btnPrint()
        {
            ...
            var t = new Thread(() =>
            {
                using (var serviceFactory = new ReportServiceClient())
                {
                    ...
                    contract.BeginGetInWardThanhTrungInPdfFormat(CurrentInwardThanhTrungMedDeptInvoice.inviID, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            var results = contract.EndGetInWardThanhTrungInPdfFormat(asyncResult);
        ...
        public void Handle(DrugDeptCloseSearchInwardIncoiceEvent message)
        {
            if (message != null && this.IsActive)
            {
                CurrentInwardThanhTrungMedDeptInvoice = message.SelectedInwardInvoice as InwardDrugMedDeptInvoice;
                InitDetailInward();
                DeepCopyInvoice();
                LoadInfoThenSelectedInvoice();
        ...
        private void InwardThanhTrungMedDeptInvoice_GetListCost(long inviID)
        {
            this.ShowBusyIndicator(eHCMSResources.Z0125_G1_DangXuLi);
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginInwardThanhTrungMedDeptInvoice_GetListCost(inviID, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                var results = contract.EndInwardThanhTrungMedDeptInvoice_GetListCost(asyncResult);
                                CurrentInwardThanhTrungMedDeptInvoice.CostTableMedDeptLists = results.ToObservableCollection();
                                SumTotalPercent();
                            }
                            catch (Exception ex)
                            {
        ...
        {
            TotalPercent = 0;
            if (CurrentInwardThanhTrungMedDeptInvoice != null && CurrentInwardThanhTrungMedDeptInvoice.CostTableMedDeptLists != null)
            {
                for (int i = 0; i < CurrentInwardThanhTrungMedDeptInvoice.CostTableMedDeptLists.Count; i++)
                {
                    TotalPercent += CurrentInwardThanhTrungMedDeptInvoice.CostTableMedDeptLists[i].Rates;
                }
            }
        }
        #endregion
        ...
        public void ActualQD_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                if (CurrentInwardThanhTrungMedDeptInvoice != null && CurrentInwardThanhTrungMedDeptInvoice.ExchangeRates != 0)
                {
                    try
                    {
                        CurrentInwardThanhTrungMedDeptInvoice.TotalPriceActual_QĐ = Convert.ToDecimal((sender as TextBox).Text);
                    }
                    catch
                    {
                        Globals.ShowMessage(eHCMSResources.A0559_G1_Msg_InfoGTriKhHopLe, eHCMSResources.T0074_G1_I);
                    }
                    CurrentInwardThanhTrungMedDeptInvoice.TotalPriceActual = CurrentInwardThanhTrungMedDeptInvoice.TotalPriceActual_QĐ / (decimal)CurrentInwardThanhTrungMedDeptInvoice.ExchangeRates;
                }
            }
        }
        ...
        public void Actual_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                if (CurrentInwardThanhTrungMedDeptInvoice != null)
                {
                    try
                    {
                        CurrentInwardThanhTrungMedDeptInvoice.TotalPriceActual = Convert.ToDecimal((sender as TextBox).Text);
                    }
                    catch
                    {
                        Globals.ShowMessage(eHCMSResources.A0559_G1_Msg_InfoGTriKhHopLe, eHCMSResources.T0074_G1_I);
