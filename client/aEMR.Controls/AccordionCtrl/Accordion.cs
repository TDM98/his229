...
//using System.Linq;
using resources = aEMR.Controls.Resources;
    ...
    public class Accordion : ItemsControl, IUpdateVisualState
    {
        ...
        private static void OnExpandDirectionPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
                ...
                expandDirection != ExpandDirection.Right)
            {
                ...
                string message = string.Format(
                    ...
                    resources.Accordion_OnExpandDirectionPropertyChanged_InvalidValue,
                    expandDirection);
            ...
            {
                DependencyObject theAccItem = source.ItemContainerGenerator.ContainerFromIndex(i);
                //AccordionItem accordionItem = source.ItemContainerGenerator.ContainerFromIndex(i) as AccordionItem;
                AccordionItem accordionItem = theAccItem as AccordionItem;
            ...
            // TxD 10/06/2018 The following had a compile error cannot convert from method group to Delegate
            //                  and fixed below 
            //source.Dispatcher.BeginInvoke(source.LayoutChildren);
            Action theAct = () => { source.LayoutChildren(); };
            source.Dispatcher.BeginInvoke (theAct);                       
        }
        ...
        private static void OnSelectionModePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
                ...
                newValue != AccordionSelectionMode.ZeroOrOne)
            {
                ...
                string message = string.Format(
                    CultureInfo.InvariantCulture,
                    resources.Accordion_OnSelectionModePropertyChanged_InvalidValue,
                    newValue);
                throw new ArgumentOutOfRangeException("e", message);
            }
        ...
        private static void OnSelectedItemPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (oldValue != null && oldValue.Equals(newValue))
            {
                ...
                // TxD 10/06/2018 SelectionChangedEventArgs first argument is of type RoutedEvent
                //                passing null for NOW until findout how to get RoutedEvent id 
                source.OnSelectedItemChanged(new SelectionChangedEventArgs(null, oldValues, newValues));
                return;
            }
        ...
        private static void OnSelectedIndexPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            else if (source._selectedIndexNestedLevel == 0)
            {
                ...
                // TxD 10/06/2018 replaced ElementAtOrDefault with GetItemAt BECAUSE 
                //                it somehow DID NOT EXISTS
                //source.SelectedItem = source.Items.ElementAtOrDefault(newValue);
                source.SelectedItem = source.Items.GetItemAt(newValue);
        ...
        private static void OnSelectionSequencePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
                ...
                newValue != SelectionSequence.Simultaneous)
            {
                ...
                string message = string.Format(
                    CultureInfo.InvariantCulture,
                    resources.Accordion_OnSelectionSequencepropertyChanged_InvalidValue,
                    newValue);
                throw new ArgumentOutOfRangeException("e", message);
            }
        }
        ...
        private static void OnSelectedItemsChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!accordion._isAllowedToWriteSelectedItems)
            {
                ...
                throw new InvalidOperationException(resources.Accordion_OnSelectedItemsChanged_InvalidWrite);
            }
        }
        ...
        private static void OnSelectedIndicesChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!accordion._isAllowedToWriteSelectedIndices)
            {
                ...
                throw new InvalidOperationException(resources.Accordion_OnSelectedIndicesChanged_InvalidWrite);
            }
        }
        ...
        private void UnselectItem(int index, object item)
        {
            ...
            if (SelectedItems.OfType<object>().Contains(item) && index != newSelectedIndex && !item.Equals(SelectedItem))
            {
                ...
                if (SelectedIndices.Count(i => i != index && i < Items.Count && Items[i].Equals(item)) == 0)
                {
                    ...
                    {
                        throw new InvalidOperationException(resources.Accordion_InvalidManipulationOfSelectionCollections);
                    }
                    SelectedItems.Remove(item);
                }
            }
                ...
                default:
                    {
                        string message = string.Format(
                                CultureInfo.InvariantCulture,
                                resources.Accordion_UnsupportedCollectionAction,
                                e.Action);
        ...
        internal virtual void OnActionFinish(AccordionItem item)
        {
            if (SelectionSequence == SelectionSequence.CollapseBeforeExpand)
            {
                lock (this)
                {
                    ...
                    {
                        throw new InvalidOperationException(resources.Accordion_OnActionFinish_InvalidFinish);
                    }
