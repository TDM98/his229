    ...
    public class AccordionItem : HeaderedContentControl, IUpdateVisualState
    {
        ...
        private static void OnExpandDirectionPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!ctrl._allowedToWriteExpandDirection)
            {
                ...
                string message = string.Format(
                    ...
                    resources.AccordionItem_InvalidWriteToExpandDirection,
                    newValue);
                ...
                newValue != ExpandDirection.Up)
            {
                ...
                string message = string.Format(
                    CultureInfo.InvariantCulture,
                    resources.Expander_OnExpandDirectionPropertyChanged_InvalidValue,
                    newValue);
        ...
        private static void OnIsSelectedPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (ctrl.IsLocked && ctrl._isSelectedNestedLevel == 0)
            {
                ...
                ctrl._isSelectedNestedLevel--;
                throw new InvalidOperationException(resources.AccordionItem_OnIsSelectedPropertyChanged_InvalidChange);
            }
        ...
        private static void OnContentTargetSizePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!source._allowedToWriteContentTargetSize)
            {
                ...
                throw new InvalidOperationException(resources.AccordionItem_InvalidWriteToContentTargetSize);
            }
        ...
        internal virtual void StartAction()
        {
            ...
            {
                throw new InvalidOperationException(resources.AccordionItem_StartAction_InvalidCall);
            }
            Action layoutAction;
                ...
                default:
                    {
                        string message = string.Format(
                            CultureInfo.InvariantCulture,
                            resources.AccordionItem_StartAction_InvalidAction,
                            ScheduledAction);
        ...
        public override void OnApplyTemplate()
        {
            ...
            if (VisualTreeHelper.GetChildrenCount(this) > 0)
            {
                ...
                if (root != null)
                {
                    ExpandStoryboard = new Storyboard();
                    CollapseStoryboard = new Storyboard();
                    Collection<VisualStateGroup> Col1 = (VisualStateManager.GetVisualStateGroups(root) as Collection<VisualStateGroup>);
                    foreach (var stategroup in Col1)
                    {
                        if (stategroup.Name == VisualStates.GroupExpansion)
                        {
                            Collection<VisualState> theStates = stategroup.States as Collection<VisualState>;
                            if (theStates != null)
                            {
                                foreach (var state in theStates)
                                {
                                    if (state.Name == VisualStates.StateExpanded)
                                    {
                                        ExpandStoryboard = state.Storyboard;
                                    }
                                    else if (state.Name == VisualStates.StateCollapsed)
                                    {
                                        CollapseStoryboard = state.Storyboard;
                                    }
                                }
                            }
                        }
