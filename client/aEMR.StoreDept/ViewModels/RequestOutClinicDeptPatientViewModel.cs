...
{
    [Export(typeof(IStoreDeptRequestNew)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class RequestOutClinicDeptPatientViewModel : Conductor<object>, IStoreDeptRequestNew, 
        ...
        ,IHandle<PatientSelectedGoToKhamBenh_InPt<PatientRegistration>>
    {
        ...
        public RequestOutClinicDeptPatientViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            authorization();
            ...
            {
                MessageBox.Show("Bạn chưa được phân công trách nhiệm cho kho phòng nào.\n Vui lòng liên hệ với người quản trị để được phân bố!");
            }
                ...
                if (RequestDrug.ReqOutwardDetails != null)
                {
                    foreach (ReqOutwardDrugClinicDeptPatient item in RequestDrug.ReqOutwardDetails)
                    {
                        if (item.GenMedProductID != null && item.GenMedProductID != 0)
                        {
                            if (!(item.ReqQty > 0))
                            {
                                string strErr = "Dữ liệu dòng: [" + item.RefGenericDrugDetail.BrandNameAndCode + "] Số lượng yêu cầu phải > 0";
            ...
            try
            {
                var t = new Thread(() =>
                {
                    using (var serviceFactory = new PharmacyClinicDeptServiceClient())
                    {
                        ...
                        contract.BeginFullOperatorRequestDrugInwardClinicDeptNew(RequestDrug, V_MedProductType,(long)AllLookupValues.RegistrationType.NGOAI_TRU
        ...
        private bool CheckValidData()
        {
            ...
            else
            {
                ...
                {
                    if (RequestDrug.ReqOutwardDetails[i].RefGenericDrugDetail != null && RequestDrug.ReqOutwardDetails[i].ReqQty <= 0)
                    {
                        MessageBox.Show("SL YC phải > 0 !");
                        return false;
                    }
                    ...
                    using (var serviceFactory = new PharmacyClinicDeptServiceClient())
                    {
                        ...
                        contract.BeginSearchRequestDrugInwardClinicDept(SearchCriteria, V_MedProductType, PageIndex, PageSize, true, Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
                                ...
                                {
                                    if (MessageBox.Show("Không tìm thấy. Bạn có muốn tiếp tục tìm kiếm?", eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                                    {
                                        SearchCriteria.FromDate = Globals.GetCurServerDateTime().AddDays(-7);
                                        SearchCriteria.ToDate = Globals.GetCurServerDateTime();
        ...
        private void GetInPatientRequestingDrugListByReqID(long RequestID, bool bCreateNewListFromOld = false)
        {
            ...
            try
            {
                var t = new Thread(() =>
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginGetReqOutwardDrugClinicDeptPatientByID(RequestID, false, (long)AllLookupValues.RegistrationType.NGOAI_TRU
                            ,  Globals.DispatchCallback((asyncResult) =>
                        {
                            try
                            {
        ...
        {            
            ...
            if (RequestDrug.ReqOutwardDetails != null)
            {
                ...
                for (int i = 0; i < RequestDrug.ReqOutwardDetails.Count; i++)
                {
                    ...
                    {
                        results = false;
                        MessageBox.Show(string.Format(eHCMSResources.Z1124_G1_SLgYCLonHonBang0, RequestDrug.ReqOutwardDetails[i].RefGenericDrugDetail.BrandNameAndCode));
                        break;
                    }
                }
            }
        ...
        {
            if (((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails != null)
            {
                ...
                {
                    RemoveMark(((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails.SelectedItem);
                }
            }
        }
        ...
        {
            if (((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails != null && ((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails.IsValid)
            {
                if (CheckValidGrid())
                {
                    SaveRequest();
        ...
        public void btnAddItem()
        {
            ...
            {
                if (string.IsNullOrEmpty(CurrentReqOutwardDrugClinicDeptPatient.RefGenericDrugDetail.Code))
                {
        ...
        private void HideShowColumnDelete()
        {
            if (((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails != null)
            {
                ...
                {
                    ((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails.Columns[(int)DataGridCol.ColDelete].Visibility = Visibility.Visible;
                    ((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails.Columns[(int)DataGridCol.ColMultiDelete].Visibility = Visibility.Visible;
                }
                else
                {
                    ((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails.Columns[(int)DataGridCol.ColDelete].Visibility = Visibility.Collapsed;
                    ((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails.Columns[(int)DataGridCol.ColMultiDelete].Visibility = Visibility.Collapsed;
                }
            }
        }
        ...
        private void CloseOrOpenGroups(bool bClosed)
        {
            ...
            AxDataGridNy theGrid = ((RequestOutClinicDeptPatientView)this.GetView()).grdReqOutwardDetails;
            //ChangedWPF-CMN
        ...
        public void AxTextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            if (txt != (sender as TextBox).Text)
            {
                ...
                {
                    GetRefGenMedProductDetails_Auto(txt, 0, RefGenMedProductDetails.PageSize);
                }
                else
                {
