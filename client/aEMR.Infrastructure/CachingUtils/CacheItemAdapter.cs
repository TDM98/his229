...
{
    public class CacheItemAdapter<Key,Value> 
		where Key: IComparable<Key>
		where Value: ICachable

    {
        ...
        public CacheItemAdapter(int maxBytes)	
		{
		...
		/// </summary>
        private ConcurrentDictionary<Key, Value> cacheStore = null;
		...
		/// <remarks>The cache is at the mercy of the <c>ICachable</c> items
		...
		public int CurrentCacheUsage
		{
			get
			{
				...
				if (cacheStore != null)
				{
					...
					{
						size += (c.Value as ICachable).BytesUsed;
					}
				}
				return size;
			}
		...
		private void PurgeSpace(int bytes)
		{
				 ...
				 cacheStore.Count != 0)
			{
				...
				while (MaxBytes <= (CurrentCacheUsage + bytes) && arrivalOrder.Count > 0 )
				{
				    ...
				    var temp = default(Value);
				    cacheStore.TryRemove(k, out temp);
		...
		private void StoreItem(Key k, Value v)
		{
			...
			{
				// Create the stores.
			    cacheStore = new ConcurrentDictionary<Key, Value>();
				arrivalOrder = new Queue<Key>();
			}
			arrivalOrder.Enqueue(k);
		    cacheStore.TryAdd(k, v);
		}
		...
		public void Add(Key k, Value v)
      {
		...
		{ 
			get
			{
			    return (cacheStore == null) ? 0 : cacheStore.Count;
			} 
		}
