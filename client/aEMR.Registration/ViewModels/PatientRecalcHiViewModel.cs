...
{
    [Export(typeof(IPatientRecalcHi)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class PatientRecalcHiViewModel : HandlePayCompletedViewModelBase, IPatientRecalcHi
        ...
        , IHandle<ConfirmHiBenefit>
    {
        ...
        public PatientRecalcHiViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            _eventArg = eventArg;
            authorization();
            ...
            patientInfoVm.mInfo_XacNhan = true;
            patientInfoVm.CanConfirmHi = true;
            ...
            RegistrationDetailsContent.ShowClickButton = false;
        ...
        public void ApplyHiToInPatientRegistration()
        {
            ...
            // TxD 02/03/2018 HiBenefit has been removed from Interface for PatientSummaryInfoContent
            //var benefit = PatientSummaryInfoContent.HiBenefit;
            double benefit = 0.8;

            //HPT 24/08/2015: Khai báo biến cục bộ IsHICard_FiveYearsCont để lưu giá trị thuộc tính này của đối tượng CurRegistration, truyền vào cho parameter của hàm ApplyHiToInPatientRegistration
            //Vì hàm này sử dụng lại hàm ApplyHiToInPatientRegistration, kể từ 20/08/2015 đã được chỉnh sửa thêm một parameter phục vụ tính quyền lợi bảo hiểm dành cho BN có BHYT 5 năm liên tiếp
            ...
            bool IsChildUnder6YearsOld = false;
            if (CurRegistration != null)
            {
                IsHICard_FiveYearsCont = CurRegistration.IsHICard_FiveYearsCont;
                IsChildUnder6YearsOld = CurRegistration.IsChildUnder6YearsOld;
            }
            //HPT 24/08/2015 END
                ...
                try
                {
                    using (var serviceFactory = new PatientRegistrationServiceClient())
                    {
                        ...
                        contract.BeginApplyHiToInPatientRegistration(CurRegistration.PtRegistrationID,
                            ...
                            ConfirmedHiItem.HIID, benefit, false, PaperReferalID, (int)AllLookupValues.V_FindPatientType.NOI_TRU, false,  
                            Globals.LoggedUserAccount.StaffID.GetValueOrDefault(), IsHICard_FiveYearsCont, IsChildUnder6YearsOld, eConfirmType,
                            Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
        ...
        public void Handle(HiCardConfirmedEvent message)
        {
            if (message != null)
            {
                ...
                Coroutine.BeginExecute(DoCalcHiBenefit(message.HiProfile, message.PaperReferal), null, (o, e) =>
                {
        ...
        {
            var calcHiTask = new CalcHiBenefitTask(hiItem, referal, (long)AllLookupValues.RegistrationType.NGOAI_TRU, false);
            ...
            var t = new Thread(() =>
            {
                try
                {
                    using (var serviceFactory = new PatientRegistrationServiceClient())
                    {
                        ...
                        contract.BeginApplyHiToInPatientRegistration(CurRegistration.PtRegistrationID,
                            //HIID,HisID,HiBenefit,IsCrossRegion,PaperReferalID
                            ConfirmedHiItem.HIID, benefit, false, PaperReferalID, (int)AllLookupValues.V_FindPatientType.NGOAI_TRU, false,
                            Globals.LoggedUserAccount.StaffID.GetValueOrDefault(), CurRegistration.IsHICard_FiveYearsCont, CurRegistration.IsChildUnder6YearsOld, eConfirmType,
                            Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
