...
* 20200318 #014 TTM:   BM ?: Lấy danh sách dịch vụ theo giới tính của bệnh nhân.
* 20200402 #015 TBL:   BM 0029072: Khi chỉ định CLS từ quầy đăng ký phải chọn BS chỉ định (có cấu hình)
* 20200404 #016 TTM:   BM 0029085: Chức năng khám tại gia, nếu chức năng khám tại gia được bật thì khi đăng ký sẽ không có chọn phòng cho dịch vụ khám và không cấp số thứ tự
* 20200718 #017 TTM:   BM 0039388: Cấp số lần 2 cho bệnh nhân huỷ đăng ký.
* 20200807 #018 TNHX:   BM : Thêm nút xác nhận đăng ký ngoại trú là cấp cứu
* 20210704 #019 TNHX:   385 Lấy giờ tạo STT từ QMS làm giờ vào viện nếu có
* 20211004 #020 TNHX: Lọc danh sách bsi theo cấu hình trách nhiệm + theo tích tạm khóa
* 20230530 #021 DatTB:
...
{
    [Export(typeof(IPatientRegistration)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class PatientRegistrationViewModel : HandlePayCompletedViewModelBase, IPatientRegistration
        ...
        //, IHandle<NoChangeConfirmHiBenefit>
        ...
        , IHandle<SetRefByStaffForRegistrationSummaryV2>
        , IHandle<SetTicketNumnerTextForPatientRegistrationView>
        , IHandle<SetTicketIssueForPatientRegistrationView>
        , IHandle<SetTicketForNewRegistrationAgain>
    {
        IEventAggregator _eventArg;
        [ImportingConstructor]
        public PatientRegistrationViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            _eventArg = eventArg;
            Initialize();
            ...
            searchPatientAndRegVm.IsShowCallQMS = Globals.ServerConfigSection.CommonItems.UseQMSSystem;
            searchPatientAndRegVm.IsShowCallButton = true;
            ...
            //▼===== #015
            if (Globals.ServerConfigSection.OutRegisElements.CheckDoctorStaffID)
            {
            ...
            ListV_MedicalServiceParentGroup = Globals.AllLookupValueList.Where(x => x.ObjectTypeID == (long)LookupValues.V_MedicalServiceParentGroup).ToObservableCollection();
            ListV_MedicalServiceParentGroup.Insert(0,new Lookup { LookupID = 0 , ObjectName = ""});
        }
        ...
        public bool MaleChecked
        {
            ...
            {
                _MaleChecked = value;
                NotifyOfPropertyChange(() => MaleChecked);
            }
        }
        ...
        public bool FemaleChecked
        {
            ...
            set
            {
                ...
                NotifyOfPropertyChange(() => FemaleChecked);
            }
        }
        private bool _NoGenderChecked;
        public bool NoGenderChecked
        {
            get { return _NoGenderChecked; }
            set
            {
                _NoGenderChecked = value;
                NotifyOfPropertyChange(() => NoGenderChecked);
            }
        }
            ...
            set
            {
                if (_medServiceItem != value)
                {
                    ...
                    if (medServiceID > 0 && _medServiceType != null)
                    {
                        ...
                        {
                            //▼===== #016
                            if (Globals.DeptLocation.LID != Globals.ServerConfigSection.CommonItems.LIDForConsultationAtHome)
                            {
        ...
        public bool ShowCBBDoctor
        {
            get { return Globals.ServerConfigSection.OutRegisElements.CheckDoctorStaffID; }
        }
        ...
        public void rdRegistrationType_Checked(object sender, RoutedEventArgs e)
        {
            ...
            {
                RefMedicalServiceGroupObj.V_RegistrationType = (long)AllLookupValues.RegistrationType.NGOAI_TRU;
            }
            else if (InPatientChecked)
            {
                RefMedicalServiceGroupObj.V_RegistrationType = (long)AllLookupValues.RegistrationType.NOI_TRU;
            }
            else
            {
                RefMedicalServiceGroupObj.V_RegistrationType = 0;
            }
        }
        public void rdGender_Checked(object sender, RoutedEventArgs e)
        {
            if (MaleChecked)
            {
                RefMedicalServiceGroupObj.Gender = false;
            }
            else if (FemaleChecked)
            {
                RefMedicalServiceGroupObj.Gender = true;
            }
            else
            {
        ...
        public Patient RefByPatient
        {
            ...
            set
            {
                if (_RefByPatient != value)
                {
                    ...
                    if (RegistrationDetailsContent.CurrentRegistration != null)
                    {
                        RegistrationDetailsContent.CurrentRegistration.RefByPatient = RefByPatient;
                    }
                }
            }
        }
        ...
        public bool IsPriority
        {
            ...
            set
            {
                ...
                {
                    if (_IsPriority == false && !CheckAgeIsPriority(CurRegistration) && CurRegistration.Patient.Gender != "F")
                    {
                        MessageBox.Show("Bệnh nhân đăng ký khám BHYT và không nằm trong đối tượng ưu tiên!");
                    }
                    else
                    {
                        if (_IsPriority == true && CheckAgeIsPriority(CurRegistration))
                        {
                            if (MessageBox.Show("Bệnh nhân thuộc đối tượng ưu tiên! " + 
                                Environment.NewLine + "Xác nhận bỏ chọn ưu tiên?", "Thông báo!", MessageBoxButton.OKCancel) == MessageBoxResult.Cancel)
                            {
                                return;
                            }
                        }
        ...
        public bool CheckAgeIsPriority(PatientRegistration curRegistration)
        {

            if(curRegistration != null && curRegistration.HealthInsurance != null)
            {
                if (curRegistration.Patient.Age != 0)
                {
                    if (curRegistration.Patient.Age <= 70 && curRegistration.Patient.Age >= 6)
                    {
                        return false;
                    }
                }
                else
                {
                    ...
                    int monthnew = 0;
                    monthnew = (loadCurrentDate.Month + loadCurrentDate.Year * 12) - (Convert.ToDateTime(curRegistration.Patient.DOB).Month + Convert.ToDateTime(curRegistration.Patient.DOB).Year * 12);
                    if (monthnew > 72)
                        return false;
                }
            }
        ...
        public void Handle(ItemSelected<Patient> message)
        {
            ...
            // VuTTM
            if (GlobalsNAV.IsQMSEnable() && CurrentPatient.OrderNumber != GlobalsNAV.EXCEPT_ORDER_NUMBER)
            {
                if (!GlobalsNAV.ValidOrder(Globals.DeptLocation.DeptID, CurrentPatient.OrderNumber, CurrentPatient.PatientCode))
                {
                    return;
                }

                OrderDTO orderDTO = GlobalsNAV.UpdateOrder(message.Item, OrderDTO.CALLING_STATUS);
                if (null == orderDTO)
                {
                    MessageBox.Show("Thông tin bệnh nhân và STT không khớp.");
                    return;
                }
                else if (orderDTO.patientCode == message.Item.PatientCode
                    && 0 != message.Item.OrderNumber
                    && message.Item.OrderNumber != orderDTO.orderNumber)
                {
                    MessageBox.Show("Thông tin bệnh nhân và STT không khớp.");
                    return;
                }
                //▼====: #019
                if (orderDTO != null && CurrentPatient.DateCreatedQMSTicket == null)
                {
                    DateCreatedQMSTicket = DateTime.Parse(orderDTO.createdAt);
                }
                else
                {
                    DateCreatedQMSTicket = CurrentPatient.DateCreatedQMSTicket;
                }
                //▲====: #019
            }
        ...
        public void Handle(CreateNewPatientEvent message)
        {
            ...
            PatientDetailsContent.CurrentPatient.OrderNumber = message.OrderNumber;
            PatientDetailsContent.CurrentPatient.ServiceStartedAt = message.ServiceStartedAt;
        ...
        public void Handle(HiCardConfirmedEvent evtMsg)
        {
            if (evtMsg == null || evtMsg.HiProfile == null)
                return;
            //20210911 BLQ: Kiểm tra bệnh nhân là đối tượng không bảo hiểm thì không cho xác nhận bảo hiểm
            if(CurClassification.PatientClassID == 1 && CurRegistration.PtRegistrationID>0)
            {
                MessageBox.Show("Đăng ký này không thể xác nhận bảo hiểm");
                return;
            }
            ...
            (
                CalcHIBenefit_ForRegistration(evtMsg.HiProfile, evtMsg.PaperReferal, evtMsg.IsEmergInPtReExamination, evtMsg.IsHICard_FiveYearsCont
                    , evtMsg.IsChildUnder6YearsOld, evtMsg.IsAllowCrossRegion, evtMsg.IsHICard_FiveYearsCont_NoPaid, evtMsg.FiveYearsAppliedDate
                    , evtMsg.FiveYearsARowDate, evtMsg.V_ReceiveMethod),
                null,
                (o, e) =>
                {
        ...
        public void Handle(ResultFound<Patient> message)
        {
            if (message != null)
            {
                ...
                if (CurrentPatient != null)
                {
                    ...
                    //▼===== 20200525 TTM: Nếu như tìm kiếm bệnh nhân mới thì sẽ clear ticket nếu ticket đang tình trạngd đã đăng ký
                    if (RegistrationDetailsContent != null && RegistrationDetailsContent.TicketIssueObj != null)
                    {
                        RegistrationDetailsContent.TicketIssueObj = null;
                    }
                }
            }
        }
        ...
        public void Handle(ResultNotFound<Patient> message)
        {
            ...
            if (GlobalsNAV.IsQMSEnable() && 0 == message.OrderNumber)
            {
                MessageBox.Show("Vui lòng nhập STT!", eHCMSResources.G0442_G1_TBao);
                return;
            }
            ...
            if (message != null)
            {
                ...
                if (criteria != null && criteria.QRCode != null) //DatTB Thêm kiểm tra null
                {
                    // Tìm theo QRCode thì tự động thêm BN nếu tìm không thấy
                    PatientDetailsContent = null;
                    PatientDetailsContent = Globals.GetViewModel<IPatientDetails>();
                    PatientDetailsContent.Enable_ReConfirmHI_InPatientOnly = true;
                    PatientDetailsContent.CreateNewPatient();
                    ...
                    PatientDetailsContent.CurrentPatient.OrderNumber = message.OrderNumber;
                    GlobalsNAV.ShowDialog_V3(PatientDetailsContent);
                }
                //▼==== #021
                else if (criteria != null && criteria.IDCardQRCode != null)
                {
                    PatientDetailsContent = null;
                    PatientDetailsContent = Globals.GetViewModel<IPatientDetails>();
                    PatientDetailsContent.Enable_ReConfirmHI_InPatientOnly = true;
                    PatientDetailsContent.CreateNewPatient();
                    PatientDetailsContent.IsChildWindow = true;
                    PatientDetailsContent.IDCardQRCode = criteria.IDCardQRCode;
                    PatientDetailsContent.CurrentPatient.OrderNumber = message.OrderNumber;
                    GlobalsNAV.ShowDialog_V3(PatientDetailsContent);
                }
                //▲==== #021
                else
                {
                    ...
                    if (result == MessageBoxResult.OK)
                    {
                        ...
                        if (criteria != null)
                        {
                            ...
                            //▼==== #021
                            PatientDetailsContent.IDCardQRCode = criteria.IDCardQRCode;
                            if (criteria.IDNumber == null && criteria.IDNumberOld != null)
                            {
                                PatientDetailsContent.CurrentPatient.IDNumber = criteria.IDNumberOld;
                            }
                            else
                            {
                                PatientDetailsContent.CurrentPatient.IDNumber = criteria.IDNumber;
                            }
                            //▲==== #021
                        }
                        PatientDetailsContent.CurrentPatient.OrderNumber = message.OrderNumber;
                            ...
                            Globals.DispatchCallback(asyncResult =>
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            item.RefMedicalServiceType = sType;
                                            if (string.Compare(CurrentPatient.Gender, item.GenderType) == 0 || item.GenderType == "A")
                                            {
                                                col.Add(item);
                                            }
                                        }
        ...
        private void CallAddRegItemCmd(long? ConsultationRoomStaffAllocID = null, DateTime? ApptStartDate = null, DateTime? ApptEndDate = null)
        {
            ...
            else
            {
                RegistrationDetailsContent.AddNewService(ObjectCopier.DeepCopy(MedServiceItem), DeptLocation != null ? DeptLocation : null
                    , MedServiceType, ConsultationRoomStaffAllocID, ApptStartDate, ApptEndDate, null, null, null, IsPriority, (decimal)ServiceQty);
            }
            //▲====: #013
        }
        ...
        public IEnumerator<IResult> CalcHIBenefit_ForRegistration(HealthInsurance hiItem, PaperReferal referal, bool IsEmergInPtReExamination
            , bool IsHICard_FiveYearsCont, bool IsChildUnder6YearsOld, bool IsAllowCrossRegion, bool IsHICard_FiveYearsCont_NoPaid, DateTime? FiveYearsAppliedDate
            , DateTime? FiveYearsARowDate, long V_ReceiveMethod)
        {
            // =========================================================================================================================
            ...
            Globals.EventAggregator.Publish(new HiCardConfirmCompleted {
                RegistrationCode = hiItem.RegistrationCode
                , HICardNo = hiItem.HICardNo
                , HIPCode = hiItem.HIPCode
                , KVCode = hiItem.KVCode
            });
            if (PatientSummaryInfoContent != null)
            {
            ...
            if (Globals.ServerConfigSection.CommonItems.ApplyFixReCalcHIBenefit)
            {
                if (CurRegistration.HisID == null)
                {
                    CurRegistration.HisID = Globals.DefaultHisID_ForNewRegis_BeforeSaving;
                }
                if (CurRegistration.Appointment != null)
                {
            ...
            IsPriority = CheckAgeIsPriority(CurRegistration);
        }
        ...
        public void setPatientSummaryInfo(PatientRegistration curRegistration)
        {
            ...
            {
                PatientSummaryInfoContent.RefByPatient = curRegistration.RefByPatient;
                PatientSummaryInfoContent.RefByStaff = curRegistration.RefByStaff;
            }
        }
        public IEnumerator<IResult> DoSetCurrentPatient(Patient patient, bool IsClearTicket = true, bool IsFindBySerialTicket = false)
        {
            ...
            long tmpOrderNumber = CurrentPatient.OrderNumber;
            DateTime tmpServiceStartedAt = CurrentPatient.ServiceStartedAt;
            ...
            if (loadPatient.CurrentPatient != null)
            {
                ...
                // VuTTM
                CurrentPatient.OrderNumber = tmpOrderNumber;
                CurrentPatient.ServiceStartedAt = tmpServiceStartedAt;
                    ...
                    && (regDate <= now && regDate.AddDays(ConfigValues.PatientRegistrationTimeout) >= now))
                {
                      ...
                      )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                TitleStatus = RegStatus.ChuaDangKy;
                                if (RegistrationDetailsContent != null && RegistrationDetailsContent.TicketIssueObj != null && RegistrationDetailsContent.TicketIssueObj.V_TicketStatus == (int)V_TicketStatus_Enum.TKT_ALREADY_REGIS)
                                {
                                    RegistrationDetailsContent.TicketIssueObj.V_TicketStatus = (int)V_TicketStatus_Enum.TKT_BEING_CALLED;
                                }
                                DoCreateNewRegistration();
                            }
                            else
            ...
            //▼===== 20200525 TTM: Nếu như tìm kiếm bệnh nhân mới thì sẽ clear ticket nếu ticket đang tình trạngd đã đăng ký
            if (IsClearTicket && RegistrationDetailsContent != null && RegistrationDetailsContent.TicketIssueObj != null)
            {
                RegistrationDetailsContent.TicketIssueObj = null;
            }

            //▼=====: Nếu như tìm kiếm bệnh nhân mới thì đặt IsPriority thành true
            if (RegistrationDetailsContent != null)
            {
                //IsPriority = true;
                IsPriority = CheckAgeIsPriority(CurRegistration);
        ...
        public void DoCreateNewRegistration(bool forNewPatient = true, bool confirm = false, bool IsClearTicketObj = false)
        {
            ...
            DateTime loadCurrentDate = Globals.ServerDate.Value;
            //▼====: #019
            if (DateCreatedQMSTicket != null)
            {
                loadCurrentDate = (DateTime)DateCreatedQMSTicket;
            }
            //▲====: #019
            ...
            if (Globals.ServerConfigSection.CommonItems.UseQMSSystem && RegistrationDetailsContent != null && RegistrationDetailsContent.TicketIssueObj != null
                && !string.IsNullOrEmpty(RegistrationDetailsContent.TicketIssueObj.TicketNumberText) && IsClearTicketObj)
            {
                RegistrationDetailsContent.TicketIssueObj = null;
            }
            ...
            //▲===== #012
            //bool IsSpecialHIRegistration = false;
                    ...
                    )
                {
                    if (regDate <= now && regDate.AddDays(ConfigValues.PatientRegistrationTimeout) >= now)
                    {
                        ...
                        else
                        {
                            OrderDTO order = null;
                            // VuTTM
                            if (GlobalsNAV.IsQMSEnable() && GlobalsNAV.IsApplyingQMS() && null != CurrentPatient)
                            {
                                // Cap trang thai cua STT hien tai la DONE
                                order = GlobalsNAV.UpdateLastOrder(CurrentPatient, OrderDTO.DONE_STATUS);
                                if (null == order)
                                {
                                    MessageBox.Show("Không thể tạo STT cho Đăng Ký Mới!");
                                    return;
                                }
                                // Tao moi STT cho dang ky moi 
                                order = GlobalsNAV.GetOrder(
                                    GlobalsNAV.InitOrder(CurrentPatient, order.createdAt, OrderDTO.CALLING_STATUS));
                                if (null != order)
                                {
                                    MessageBox.Show("STT mới của bệnh nhân " + order.patientName + " là: " + order.orderNumber);
                                }
                                else
                                {
                                    MessageBox.Show("Không thể tạo STT cho Đăng Ký Mới!");
                    ...
                    //if (CurRegistration != null && CurRegistration.HisID > 0 && CurRegistration.PtInsuranceBenefit > 0
                    //    && CurRegistration.OutPtTreatmentProgramID > 0
                    //    && Globals.ServerConfigSection.CommonItems.SpecialHIRegistration
                    //    && CurRegistration.RegistrationStatus != AllLookupValues.RegistrationStatus.REFUND
                    //    && CurRegistration.ExamDate.Date == Globals.GetCurServerDateTime().Date)
                    //{
                    //    IsSpecialHIRegistration = true;
                    //}
                }
            }
            ...
            //CurRegistration.IsSpecialHIRegistration = IsSpecialHIRegistration;
            if (forNewPatient)
            {
        ...
        public IEnumerator<IResult> DoCancelRegistration()
        {
            ...
            bool IsExistsDrugInvoices = CurRegistration.DrugInvoices.Any(x => (x.V_OutDrugInvStatus != (long)AllLookupValues.V_OutDrugInvStatus.CANCELED
        ...
        private void ShowOldRegistration(PatientRegistration regInfo, PatientAppointment aAppointment, bool fromAppointment = false)
        {
			...
			if (GlobalsNAV.IsQMSEnable() && DateCreatedQMSTicket != null)
            {
                CurRegistration.ExamDate = (DateTime)DateCreatedQMSTicket;
            }
        ...
        {
            DoCreateNewRegistration(false, true, true);
        }
        ...
        public void AddPclExamTypeCmd()
        {
            ...
            if (Globals.ServerConfigSection.OutRegisElements.CheckDoctorStaffID && gSelectedDoctorStaff == null)
            {
                MessageBox.Show(eHCMSResources.A0571_G1_Msg_InfoChonBSCDinh);
                return;
            }
            ...
            SelectPCLContent.SelectedPCLExamType.Qty = (byte)PCLQty;
            RegistrationDetailsContent.AddNewPclRequestDetailFromPclExamType(SelectPCLContent.SelectedPCLExamType, deptLocation, gSelectedDoctorStaff, null, null, IsPriority);
        }
        ...
        public void AddPclExamTypeCmdLAB()
        {
            ...
            SelectPCLContentLAB.SelectedPCLExamType.Qty = (byte)PCLQty;
            RegistrationDetailsContent.AddNewPclRequestDetailFromPclExamType(SelectPCLContentLAB.SelectedPCLExamType, deptLocation, gSelectedDoctorStaff);
        }
        ...
        public void AddAllPclExamTypeCmdLAB()
        {
            ...
            RegistrationDetailsContent.AddNewAllPclRequestDetailFromPclExamType(SelectPCLContentLAB.PclExamTypes, gSelectedDoctorStaff);
        }
        ...
        public void ApplyViewCase(RegistrationViewCase aViewCase, DiagnosisTreatment CurrentDiagnosisTreatment)
        {
            ...
            else if (ViewCase == RegistrationViewCase.RegistrationRequestView)
            {
                if (RegistrationDetailsContent != null && CurRegistration != null)
                {
                    ...
                    //CurRegistration = Globals.PatientAllDetails.PtRegistrationInfo.DeepCopy();
                    //RegistrationDetailsContent.CurrentRegistration = CurRegistration;
                    //ShowOldRegistration(CurRegistration, null);
                }
            }
        }
        ...
        public void btnSearchMedServiceGroups()
        {
            ...
            var SearchMedicalServiceGroups = MedicalServiceGroupCollection.Where(x => ((!string.IsNullOrEmpty(x.MedicalServiceGroupCode) && Globals.RemoveVietnameseString(x.MedicalServiceGroupCode.ToLower()).Contains(SearchCode)) 
            ...
            {
                switch (SearchMedicalServiceGroups.Count())
                {
                    case 0:
                    ...
                    default:
                        ...
                        SearchView.V_RegistrationType = 0;
                        SearchView.SearchMedicalServiceGroupName = SearchMedicalServiceGroupName;
                        SearchView.V_MedicalServiceGroupType = RefMedicalServiceGroupObj.V_MedicalServiceGroupType;
                        ...
                        if (SearchView.SelectedRefMedicalServiceGroup != null)
                        {
                            ...
                            if (SearchView.SelectedRefMedicalServiceGroup.V_RegistrationType == (long)AllLookupValues.RegistrationType.NGOAI_TRU)
                            {
                                OutPatientChecked = true;
                            }
                            else if (SearchView.SelectedRefMedicalServiceGroup.V_RegistrationType == (long)AllLookupValues.RegistrationType.NOI_TRU)
                            {
                            ...
                            switch (SearchView.SelectedRefMedicalServiceGroup.Gender)
                            {
                                case false:
                                    MaleChecked = true;
                                    break;
        ...
        public void CreateNewMedicalServiceGroupCmd()
        {
            Globals.EventAggregator.Publish(new CreateNewMedicalServiceGroup());
            //RegistrationDetailsContent.ApplyRefMedicalServiceGroup(RegistrationDetailsContent.RefMedicalServiceGroupObj, DeptLocation != null ? DeptLocation : null);
            //RegistrationDetailsContent.ApplyRefMedicalServiceGroup(new RefMedicalServiceGroups(), DeptLocation != null ? DeptLocation : null);
            NotifyOfPropertyChange(() => RefMedicalServiceGroupObj);
        }
        public void AddRegPackCmd()
        {
            ...
            if (SearchView.SelectedRefMedicalServiceGroup != null)
            {
                RegistrationDetailsContent.ApplyRefMedicalServiceGroup(SearchView.SelectedRefMedicalServiceGroup, DeptLocation != null ? DeptLocation : null, true);
            }
        }
        ...
        public bool IsAddRegPackVisible
        {
            get
            {
                ...
                return mDangKyDV_DichVu_Them && IsRegistrationView;
                //return false;
            }
        }
        ...
        //▲======= #004
        public void Handle(SetRefByStaffForRegistrationSummaryV2 message)
        {
            if (RegistrationDetailsContent.CurrentRegistration != null && PatientSummaryInfoContent.RefByStaff != null && PatientSummaryInfoContent.RefByStaff.StaffID > 0)
            {
                RegistrationDetailsContent.CurrentRegistration.RefByStaff = PatientSummaryInfoContent.RefByStaff;
            }
        }
        //▼===== #008
        public void GetPriceForAppointment(PatientRegistration CurRegistration)
        {
            ...
            CommonGlobals.CorrectRegistrationDetails_V2(CurRegistration);
        ...
        public void SetEkip()
        {
            ...
            if (RegistrationDetailsContent.CurrentRegistration != null && RegistrationDetailsContent.CurrentRegistration.PatientRegistrationDetails != null && RegistrationDetailsContent.CurrentRegistration.PatientRegistrationDetails.Count > 0)
            {
                ...
                {
                    if (((item.PtRegDetailID > 0 && item.PaidTime != null && item.V_ExamRegStatus != (long)AllLookupValues.ExamRegStatus.NGUNG_TRA_TIEN_LAI && (long)item.RecordState != 4)
                        || (item.ExamRegStatus != AllLookupValues.ExamRegStatus.NGUNG_TRA_TIEN_LAI && item.PaidTime != null) || item.PtRegDetailID <= 0
                        || (item.PaidTime == null && item.PtRegDetailID > 0 && (long)item.RecordState != 3))
                        ...
                        && (item.RefMedicalServiceItem.HIAllowedPrice > 0 || item.RefMedicalServiceItem.HIAllowedPriceNew > 0) && item.IsCountHI)
                    {
            ...
            if (!aView.SaveOK) //20200509 TBL: Nếu không lưu thì trả về như trước lúc thiết lập
            {
                ...
                //    item.TotalHIPayment = item.HIAllowedPrice.GetValueOrDefault() * item.Qty * (decimal)item.HIBenefit.GetValueOrDefault();
                //    item.HIPaymentPercent = 1;
                //    item.TotalPatientPayment = item.InvoicePrice - item.TotalHIPayment;
                //    if (item.IsDiscounted)
                //    {
                //        RegistrationDetailsContent.CurrentRegistration.ApplyDiscount(item, Globals.ServerConfigSection.PharmacyElements.OnlyRoundResultForOutward, false);
                //    }
                //}
        ...
        #region Ticket
        private void SearchPatientByPatientCode(TicketIssue curTicket, bool IsNewRegistration = false)
        {
            ...
            var t = new Thread(() =>
            {
                ...
                try
                {
                    using (var serviceFactory = new PatientRegistrationServiceClient())
                    {
                        ...
                        PatientSearchCriteria mSearchCriteria = new PatientSearchCriteria { PatientCode = curTicket.PatientCode, PatientNameString = curTicket.PatientCode };
                        client.BeginSearchPatients(mSearchCriteria, 0, 5, false, Globals.DispatchCallback((asyncResult) =>
                        {
                            ...
                            if (bOK)
                            {
                                if (allItems.Count == 1)
                                {
                                    CurrentPatient = allItems[0];
                                    if (!IsNewRegistration)
                                    {
                                        Coroutine.BeginExecute(DoSetCurrentPatient(allItems[0], false, false));

                                    }
                                    else
                                    {
                                        Coroutine.BeginExecute(DoSetCurrentPatientForRegistrationByTicketAgain(allItems[0]));
                                    }
                                }
                            }
                        }), null);
        ...
        public void Handle(SetTicketNumnerTextForPatientRegistrationView message)
        {
            if (message != null && message.Item != null)
            {
                TicketNumberText = message.Item.TicketNumberText;
            }
        }
        public void Handle(SetTicketIssueForPatientRegistrationView message)
        {
            ...
            if (message != null && message.Item != null)
            {
                RegistrationDetailsContent.TicketIssueObj = message.Item;
                if (message.IsLoadPatientInfo)
                {
                    if (!string.IsNullOrEmpty(message.Item.PatientCode))
                    {
                        SearchPatientByPatientCode(message.Item);
                    }
                    else
                    {
                        GlobalsNAV.ShowMessagePopup(eHCMSResources.Z3073_G1_QMSTicketNonePatientInfo);
                    }
                }
            }
            else
        ...
        {
            get { return _TicketNumberText; }
            set
            {
                if (_TicketNumberText != value)
                {
                    _TicketNumberText = value;
                    NotifyOfPropertyChange(() => TicketNumberText);
                }
            }
        }
        private bool _IsUseSystemQMS = Globals.ServerConfigSection.CommonItems.UseQMSSystem;
        public bool IsUseSystemQMS
        {
            get { return _IsUseSystemQMS; }
            set
            {
                if (_IsUseSystemQMS != value)
                {
                    _IsUseSystemQMS = value;
                    NotifyOfPropertyChange(() => IsUseSystemQMS);
                }
            }
        }
        #endregion
        ...
        public bool ShowConfirmSpecialRegistration = Globals.ServerConfigSection.CommonItems.SpecialHIRegistration;
        public void ConfirmEmergencyPatientCmd()
        {
            if (CurRegistration == null || CurRegistration.PtRegistrationID == 0)
            {
                return;
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginConfirmEmergencyOutPtByPtRegistrionID(CurRegistration.PtRegistrationID, Globals.LoggedUserAccount.StaffID.Value
                            , (long)CurRegistration.V_RegistrationType, Globals.DispatchCallback(asyncResult =>
                            {
                                try
                                {
                                    bool result = contract.EndConfirmEmergencyOutPtByPtRegistrionID(asyncResult);
                                    if (result)
                                    {
                                        MessageBox.Show(eHCMSResources.Z2387_G1_XacNhanThanhCong, eHCMSResources.G0442_G1_TBao);
                                    }
                                    else
                                    {
        ...
        public void ConfirmSpecialRegistrationCmd()
        {
            if (CurRegistration == null || CurRegistration.PtRegistrationID == 0)
            {
                return;
            }
            if (CurRegistration.IsSpecialHIRegistration)
            {
                MessageBox.Show(eHCMSResources.Z3066_G1_DangKyBHDacBietKhongChoDangKy);
                return;
            }

            if (CurRegistration.HisID <= 0)
            {
                MessageBox.Show(eHCMSResources.Z3067_G1_KhongPhaiDangKyBH);
                return;
            }

            if (CurRegistration.OutPtTreatmentProgramID > 0)
            {
                MessageBox.Show(eHCMSResources.Z3068_G1_DangKyDieuTriNgoaiTruKhongTheXacNhanDangKyBHDacBiet);
                return;
            }
            ...
            var t = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        var contract = serviceFactory.ServiceInstance;
                        contract.BeginConfirmSpecialRegistrationByPtRegistrationID(CurRegistration, Globals.LoggedUserAccount.StaffID.Value, Globals.DispatchCallback(asyncResult =>
                            {
                                try
                                {
                                    bool result = contract.EndConfirmSpecialRegistrationByPtRegistrationID(asyncResult);
                                    CurRegistration.IsSpecialHIRegistration = result;
                                    if (result)
                                    {
                                        MessageBox.Show(eHCMSResources.Z2387_G1_XacNhanThanhCong, eHCMSResources.G0442_G1_TBao);
                                    }
        ...
        public void Handle(SetTicketForNewRegistrationAgain message)
        {
            ...
            if (message != null && message.Item != null)
            {
                ...
                if (!string.IsNullOrEmpty(message.Item.PatientCode))
                {
                    SearchPatientByPatientCode(message.Item, true);
                }
            }
            else
            {
        ...
        public IEnumerator<IResult> DoSetCurrentPatientForRegistrationByTicketAgain(Patient patient)
        {
            bCreateNewRegistration = true;
            var p = patient;
            if (p == null || p.PatientID <= 0)
            ...
            if (loadPatient.CurrentPatient != null)
            {
                ...
                //▼===== 20200822 TTM: Xóa các code không cần thiết vì hàm này viết ra để chỉ mở đăng ký mới cho bệnh nhân tạo mới đăng ký thì không cần phải kiểm tra liên quan đến đăng ký cũ
                if (_currentPatient.AppointmentList == null || _currentPatient.AppointmentList.Count <= 0)
                {
                    TitleStatus = RegStatus.ChuaDangKy;
                    DoCreateNewRegistration();
        ...
        private ObservableCollection<Lookup> _ListV_MedicalServiceParentGroup;
        public ObservableCollection<Lookup> ListV_MedicalServiceParentGroup
        {
            get { return _ListV_MedicalServiceParentGroup; }
            set
            {
                if (_ListV_MedicalServiceParentGroup != value)
                    _ListV_MedicalServiceParentGroup = value;
                NotifyOfPropertyChange(() => ListV_MedicalServiceParentGroup);
            }
        }
        private bool _IsKioskPack;
        public bool IsKioskPack
        {
            get { return _IsKioskPack; }
            set
            {
                if (_IsKioskPack != value)
                    _IsKioskPack = value;
                NotifyOfPropertyChange(() => IsKioskPack);
            }
        }
        
        public void AxComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            IsKioskPack = RefMedicalServiceGroupObj.V_MedicalServiceGroupType == (long)AllLookupValues.V_MedicalServiceGroupType.Kiosk;
        }
    }
