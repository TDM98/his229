...
{
    [Export(typeof(IPrescriptionOutPtTreatment)), PartCreationPolicy(CreationPolicy.NonShared)]
    public partial class PrescriptionOutPtTreatmentViewModel: Common.ViewModels.PharmacyHandlePayCompletedViewModelBase, IPrescriptionOutPtTreatment
        ...
        , IHandle<RemoveItem<InPatientBillingInvoice>>
    {
        ...
        public PrescriptionOutPtTreatmentViewModel(IWindsorContainer container, INavigationService navigationService, IEventAggregator eventArg)
        {
            authorization();
            //==== #001
        ...
        public bool mBanThuocTheoToa_ThuTien
        {
            get
            {
                return _mBanThuocTheoToa_ThuTien;
            }
            set
            {
                            ...
                            try
                            {
                                ...
                                if (Globals.CheckValidRegistrationForPay(RegistrationInfo, out mErrorMessage, this.IsConfirmHIView ? (long)AllLookupValues.V_TradingPlaces.QUAY_XAC_NHAN : (long)AllLookupValues.V_TradingPlaces.NHA_THUOC))
                                {
                                    Action<ISimplePay> onInitDlg = delegate (ISimplePay vm)
                                    {
                                        ...
                                        //▼===== #010:  Set V_TradingPlaces theo tình trạng bảo hiểm của đăng ký cho chính xác vì nếu không set lại.
                                        //              Bệnh nhân dịch vụ khi trả tiền sẽ phát sinh 1 hoá đơn rỗng (Vì quầy đang là quầy xác nhận và không có thông tin
                                        //              của bất cứ thứ gì về bảo hiểm nên trả về hoá đơn rỗng)
                                        if (RegistrationInfo.HisID > 0)
                                        {
                                        ...
                                        vm.IsOutPtTreatmentPrescription = true;
        ...
        private void LoadInfoCommon()
        {
            if (SelectedPrescription != null)
            {
                ...
                strNgayDung = "";
                ((PrescriptionOutPtTreatmentView)this.GetView()).AutoDrug_Text.Text = "";
                ((PrescriptionOutPtTreatmentView)this.GetView()).chbBH.IsChecked = false;
                //---------------------------------------------------------
                SelectedOutInvoice = null;
                SelectedOutInvoice = new OutwardDrugInvoice();
                SelectedOutInvoice.OutDate = Globals.ServerDate.Value;
        ...
        private IEnumerator<IResult> DoGetInfoPatientPrescription()
        {
            ...
            //▼===== #010:  Để xác định xem toa thuốc vừa chọn có phải là toa đã bán rồi hay không, nếu bán rồi mà trong toa có thuốc
            //              bảo hiểm thì cờ này sẽ giúp store load dữ liệu lấy được thuốc trong kho bảo hiểm và set tiền liên quan bảo hiểm về 0 đồng
            bool SecondTimesSell = false;
            if (SelectedPrescription.IsSold)
            {
                SecondTimesSell = true;
            }

            if (IsHIPatient && !SecondTimesSell && !IsLoadPrescriptionOne)
            {
                GetOutwardDrugForOutPtTreatment(0, StoreID, isobject, true, SelectedPrescription.PtRegistrationID); 
            }
            else if (IsHIPatient && !SecondTimesSell && IsLoadPrescriptionOne)
            {
                GetOutwardDrugForOutPtTreatment(SelectedPrescription.IssueID, StoreID, isobject, true, null);
            }
            else
            {
                GetOutwardDrugForOutPtTreatment(SelectedPrescription.IssueID, StoreID, isobject, true, null, SecondTimesSell);
            }
            //▲=====
        ...
        private void SearchPrescription(int PageIndex, int PageSize)
        {
            ...
            {
                if (PatientInfo != null && PatientInfo.LatestRegistration != null && PatientInfo.LatestRegistration.OutPtTreatmentProgramID > 0)
                {
                    SearchCriteria.FromDate = Globals.GetCurServerDateTime().AddDays(-30);
                }
                else
                {
                ...
                using (var serviceFactory = new ePrescriptionsServiceClient())
                {
                    ...
                    contract.BeginSearchPrescription(SearchCriteria, PageIndex, PageSize, true, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            ...
                            {
                                //▼===== #010:  Dù tìm được 1 hay nhiều toa thuốc thì luôn luôn bung popup lên để người dùng lựa chọn, không tự động chọn nếu chỉ
        ...
        public void GetOutwardDrugForOutPtTreatment(long IssueID, long aStoreID, Int16 aIsObject, bool IsIssueID = true, long? PtRegistrationID = null, bool SecondTimesSell = false)
        {
            if (SelectedOutInvoice == null)
            {
                return;
            ...
            var t = new Thread(() =>
            {
                using (var serviceFactory = new PharmacyInwardDrugServiceClient())
                {
                    var contract = serviceFactory.ServiceInstance;
                    SelectedOutInvoice.OutwardDrugs = new ObservableCollection<OutwardDrug>();
                    contract.BeginGetOutwardDrugForOutPtTreatment(IssueID, aStoreID, aIsObject, IsHIOutPt, true, PtRegistrationID, SecondTimesSell, Globals.DispatchCallback((asyncResult) =>
                    {
                        try
                        {
                            var results = contract.EndGetOutwardDrugForOutPtTreatment(asyncResult);
                            if (results != null)
                            {
                                if (SelectedOutInvoice != null)
                                {
        ...
        {
            //▼===== #010: Luôn luôn tìm kiếm bằng đăng ký đã tìm ra ở màn hình cha (PrescriptionAndConfirmViewModel). Vì trong màn hình con đã bỏ textbox tìm kiếm
            SearchCriteria = new PrescriptionSearchCriteria();
            if (PatientInfo != null && PatientInfo.LatestRegistration != null)
            {
        ...
        public void Handle(PharmacyCloseSearchPrescriptionEvent message)
        {
            if (message != null)
            {
                //▼===== #010:  Tạo ra hàm load mới để load toa cũ đã bán lên bán tiếp lần 2, nếu sử dụng lại hàm cũ thì khi tìm lên thấy đã bán sẽ load lại toàn bộ
                //              Đăng ký và phiếu xuất => Không thể bán lần 2 được.
                LoadPrescriptionDetailOne(message.SelectedPrescription as Prescription);
                //▲===== #010
            }
        }
        #endregion
        ...
        //▼===== #010:  Hàm này tạo ra với mục đích nếu load bệnh nhân mà danh sách toa thuốc và danh sách toa thuốc đã bán có số lượng bằng nhau thì
        //              Load toàn bộ thông tin bệnh nhân lên. 
        public void SearchInvoiceFull()
        {
            SearchCriteria = new PrescriptionSearchCriteria();
            if (PatientInfo != null && PatientInfo.LatestRegistration != null)
            {
                SearchOutwardPrescriptionIndex(0, Globals.PageSize, PatientInfo.LatestRegistration.PtRegistrationID);
            }
            else
        {
                MessageBox.Show(eHCMSResources.A0635_G1_Msg_InfoKhCoDKDeThaoTac);
        ...
        //▼===== #010: Fix lại để tìm kiếm danh sách phiếu xuất sẽ chỉ tìm theo bệnh nhân hiện tại trên màn hình không tìm toàn bộ bệnh nhân.
        ...
        public void SearchInvoiceOld()
        {
            ...
            if (PatientInfo != null && PatientInfo.LatestRegistration != null)
            {
                SearchOutwardPrescriptionIndex0(0, Globals.PageSize, PatientInfo.LatestRegistration.PtRegistrationID);
            }
            else
                {
                MessageBox.Show(eHCMSResources.A0635_G1_Msg_InfoKhCoDKDeThaoTac);
        ...
        private IEnumerator<IResult> DoGetInfoPatientInvoice(bool isCallByPayViewModel = false, bool aIsComfirmHICalled = false)
        {
            ...
            SearchGetDrugForSellVisitorByPrescriptID();
            if (!IsLoadOne && IsConfirmHIView && (PatientInfo != null && PatientInfo.LatestRegistration != null && PatientInfo.LatestRegistration.PtInsuranceBenefit.GetValueOrDefault(0) > 0))
            {
                //▼====: #006
        ...
        private void SearchOutwardPrescriptionIndex0(int PageIndex, int PageSize, long? PtRegistrationID = null)
        {
            int Total = 0;
            this.ShowBusyIndicator();
            if (SearchInvoiceCriteria == null)
        ...
        private void SearchOutwardPrescriptionIndex(int PageIndex, int PageSize, long? PtRegistrationID = null)
        {
            int Total = 0;
            this.ShowBusyIndicator();
                        ...
                        try
                        {
                            ...
                            if (results != null && results.Count > 0)
                            {
                                if (results.Count > 1 && this.IsConfirmHIView && results.Select(x => x.PtRegistrationID).Distinct().Count() == 1)
                                {
                                    ...
                                    //▼===== 20200630 TTM:  Sửa lại điều kiện load phiếu xuất để hiển thị trên màn hình. Do để Descending PaidTime nên luôn luôn lấy phiếu xuất đã trả tiền để lên màn hình
                                    //                      dẫn tới nếu bệnh nhân có phiếu xuất chưa trả tiền thì sẽ không thu tiền được do nút thu tiền ẩn theo điều kiện (PaidTime) của phiếu xuất.
                                    ...
                                    SelectedOutInvoice = results.OrderBy(x => x.V_OutDrugInvStatus).OrderBy(x => x.PaidTime).ThenByDescending(x => x.IsHICount).ThenByDescending(x => x.StoreID).FirstOrDefault();
                                    //▲===== 
                                    SelectedPrescription = SelectedOutInvoice.SelectedPrescription;
        ...
        private bool _IsLoadPrescriptionOne = false;
        public bool IsLoadPrescriptionOne
        {
            get { return _IsLoadPrescriptionOne; }
            set
            {
                if (_IsLoadPrescriptionOne != value)
                {
                    _IsLoadPrescriptionOne = value;
                    NotifyOfPropertyChange(() => IsLoadPrescriptionOne);
                }
            }
        }
        ...
        private bool _IsLoadOne = false;
        public bool IsLoadOne
        {
            get { return _IsLoadOne; }
            set
            {
                if (_IsLoadOne != value)
                {
                    _IsLoadOne = value;
                    NotifyOfPropertyChange(() => IsLoadOne);
                }
            }
        }

        public void LoadPrescriptionDetailOne(Prescription aPrescription)
        {
            SelectedPrescription = aPrescription;
            IsLoadPrescriptionOne = true;
            if (SelectedPrescription.IsSold)
            {
                if (MessageBox.Show(eHCMSResources.Z0896_G1_ToaDaBanCoMuonBanTiepKg, eHCMSResources.G0442_G1_TBao, MessageBoxButton.OKCancel) == MessageBoxResult.Cancel)
                {
        ...
        private ObservableCollection<Prescription> _ListPrescriptionForConfirm = new ObservableCollection<Prescription>();
        ...
        private ObservableCollection<Prescription> _ListPrescriptionIsNotSold = new ObservableCollection<Prescription>();
        ...
        private IEnumerator<IResult> GetDataForConfirmTask(long PtRegistrationID)
        {
            ...
            {
                FirstPrescriptionOnList = ListPrescriptionIsNotSold.FirstOrDefault();
            }
        ...
        public void SetCurPatientAndRegistration(PatientRegistration ObjRegistration)
        {
            if (ObjRegistration != null && ObjRegistration.Patient != null)
            {
                PatientInfo = ObjRegistration.Patient;
                PatientInfo.LatestRegistration = ObjRegistration;
            }
        }
    }
}