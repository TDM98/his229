 ...
 * 20170610 #003 CMN: Removed recheck count hi follow HIPrice when RecalHI bill
 * 20180523 #004 TBLD: Added parameter billingInv.InvDate
*/
...
namespace CommonServices
{
    ...
    public abstract class RegAndPaymentProcessorBase
    {
        ...
        protected void InitTxd(long registrationID, int FindPatient, bool loadFullInfo = true)
        {
            ...
            CurrentRegistration = GetRegistrationTxd(registrationID, FindPatient, true);
        ...
        public void InitNewTxd(PatientRegistration regInfo, bool loadFullInfo, long V_RegistrationType = 0)
        {
            if (regInfo == null)
            {
            ...
            if (regInfo.PtRegistrationID > 0)
            {
                if (loadFullInfo)
                {
                    InitTxd(regInfo.PtRegistrationID, regInfo.FindPatient);
                }
                else
                {
                                                ...
                                                , long V_RegistrationType, DbConnection conn, DbTransaction tran, out long newRequestId)
        {
            ...
            PatientProvider.Instance.AddPCLRequestWithDetails(registrationID.GetValueOrDefault(), V_RegistrationType, medicalRecordId, pclRequest, conn, tran, out newRequestId);
        }
        ...
        //********************************************
        protected virtual void GetBillingInvItemPrice_New(MedRegItemBase invoiceItem, PatientRegistration registration, double hiBenefit)
        {
            invoiceItem.HisID = null;
            invoiceItem.HIBenefit = null;
            ...
            if (invoiceItem.ChargeableItem != null)
            {
                ...
                else
                {
                    //==== #001
                    //invoiceItem.InvoicePrice = invoiceItem.ChargeableItem.NormalPrice;
                    if (hiBenefit > 0.0 && invoiceItem.ChargeableItem.HIPatientPrice > 0)
            ...
            using (var conn = PatientProvider.Instance.CreateConnection())
            {
                ...
                try
                {
                    ...
                    FinializeBillingInv_CreateTranPaymt_ConsolidateTranDetailsAndAdvPaymt(bCreateNewTran, currentTransaction, colBillingInvoices, paidTime, balancedTranDetails,
                        registrationInfo.PtRegistrationID, (long)registrationInfo.V_RegistrationType, ListIDOutTranDetails, paymentDetails, currentTransaction.TransactionID, out NewTransactionIDReturned, conn, tran);
        ...
        protected virtual void FinializeBillingInv_CreateTranPaymt_ConsolidateTranDetailsAndAdvPaymt(bool bCreateNewTran, PatientTransaction newTranInfo, List<InPatientBillingInvoice> colBillingInvoices, 
                    DateTime paidTime, List<PatientTransactionDetail> balancedTranDetails, 
                    long PtRegistrationID, long V_RegistrationType, string ListIDOutTranDetails, PatientTransactionPayment paymentDetails, long transactionId, out long NewTransactionIDRet, DbConnection conn, DbTransaction tran)
        {            
            PatientProvider.Instance.FinializeBillingInv_CreateTranPaymt_ConsolidateTranDetailsAndAdvPaymt(bCreateNewTran, newTranInfo, colBillingInvoices, paidTime, balancedTranDetails, PtRegistrationID, V_RegistrationType, ListIDOutTranDetails, paymentDetails, transactionId, out NewTransactionIDRet, conn, tran);            
        }


        #endregion

        public static PatientRegistration GetRegistrationTxd(long regID, int FindPatient, bool getFullInfo = false)
        {
            if (regID <= 0)
            {
                return null;
            ...
            using (DbConnection connection = PatientProvider.Instance.CreateConnection())
            {
                ...
                {
                    return GetRegistrationNew(regID, FindPatient, connection, null, getFullInfo);
                }
                else
                {
        ...
        protected static PatientRegistration GetRegistrationNew(long regID, int FindPatient, DbConnection connection, DbTransaction tran, bool getFullInfo = false)
        {

            return CalcAllTotalAndPayableAmtOutPt(regID, FindPatient, connection, tran, getFullInfo);

            //KMx: Bỏ tất cả code bên dưới, vì ở trên A.Tuấn đã return rồi (18/09/2014 09:53).
            ...
            ////KMx: Bỏ code bên dưới vì dư thừa, registrationInfo ở trên đã lấy PatientRegistrationDetail rồi, nên không cần lấy lại nữa.
        ...
        protected static PatientRegistration CalcAllTotalAndPayableAmtOutPt(long regID, int FindPatient, DbConnection connection, DbTransaction tran, bool getFullInfo = false)
        {
            PatientRegistration registrationInfo = PatientProvider.Instance.GetRegistration(regID, FindPatient, connection, tran);
            if (registrationInfo == null)
            {
        ...
        public InPatientBillingInvoice LoadInPatientRegItemsIntoBill(PatientRegistration regInfo, long? DeptID, long StoreID, long StaffID, bool IsAdmin)
        {
            ...
            using (var conn = PatientProvider.Instance.CreateConnection())
            {
                ...
                var tempInvoiceList = PatientProvider.Instance.GetAllInPatientInvoices_NoBill(regInfo.PtRegistrationID, DeptID, StoreID, StaffID, IsAdmin, conn, null);
                ...
                if (allBedPatientAlloc != null)
                {
                    foreach (var item in allBedPatientAlloc)
                    {
                        ...
                        if (item.IsDeleted || !item.IsActive || item.CheckOutDate != null)
                        {
                            continue;
                        }
        ...
        public bool SaveRegistrationForOutPatient(long StaffID, long CollectorDeptLocID, int? Apply15HIPercent, DateTime modifiedDate
            ...
            , out string PaymentIDListNy, long V_RegistrationType = 0)
        {
            try
            {
                // Txd 05/07/2012
        ...
        public virtual bool AddPCLRequestsForInPt(long StaffID, long ReqFromDeptLocID, long ReqFromDeptID, PatientPCLRequest pclRequest, PatientPCLRequest deletedPclRequest
                , DateTime modifiedDate, out List<long> newPclRequestIdList)
        {
            ...
            long? medicalFileID;
            CreatePatientMedialRecordIfNotExists(CurrentRegistration.Patient.PatientID, CurrentRegistration.ExamDate, out medicalRecordID, out medicalFileID);
            return PatientProvider.Instance.SavePCLRequestsForInPt(CurrentRegistration.PtRegistrationID, StaffID
                                   , medicalRecordID
                                   , ReqFromDeptLocID
                                   , ReqFromDeptID
                                   , newPclList
                                   , deletedPclRequest
                                   , lstDetail_ReUseServiceSeqNum
                                   , (int)AllLookupValues.RegistrationType.NOI_TRU
                                   , out newPclRequestIdList
                                    );
        }
            ...
            if (!string.IsNullOrEmpty(PaymentIDListNy))
            {
                ...
                ListPaymentIDAndService = (from _student in xdoc1.Element("Root").Elements("IDList")
                                           ...
                                           select new PaymentAndReceipt
                                           {
                                               ...
                                               IsPrintReceiptForPT = Convert.ToBoolean(_student.Element("IsPrintReceiptForPT").Value),
                                               IsPrintReceiptForHI = Convert.ToBoolean(_student.Element("IsPrintReceiptForHI").Value)
                                           }).ToList();
        ...
        {
            if (CurrentRegistration.V_RegistrationType != AllLookupValues.RegistrationType.NOI_TRU)
                throw new Exception(eHCMSResources.Z1837_G1_ChonDKNoiTru);
        ...
        public virtual void RecalcInPatientBillingInvoice(out string ListIDOutTranDetails, long? StaffID, PatientRegistration registrationInfo, InPatientBillingInvoice billingInv)
        {
            ...
            using (var conn = PatientProvider.Instance.CreateConnection())
            {
                ...
                try
                {
                            ...
                            foreach (var outwardDrug in inv.OutwardDrugClinicDepts)
                            {
                                if (registrationInfo.PtInsuranceBenefit.GetValueOrDefault(0) > 0)
                                {
                                    if (outwardDrug.GenMedProductItem != null && outwardDrug.GenMedProductItem.InsuranceCover == true)
                                    {
                                        ...
                                        /*▼====: #003*/
                                        if (outwardDrug.HIBenefit > 0 && outwardDrug.HIAllowedPrice > 0 && !billingInv.IsHighTechServiceBill)
                                        {
                    ...
                    if (billingInv.RegistrationDetails != null && billingInv.RegistrationDetails.Count > 0)
                    {
                        foreach (PatientRegistrationDetail p in billingInv.RegistrationDetails)
                        {
                            ...
                            if (p.RefMedicalServiceItem.V_NewPriceType != (Int32)AllLookupValues.V_NewPriceType.Unknown_PriceType && p.RefMedicalServiceItem.V_NewPriceType != (Int32)AllLookupValues.V_NewPriceType.Updatable_PriceType)
                            {
                                ...
                                /*▼====: #003*/
                                if (p.HIBenefit > 0 && p.HIAllowedPrice > 0 && !billingInv.IsHighTechServiceBill)
                                {
                                    p.IsCountHI = true;
                                }
                                /*▼====: #003*/
                    ...
                    /*▼====: #004*/
                    PatientProvider.Instance.CalAdditionalFeeAndTotalBill(billingInv.InPatientBillingInvID, StaffID.GetValueOrDefault(), true, conn, tran, billingInv.InvDate, billingInv.HIBenefit, billingInv.IsHICard_FiveYearsCont_NoPaid.GetValueOrDefault(false));
                    /*▲====: #004*/
                    if (billingInv.PaidTime != null)
                    {
                        if (registrationInfo.PatientTransaction != null)
