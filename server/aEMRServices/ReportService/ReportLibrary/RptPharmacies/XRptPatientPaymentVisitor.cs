...
{
    public partial class XRptPatientPaymentVisitor : DevExpress.XtraReports.UI.XtraReport
    {
        public XRptPatientPaymentVisitor()
        {
            InitializeComponent();
        ...
        public void FillData()
        {
            ...
            if (dsReceiptDrugVisistor1.spRpt_Patients_GetReceiptDrugVisistor != null && dsReceiptDrugVisistor1.spRpt_Patients_GetReceiptDrugVisistor.Rows.Count > 0)
            {
                ...
                if (total.HasValue)
                {
                    ...
                    if (total < 0)
                    {
                        temp = 0 - total.GetValueOrDefault();
                        prefix = string.Format(" {0} ",  eHCMSResources.Z0873_G1_Am.ToLower());
                    }
                    else
                    {
                        temp = total.GetValueOrDefault();
                        prefix = "";
                    }
                    this.Parameters["param_AmountInWords"].Value = prefix + converter.Convert(temp.ToString("0.00"), cutureInfo.NumberFormat.CurrencyDecimalSeparator.ToCharArray()[0], eHCMSResources.Z0871_G1_Le.ToLower()) + string.Format(" {0}",  eHCMSResources.Z0872_G1_Dong.ToLower());
                }
            }
        }
    }
