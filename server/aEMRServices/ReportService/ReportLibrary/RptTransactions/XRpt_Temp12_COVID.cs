...
{
    public partial class XRpt_Temp12_COVID : DevExpress.XtraReports.UI.XtraReport
    {
        public XRpt_Temp12_COVID()
        {
            InitializeComponent();
        }
        ...
        public void FillData()
        {
            ...
            spRpt_CreateTemp02NoiTruNewTableAdapter.Fill(dsTemp02NoiTruNew1.spRpt_CreateTemp02NoiTruNew
                ...
                , ref DefaultNumFilmsRecvStr, false, false, Convert.ToBoolean(IsCOVID.Value), false);
            spHealthInsuranceHistory_InPtTableAdapter.Fill(dsTemp02NoiTruNew1.spHealthInsuranceHistory_InPt
                ...
                , Convert.ToInt64(DeptID.Value), Convert.ToBoolean(IsKHTHView.Value), false);
            ...
            if (dsTemp02NoiTruNew1.spHealthInsuranceHistory_InPt != null && dsTemp02NoiTruNew1.spHealthInsuranceHistory_InPt.Rows.Count > 0)
            {
                ...
                if (dsTemp02NoiTruNew1.spHealthInsuranceHistory_InPt.Columns["DiagnosisFinal"] != null 
                    && dsTemp02NoiTruNew1.spHealthInsuranceHistory_InPt.Rows[0]["DiagnosisFinal"] != DBNull.Value 
                    && parDiagnosisFinal.Value.ToString() == "")
                {
                    parDiagnosisFinal.Value = ConverToStringForDiagnosisTreatment(dsTemp02NoiTruNew1.spHealthInsuranceHistory_InPt.Rows[0]["DiagnosisFinal"], dsTemp02NoiTruNew1.spHealthInsuranceHistory_InPt.Rows[0]["V_RegForPatientOfType"]);
                }
            }
            ...
            if (Convert.ToBoolean(IsCOVID.Value))
            {
                LBTitle.Text = "BẢNG KÊ CHI PHÍ KHÁM, CHỮA BỆNH NỘI TRÚ";
            }
        }
        ...
        private void ReportFooter1_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (dsTemp02NoiTruNew1.spRpt_CreateTemp02NoiTruNew.Rows.Count > 0)
            {
                OtherPayment = dsTemp02NoiTruNew1.spRpt_CreateTemp02NoiTruNew.Rows.Cast<DataRow>().Where(x => x["OrtherAmount"] != null && x["OrtherAmount"] != DBNull.Value).Sum(x => Convert.ToDecimal(x["OrtherAmount"]));
            }
            if (Convert.ToInt32(RegistrationType.Value) == 24001)
            {
                totalBNTra = totalCP - totalBHTra - OtherPayment;
            }
            ...
            Parameters["TotalOtherPayment"].Value = OtherPayment;
            ...
            Parameters["ReadMoneyNhaNuocTra"].Value = ReadMoneyToString(OtherPayment);
            decimal ChiphiBH = 0;
            if (dsTemp02NoiTruNew1.spRpt_CreateTemp02NoiTruNew.Rows.Count > 0)
            {
            ...
            Parameters["ReadMoneyBNPhaiDongThem"].Value = ReadMoneyToString(Math.Abs(totalBNPhaiDongThem));
            /*▼====: #001*/
            if (gTotalPatientPaymentPaidAmount > 0)
            {
        ...
        public string ConverToStringForDiagnosisTreatment(object ten_benh, object LoaiKCB)
        {
            string ReturnValue = "";
            if (ten_benh != null && !string.IsNullOrEmpty((Convert.ToString(ten_benh))))
            {
                ...
                {
                    if (Convert.ToString(LoaiKCB) != "24105")
                    {
