...
{
    public partial class XRpt_Form02NoiTru : DevExpress.XtraReports.UI.XtraReport
    {
        public XRpt_Form02NoiTru()
        {
            InitializeComponent();
          //  FillDataInit();
        }
        ...
        public void FillData()
        {
            ...
            spRpt_CreateForm02TableAdapter.Fill(dsForm02NoiTru1.spRpt_CreateForm02, this.RptForm02_InPtIDList.Value.ToString());
            spHealthInsuranceHistory_InPtTableAdapter.Fill(dsForm02NoiTru1.spHealthInsuranceHistory_InPt, Convert.ToInt64(this.PtRegistrationID.Value), Convert.ToDateTime(this.DischargeDate.Value));

            decimal totalCP = 0;
            decimal totalBNTra = 0;
            decimal totalBHTra = 0;
            if (dsForm02NoiTru1.spRpt_CreateForm02 != null && dsForm02NoiTru1.spRpt_CreateForm02.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dsForm02NoiTru1.spRpt_CreateForm02.Rows.Count; i++)
                {
                    totalCP += Convert.ToDecimal(dsForm02NoiTru1.spRpt_CreateForm02.Rows[i]["TotalAmount"]);
                    totalBHTra += Convert.ToDecimal(dsForm02NoiTru1.spRpt_CreateForm02.Rows[i]["HealthInsuranceRebate"]);
                }
