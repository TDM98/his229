...
{
    public partial class XRptPatientSettlement_V4 : DevExpress.XtraReports.UI.XtraReport
    {
        public XRptPatientSettlement_V4()
        {
        ...
        void XRptPatientPayment_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (dsPatientSettlement1.sp_Rpt_PatientSettlement != null && dsPatientSettlement1.sp_Rpt_PatientSettlement.Rows.Count > 0)
            {
                ...
                VATPercent = Convert.ToDecimal(dsPatientSettlement1.sp_Rpt_PatientSettlement.Rows[0]["VATPercent"]);
            ...
            decimal totalPTPaymentBeforeVAT = totalBN;
 
            if ((int)Parameters["parReceiptType"].Value == (int)ReceiptType.OUTWARD_MEDDEPT)
            {
                if (VATPercent < 1)
                {
                    throw new Exception("Lỗi: % VAT phải >= 1. Nhưng % VAT của hóa đơn này là: " + VATPercent.ToString() + ".");
                }

                totalPTPaymentBeforeVAT = Math.Round(totalBN / VATPercent);
                Parameters["parVATPercent"].Value = ((VATPercent - 1) * 100).ToString("N0");
                Parameters["parVATAmount"].Value = (totalBN - totalPTPaymentBeforeVAT).ToString("N0") + eHCMSResources.G1616_G1_VND.ToLower();
            }
            else
            {
            ...
            Parameters["parTotalPTPaymentBeforeVAT"].Value = totalPTPaymentBeforeVAT.ToString("N0") + eHCMSResources.G1616_G1_VND.ToLower();
            Parameters["parTotalPTPaymentAfterVAT"].Value = totalBN.ToString("N0") + eHCMSResources.G1616_G1_VND.ToLower();
