 ...
 * 20220331 QTD  #003: Hide table order by AllLookupValues for PCL
 */
namespace eHCMS.ReportLib.RptPatientRegistration
{
    public partial class XRptOutPatientReceipt_V4 : DevExpress.XtraReports.UI.XtraReport
    {
        public XRptOutPatientReceipt_V4()
        {
        ...
        void XRptPatientPayment_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (Convert.ToBoolean(parIsGenericPayment.Value))
            {
                ...
                xrLabel2.Text = null;
                // receipt number
                xrLabel27.ExpressionBindings.Clear();
                xrLabel27.Text = parGenPaymtCode.Value.ToString();
                //full name
                xrLabel33.ExpressionBindings.Clear();
                xrLabel33.Text = parGenPaymtCustName.Value.ToString();
                // patient code
                xrLabel26.ExpressionBindings.Clear();
                xrLabel26.Text = parPatientCode != null ? parPatientCode.Value.ToString() : "";
                // age
            ...
            if (outPatientReceipt1.sp_Rpt_spReportOutPatientCashReceipt_ByPaymentID != null && outPatientReceipt1.sp_Rpt_spReportOutPatientCashReceipt_ByPaymentID.Rows.Count > 0)
            {
                DOB = (DateTime)outPatientReceipt1.sp_Rpt_spReportOutPatientCashReceipt_ByPaymentID.FirstOrDefault()["DOB"];
            }
            int age = IssueDate.Year - DOB.Year;
            int monthnew = (IssueDate.Month + IssueDate.Year * 12) - (DOB.Month + DOB.Year * 12);
            if (monthnew <= 72)
            {
                xrLabel4.Text = monthnew.ToString() + " Tháng";
            }
            else
            {
                xrLabel4.Text = age.ToString();
            }
            ...
            if (outPatientReceipt1.sp_Rpt_spReportOutPatientCashReceipt_ByPaymentID != null && outPatientReceipt1.sp_Rpt_spReportOutPatientCashReceipt_ByPaymentID.Rows.Count > 0)
            {
                foreach (DataRow row in outPatientReceipt1.sp_Rpt_spReportOutPatientCashReceipt_ByPaymentID.Rows)
                {
                    //string deptLocID = "|" + row["DeptLocID"] + "|";
                    //if (Parameters["parDeptLocIDQMS"].Value.ToString().Contains(deptLocID))
                    //{
                    ...
                    if(row["ServiceSeqNum"].ToString() != "")
                    {
                        ...
                        xrLabel35.Text = "STT: " + outPatientReceipt1.sp_Rpt_spReportOutPatientCashReceipt_ByPaymentID.Rows.Cast<DataRow>()
                                .Where(x => x["ServiceSeqNum"] != null && x["ServiceSeqNum"] != DBNull.Value).Select(x => x["ServiceSeqNum"]).FirstOrDefault().ToString();
                        break;
                    }
                }
            }
        ...
        //{
        //    ((XRptOutPatientReceipt_V4_Sub)((XRSubreport)sender).ReportSource).Parameters["param_PaymentID"].Value = Convert.ToInt64(param_PaymentID.Value.ToString());
        //    ((XRptOutPatientReceipt_V4_Sub)((XRSubreport)sender).ReportSource).Parameters["pOutPtCashAdvanceID"].Value = Convert.ToInt64(pOutPtCashAdvanceID.Value.ToString());
        //}
    }
}
